...
SOURCE_EXPECTATIONS = {
    "curated-official-2026-07-20-ecodatacenter-falun-data-center-e.json": {
        "bytes": 16_687,
        "sha256": "9d45a25c2eff5eebc59d9a070f741f2173acab709b633509b2e66e61583478d0",
        "project_key": (
            "curated:ecodatacenter-falun-data-center-campus:data-center-e-current-build"
        ),
        "project_name": "EcoDataCenter Falun Data Center E",
    },
    "curated-official-2026-07-20-ecodatacenter-falun-data-center-f.json": {
        "bytes": 16_687,
        "sha256": "72c1901834a6d8f31c985b342572d8de008772117704fa5e5ab5c2f4d9e79af3",
        "project_key": (
            "curated:ecodatacenter-falun-data-center-campus:data-center-f-current-build"
        ),
        "project_name": "EcoDataCenter Falun Data Center F",
    },
}
SOURCES = tuple(ROOT / "sources" / name for name in SOURCE_EXPECTATIONS)
RECORDED_AT = "2026-07-21T04:51:39Z"

ANNUAL_KEY = "ecodc-2025-annual-report-falun-e-f-captured-2026-07-20"
Q1_KEY = "ecodc-q1-2026-platform-construction-captured-2026-07-20"
CAMPUS_KEY = "curated:ecodatacenter-falun-data-center-campus"
PROJECT_E_KEY = SOURCE_EXPECTATIONS[SOURCES[0].name]["project_key"]
PROJECT_F_KEY = SOURCE_EXPECTATIONS[SOURCES[1].name]["project_key"]
...
class EcoDataCenterFalunEFCuratedTests(unittest.TestCase):
    def _load(self, source: Path) -> dict[str, Any]:
        return json.loads(source.read_text(encoding="utf-8"))

    def _block_network(self, stack: ExitStack) -> None:
        failure = AssertionError(
            "EcoDataCenter E/F curated import attempted network access"
        )
        for name in (
            "socket",
            "create_connection",
    ...
    def test_capture_triples_are_exact_official_and_credential_free(self) -> None:
        ...
        for key, expected in CAPTURES.items():
            with self.subTest(evidence=key):
                ...
                self.assertEqual(item["publisher"], "EcoDC Holding AB (publ)")
                self.assertEqual(
                    item["source_family"], "ecodatacenter_financial_reports"
                )
                ...
                self.assertEqual(metadata["curl_size_header_bytes"], 1_357)
                self.assertEqual(metadata["curl_num_headers"], 28)
                self.assertEqual(
        ...
        with ExitStack() as stack:
            ...
            self.assertEqual(
                ...
                (
                    ...
                    ("project", PROJECT_E_KEY, RECORDED_AT),
                    ("project", PROJECT_F_KEY, RECORDED_AT),
                ),
            )
            relations = tuple(
                tuple(row)
            ...
            self.assertEqual(
                relations,
                ((PROJECT_E_KEY, CAMPUS_KEY), (PROJECT_F_KEY, CAMPUS_KEY)),
            )
            ...
            self.assertEqual(
                ...
                (CAMPUS_KEY, PROJECT_E_KEY, PROJECT_F_KEY),
            )
            self.assertEqual(
                tuple(row["name"] for row in snapshots),
                (
                    "EcoDataCenter Falun Data Center Campus",
                    "EcoDataCenter Falun Data Center E",
                    "EcoDataCenter Falun Data Center F",
                ),
            )
    ...
    def test_scope_preserves_phases_without_capacity_status_or_location_inference(
        self,
    ) -> None:
        documents = [self._load(source) for source in SOURCES]
        self.assertEqual(
            {document["project"]["stable_key"] for document in documents},
            {PROJECT_E_KEY, PROJECT_F_KEY},
        )
        self.assertEqual(
        ...
        for document in documents:
            ...
            for entity in (document["campus"], document["project"]):
                ...
                self.assertEqual(entity["address"], "Falun, Sweden")
                self.assertEqual(
                    entity["roles"],
                    {
                ...
                self.assertEqual(entity["evidence_key"], ANNUAL_KEY)
                self.assertEqual(entity["as_of_date"], "2025-12-31")
            ...
            self.assertEqual(normalized_evidence_keys, {ANNUAL_KEY})
            ...
            self.assertEqual(
                ...
                "Construction of Data Centers E and F began in 2025",
            )
            self.assertEqual(
                (
                    annual["reported_falun_capacity_at_balance_sheet_date_mw"],
                    annual["reported_falun_capacity_after_e_and_f_mw"],
                    annual["implied_combined_e_and_f_capacity_delta_mw"],
                ),
                (41, 65, 24),
            )
            self.assertIn("metadata only", annual["capacity_guardrail"])
            self.assertIn("not split", annual["capacity_guardrail"])
            self.assertIn(
                "not a source-typed critical IT", annual["capacity_guardrail"]
            )
            self.assertEqual(
                annual["data_center_f_forecast_as_reported"],
                "expected to be operational in Q3 2026",
            )
            self.assertEqual(
                annual["data_center_e_forecast_as_reported"],
                "expected to be operational at the end of Q4 2026",
            )
            self.assertIn("forward-looking", annual["forecast_guardrail"])
            self.assertIn("create no target date", annual["forecast_guardrail"])
                ...
                try:
                    with self.assertRaisesRegex(
                        ...
                        CuratedOfficialSourceAdapterV11().import_file(
                            connection,
                            source,
                            recorded_at="2026-07-21T04:51:38Z",
                        )
                    for table in (
                        "evidence",
                        "entities",
    ...
    def test_falun_e_f_are_unseeded_in_v61_v62(self) -> None:
        ...
        for release in RELEASES:
            with self.subTest(release=release.name):
                ...
                self.assertTrue(
                    {CAMPUS_KEY, PROJECT_E_KEY, PROJECT_F_KEY}.isdisjoint(stable_keys)
                )
                falun_rows = [
                    row
                    for row in entities
                ...
                self.assertTrue({ANNUAL_KEY, Q1_KEY}.isdisjoint(evidence_keys))
