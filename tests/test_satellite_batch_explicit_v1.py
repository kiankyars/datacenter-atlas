...
class ExplicitSatelliteBatchV1Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.config = ExplicitBatchConfig(minimum_interval_seconds=0.1)
    ...
    def test_receipt_represents_98_but_runs_only_ordered_11_once(self) -> None:
        ...
        self.assertEqual(_called_queue_ids(runner), list(AUDITED_QUEUE_IDS))
        self.assertTrue(
        ...
        self.assertEqual(
            manifest["summary"],
            {
                "jobs_completed": 11,
                "jobs_failed": 0,
                "jobs_not_selected": 87,
                "jobs_pending": 87,
                "jobs_represented": 98,
                "jobs_selected_for_execution": 11,
                "jobs_unavailable_no_scene": 0,
                "selected_jobs_completed": 11,
        ...
        self.assertEqual(
            manifest["lifetime_budget"],
            {
                ...
                "http_attempts_reserved": 22,
            },
        )
        receipt_path = output / SELECTION_RECEIPT_FILENAME
        receipt = json.loads(receipt_path.read_bytes())
        self.assertEqual(
            receipt["selection"]["selected_queue_ids"], list(AUDITED_QUEUE_IDS)
        )
        self.assertEqual(receipt["representation"]["represented_jobs"], 98)
        self.assertEqual(receipt["representation"]["distinct_aois"], 94)
        ...
        resumed = execute_explicit_satellite_batch_v1(
            self.queue,
            output,
            config=self.config,
        ...
        self.assertEqual(resumed["lifetime_budget"]["http_attempts_reserved"], 22)

    def test_partial_runs_reserve_two_each_under_lifetime_22(self) -> None:
        output = self.root / "partial"
        first_runner = _CatalogRunner()
        first = execute_explicit_satellite_batch_v1(
            self.queue,
            output,
            config=self.config,
            max_jobs=2,
        ...
        self.assertEqual(_called_queue_ids(first_runner), list(AUDITED_QUEUE_IDS[:2]))
        self.assertEqual(first["summary"]["jobs_pending"], 96)
        self.assertEqual(first["lifetime_budget"]["http_attempts_reserved"], 4)

        second_runner = _CatalogRunner()
        second = execute_explicit_satellite_batch_v1(
            self.queue,
            output,
            config=self.config,
            max_jobs=9,
        ...
        self.assertEqual(_called_queue_ids(second_runner), list(AUDITED_QUEUE_IDS[2:]))
        self.assertEqual(second["summary"]["jobs_pending"], 87)
        self.assertEqual(second["lifetime_budget"]["http_attempts_reserved"], 22)

        with self.assertRaisesRegex(ExplicitSatelliteBatchError, "eleven-job"):
            execute_explicit_satellite_batch_v1(
                self.queue,
        ...
        with self.assertRaisesRegex(ExplicitSatelliteBatchError, "lifetime cap 22"):
            execute_explicit_satellite_batch_v1(
                self.queue,
    ...
    def test_receipt_tamper_and_unexpected_output_stop_before_commands(self) -> None:
        ...
        receipt.chmod(0o444)
        runner = _CatalogRunner()
        with self.assertRaisesRegex(ExplicitSatelliteBatchError, "receipt changed"):
            execute_explicit_satellite_batch_v1(
                self.queue,
                output,
                config=self.config,
        ...
        runner = _CatalogRunner()
        with self.assertRaisesRegex(ExplicitSatelliteBatchError, "adoption prohibited"):
            execute_explicit_satellite_batch_v1(
                self.queue,
                output,
                config=self.config,
    ...
    def test_late_job_collision_is_never_replaced_or_adopted(self) -> None:
        ...
        with self.assertRaisesRegex(ExplicitSatelliteBatchError, "late catalog final collision"):
            execute_explicit_satellite_batch_v1(
                self.queue,
                output,
                config=self.config,
        ...
        sentinel = output / "jobs" / AUDITED_QUEUE_IDS[0] / "catalog" / "sentinel.txt"
    ...
    def test_output_symlink_is_rejected(self) -> None:
        ...
        runner = _CatalogRunner()
        with self.assertRaisesRegex(ExplicitSatelliteBatchError, "symlink"):
            execute_explicit_satellite_batch_v1(
                self.queue,
                link,
                config=self.config,
