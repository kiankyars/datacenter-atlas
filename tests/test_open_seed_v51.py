...
from datacenter_atlas import open_seed_v51 as v51
from datacenter_atlas.open_seed_release import validate_open_seed_release


ROOT = Path(__file__).resolve().parents[1]
WORKSPACE = ROOT.parent
DEFINITION = ROOT / "sources/open-seed-2026-07-20-v51.json"
RELEASE = ROOT / "releases/2026-07-20-open-seed-v51"
BASE_DEFINITION = ROOT / "sources/open-seed-2026-07-20-v49.json"
BASE_RELEASE = ROOT / "releases/2026-07-20-open-seed-v49"
BUILDER = ROOT / "scripts/build_open_seed_v51.py"
VALIDATOR = ROOT / "scripts/validate_open_seed_release.py"

DEFINITION_SHA256 = "eca98402dfcb5e7c2c9b9a16093dcf852d20afa932d047966e3409c07d1d3837"
MANIFEST_SHA256 = "c0534405a668df93a783e7d7bdd4cb4f18898879892f559a0dfee914ea081235"
...
RECORDED_AT = "2026-07-20T17:33:00Z"
MAX_SELECTED_RETRIEVED_AT = "2026-07-20T16:57:56Z"
TREE_SHA256 = "7ad5386590b7a8d76f4732a8020bea4c55df0ba6847fa96987a3f12f2e014a8f"
BASE_TREE_SHA256 = (
...
RELEASE_FILE_PINS = {
    ...
    "atlas.geojson": (
        2_264_090,
        "ed625dd26e37e10afc7f56398bfd2a3143ec326735566cce2e951294f9739dba",
    ),
    ...
    "summary.json": (
        2_807,
        "a645389ead262cbc0db88f8e59e4c972d9d1f92edb1367778d53f1b6741aad69",
    ),
}
...
class OpenSeedV51Tests(unittest.TestCase):
    def _validate_offline(self) -> dict[str, object]:
        blocked = AssertionError("v51 validation attempted network access")
        original_read_bytes = Path.read_bytes
        original_read_text = Path.read_text
        rejected = tuple(
            f"open-seed-2026-07-20-v{version}" for version in (45, 46, 48, 50)
        )
        ...
        def guarded_read_bytes(path: Path) -> bytes:
            if forbidden(path):
                raise AssertionError(f"v51 attempted rejected seed access: {path}")
    ...
    def test_recorded_at_is_historical_and_clock_direction_is_correct(self) -> None:
        ...
        with self.assertRaisesRegex(
            SystemExit, "at or before the build-start clock"
        ):
            v51.validate_temporal_contract(
                retrieved,
                build_started_at=cutoff - timedelta(microseconds=1),
            )
        v51.validate_temporal_contract(retrieved, build_started_at=cutoff)
        v51.validate_temporal_contract(
            retrieved,
    ...
    def test_definition_is_exact_four_input_addition_to_v49_only(self) -> None:
        ...
        self.assertEqual(current["release_id"], "2026-07-20-open-seed-v51")
        self.assertEqual(current["build"]["recorded_at"], RECORDED_AT)
        self.assertEqual(v51.BASE_DEFINITION, BASE_DEFINITION)
        self.assertEqual(v51.BASE_RELEASE, BASE_RELEASE)
        ...
        implementation = sys.modules[v51.selected_inputs.__module__]
        for version in (42, 45, 46, 47, 48, 50):
            with (
                patch.object(
                    implementation,
                    "BASE_DEFINITION",
    ...
    def test_scope_disclaims_parity_completeness_and_unique_sites(self) -> None:
    ...
    def test_existing_and_active_lock_collisions_preserve_frozen_outputs(self) -> None:
        ...
        self.assertFalse(v51.PUBLICATION_LOCK.exists())

        descriptor = os.open(
            v51.PUBLICATION_LOCK, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600
        )
        try:
            ...
            if descriptor >= 0:
                os.close(descriptor)
            v51.PUBLICATION_LOCK.unlink(missing_ok=True)
    ...
    def test_late_arrival_collision_is_atomic_and_non_mutating(self) -> None:
        with tempfile.TemporaryDirectory(prefix="open-seed-v51-collision-") as temporary:
            root = Path(temporary)
            stage = root / "stage"
            ...
            with self.assertRaisesRegex(
                ...
                v51.promote_noreplace(stage, destination)
    ...
    def test_validator_and_import_work_in_both_layouts(self) -> None:
        ...
        import_command = [
            ...
            "-c",
            (
                "from datacenter_atlas.open_seed_v51 import BASE_DEFINITION, RELEASE; "
