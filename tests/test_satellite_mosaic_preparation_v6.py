...
import datacenter_atlas.satellite_mosaic_preparation_v6 as v6
from datacenter_atlas.satellite_mosaic_preparation_v6 import (
    RELEASE_FILES,
    SatelliteMosaicPreparationError,
    validate_satellite_mosaic_preparation_v6,
    write_satellite_mosaic_preparation_v6,
)
...
RELEASE = ROOT / "satellite_mosaic_preparation/2026-07-19-algorithm-v2-blocked-v6"
V2_RELEASE = ROOT / "satellite_mosaic_preparation/2026-07-19-algorithm-v2-blocked-v2"
CLI = ROOT / "scripts/build_satellite_mosaic_preparation_v6.py"
...
class SatelliteMosaicPreparationV6Tests(unittest.TestCase):
    def _parent(self) -> Path:
        return Path(tempfile.mkdtemp(prefix=".mosaic-v6-test-", dir=ROOT))

    def test_v1_through_v5_rejected_evidence_is_byte_preserved(self) -> None:
        definition_hashes = {
            ...
            5: "cf566ec4d747c11bba4ac9e58975629ca41749b3b1be1c07db7841a9e1ba6f70",
        }
        tree_hashes = {
            ...
            5: "4b5b52f44fe5947bf7d2e2f001f86b9ed50943bed349e6f83d360110564bc8f6",
        }
    ...
    def test_definition_marks_v5_rejected_and_declares_acquisition_ledger(self) -> None:
        definition = json.loads(DEFINITION.read_text(encoding="utf-8"))
        self.assertEqual(definition["schema_version"], 6)
        self.assertEqual(
            definition["rejected_predecessor"],
            {
                "bytes": 2515,
                "mode": "0444",
                "path": "sources/satellite-mosaic-preparation-2026-07-19-algorithm-v2-blocked-v5.json",
                "sha256": "cf566ec4d747c11bba4ac9e58975629ca41749b3b1be1c07db7841a9e1ba6f70",
            },
        )
        contract = definition["publication_contract"]
        self.assertTrue(contract["descriptor_acquisition_cleanup_ledger"])
        self.assertTrue(contract["acquisition_and_cleanup_errors_aggregated"])
        self.assertTrue(contract["requested_target_path_always_reported"])
        self.assertEqual(
            contract["ambiguous_mkdir_failure_policy"],
            "fail_closed_and_report_exact_candidate",
        )

    def test_v6_release_validates_and_preserves_scientific_bytes(self) -> None:
        manifest = validate_satellite_mosaic_preparation_v6(
    ...
    def test_cli_validates_v6_release(self) -> None:
        ...
        self.assertIn(
            '"format": "datacenter-atlas-satellite-mosaic-v3-preparation-v6"',
            completed.stdout,
        )
    ...
    def test_two_socket_blocked_fresh_builds_are_byte_exact(self) -> None:
        ...
        try:
            with patch.object(
                ...
                write_satellite_mosaic_preparation_v6(
                    first, definition_path=DEFINITION
                )
                write_satellite_mosaic_preparation_v6(
                    second, definition_path=DEFINITION
                )
            for name in RELEASE_FILES:
    ...
    def test_initial_target_stat_eio_closes_parent_and_reports_exact_target(self) -> None:
        parent = self._parent()
        output = parent / "release"
        v6._payloads(DEFINITION, output)
        before = _fd_count()
        real_open = v6.os.open
        real_close = v6.os.close
        ...
        try:
            with patch.object(v6.os, "open", side_effect=capture_open), patch.object(
                v6.os, "close", side_effect=capture_close
            ), patch.object(
                v6.v4,
                "_stat_at",
                side_effect=OSError(errno.EIO, "injected initial target-stat EIO"),
            ):
            ...
            self.assertIn("injected initial target-stat EIO", message)
            self.assertIn(f"exact requested target path: {output}", message)
            self.assertIn("staging path was not allocated", message)
            self.assertFalse(output.exists())
        finally:
            _force_remove(parent)

    def test_existing_target_post_close_eio_aggregates_primary_and_cleanup(self) -> None:
        parent = self._parent()
        output = parent / "release"
        output.mkdir()
        v6._payloads(DEFINITION, output)
        ...
        try:
            with patch.object(v6.os, "open", side_effect=capture_open), patch.object(
                v6.os, "close", side_effect=real_close_then_raise
            ):
                with self.assertRaises(SatelliteMosaicPreparationError) as raised:
                    write_satellite_mosaic_preparation_v6(
                        output, definition_path=DEFINITION
            ...
            self.assertEqual(_fd_count(), before)
            message = str(raised.exception)
            self.assertIn("primary publication failure", message)
            self.assertIn("output already exists", message)
            self.assertIn("output parent close failed", message)
    ...
    def test_validator_identity_failure_post_close_eio_is_aggregated(self) -> None:
        before = _fd_count()
        real_open = v6.os.open
        real_close = v6.os.close
        ...
        def capture_open(path, flags, mode=0o777, *, dir_fd=None):
            nonlocal output_fd
            descriptor = real_open(path, flags, mode, dir_fd=dir_fd)
            if dir_fd is None and Path(path) == V2_RELEASE:
        ...
        def real_close_then_raise(descriptor: int) -> None:
            ...
            if descriptor == output_fd:
                raise OSError(errno.EIO, "injected post-output-close EIO")
        ...
        with patch.object(v6.os, "open", side_effect=capture_open), patch.object(
            ...
            side_effect=SatelliteMosaicPreparationError(
                "injected validator acquisition identity failure"
            ),
        ):
            with self.assertRaises(SatelliteMosaicPreparationError) as raised:
                validate_satellite_mosaic_preparation_v6(
                    V2_RELEASE, definition_path=DEFINITION
                )
        self.assertIsNotNone(output_fd)
        self.assertIn(output_fd, close_calls)
        self.assertEqual(_fd_count(), before)
        message = str(raised.exception)
        self.assertIn("primary validation failure", message)
        self.assertIn("injected validator acquisition identity failure", message)
        self.assertIn("output close failed", message)
        self.assertIn("injected post-output-close EIO", message)
        self.assertIn(f"exact requested target path: {V2_RELEASE}", message)

    def test_staging_identity_failure_close_attempts_every_acquired_descriptor(self) -> None:
        parent = self._parent()
        output = parent / "release"
        v6._payloads(DEFINITION, output)
        before = _fd_count()
        ...
        close_calls: list[int] = []
        real_identity_check = v6.v2._require_open_directory_identity

        def capture_open(path, flags, mode=0o777, *, dir_fd=None):
            nonlocal parent_fd, tree_fd
            descriptor = real_open(path, flags, mode, dir_fd=dir_fd)
            if dir_fd is None and Path(path) == parent:
                parent_fd = descriptor
            elif dir_fd is not None and str(path).startswith(".release.staging-"):
                tree_fd = descriptor
            return descriptor

        def fail_staging_identity(descriptor, expected, label):
            if label == "staging":
                raise SatelliteMosaicPreparationError(
                    "injected staging acquisition identity failure"
                )
            return real_identity_check(descriptor, expected, label)
        ...
        try:
            with patch.object(v6.os, "open", side_effect=capture_open), patch.object(
                ...
                v6.secrets, "token_hex", return_value="0123456789abcdef"
            ):
                with self.assertRaises(SatelliteMosaicPreparationError) as raised:
                    write_satellite_mosaic_preparation_v6(
                        output, definition_path=DEFINITION
            ...
            self.assertEqual(_fd_count(), before)
            message = str(raised.exception)
            self.assertIn("injected staging acquisition identity failure", message)
            self.assertIn(os.fspath(output), message)
            self.assertIn(
                os.fspath(parent / ".release.staging-0123456789abcdef"), message
            )
        finally:
            _force_remove(parent)

    def test_create_then_fileexists_retains_v5_fail_closed_behavior(self) -> None:
        parent = self._parent()
        output = parent / "release"
        real_mkdir = v6.os.mkdir
        ...
        try:
            with patch.object(v6.os, "mkdir", side_effect=create_then_fileexists):
                with self.assertRaises(SatelliteMosaicPreparationError) as raised:
                    write_satellite_mosaic_preparation_v6(
                        output, definition_path=DEFINITION
                    )
    ...
    def test_final_close_errors_retain_v5_aggregation_and_never_false_success(self) -> None:
        parent = self._parent()
        output = parent / "release"
        v6._payloads(DEFINITION, output)
        before = _fd_count()
