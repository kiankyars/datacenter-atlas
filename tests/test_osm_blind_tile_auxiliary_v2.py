...
from datacenter_atlas import osm_blind_tile_auxiliary_v2 as auxiliary


ROOT = Path(__file__).resolve().parents[1]
FIXTURE = ROOT / "tests" / "fixtures" / "osm_blind_tile_auxiliary_v2_minimal.osm"
...
class OsmBlindTileAuxiliaryV2UnitTests(unittest.TestCase):
    def test_preselection_rejects_area_no_empty_and_no_outer(self) -> None:
        ...
        self.assertEqual(
            inner_only["geometry_preselection_reason"],
    ...
    def test_stream_scan_preserves_grid_and_records_structural_rejections(self) -> None:
        ...
        self.assertEqual(industrial.getvalue(), "w100\nw102\nr200\n")
        self.assertEqual(statistics["preselection_industrial_rejected"], 5)
        self.assertEqual(statistics["preselection_explicit_area_no"], 2)
        self.assertEqual(statistics["preselection_relation_empty"], 1)
        self.assertEqual(
            statistics["preselection_relation_missing_outer_member"], 1
        )
        self.assertEqual(
            statistics["preselection_relation_missing_outer_way_member"], 1
        )

    def test_candidate_derived_paths_are_rejected(self) -> None:
        with self.assertRaisesRegex(
            auxiliary.OsmBlindTileAuxiliaryV2Error, "forbidden candidate-derived"
        ):
...
class OsmBlindTileAuxiliaryV2PipelineTests(unittest.TestCase):
    @classmethod
    ...
    def test_fixture_pipeline_is_geometry_complete_and_offline_reproducible(self) -> None:
        ...
        self.assertEqual(final["selected_objects"], 6)
        self.assertEqual(final["selected_industrial"], 2)
        self.assertEqual(final["selected_grid"], 4)
        self.assertEqual(final["unresolved_geometry_objects"], 6)
        self.assertEqual(final["preselection_geometry_rejections"], 5)
        self.assertEqual(final["polygon_export_unresolved"], 1)
        self.assertEqual(final["unresolved_with_bounds"], 5)
        self.assertEqual(final["unresolved_without_bounds"], 1)
        completeness = self.document["geometry_completeness"]
        self.assertEqual(completeness["initial_expected_industrial"], 3)
        self.assertEqual(completeness["initial_exported_industrial"], 2)
    ...
    def test_unresolved_ledger_encodes_bounded_and_null_tile_contracts(self) -> None:
        ...
        self.assertEqual(
            set(records), {"w101", "w102", "w105", "r201", "r202", "r203"}
        )
    ...
    def test_frozen_modes_and_closed_tree(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            copied.chmod(0o555)
            with self.assertRaisesRegex(
                auxiliary.OsmBlindTileAuxiliaryV2Error, "inventory is not closed"
            ):
    ...
    def test_recomputed_unresolved_tamper_fails_semantically(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(
                auxiliary.OsmBlindTileAuxiliaryV2Error,
    ...
    def test_recomputed_manifest_extra_key_fails_exact_schema(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            _resign(copied, manifest)
            with self.assertRaisesRegex(
                auxiliary.OsmBlindTileAuxiliaryV2Error, "top-level schema"
            ):
                self._validate(copied)
    ...
    def test_recomputed_processor_lineage_tamper_fails(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            _resign(copied, manifest)
            with self.assertRaisesRegex(
                auxiliary.OsmBlindTileAuxiliaryV2Error, "lineage changed"
            ):
                self._validate(copied)
    ...
    def test_sidecar_and_symlink_tamper_fail(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            sidecar.chmod(0o444)
            with self.assertRaisesRegex(
                auxiliary.OsmBlindTileAuxiliaryV2Error, "sidecar mismatch"
            ):
                self._validate(copied)
            ...
            linked.chmod(0o555)
            with self.assertRaisesRegex(
                auxiliary.OsmBlindTileAuxiliaryV2Error, "must not be a symlink"
            ):
    ...
    def test_mode_drift_fails(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            target.chmod(0o644)
            with self.assertRaisesRegex(
                auxiliary.OsmBlindTileAuxiliaryV2Error, "mode must be 0444"
            ):
                self._validate(copied)
    ...
    def test_forced_prepublish_validation_failure_publishes_nothing(self) -> None:
        ...
        with mock.patch.object(
            auxiliary,
            "validate_bundle",
            side_effect=auxiliary.OsmBlindTileAuxiliaryV2Error("forced validation failure"),
        ):
            with self.assertRaisesRegex(
                auxiliary.OsmBlindTileAuxiliaryV2Error, "forced validation failure"
            ):
