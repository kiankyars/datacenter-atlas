...
class OpenSeedV90PrepTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_accepted_v89_and_official_artifact_are_exact(self) -> None:
    ...
    def test_definition_selection_is_exact_seven_input_append(self) -> None:
        ...
        self.assertEqual(len(selected), 477)
        self.assertEqual(len(paths), 477)
        self.assertEqual(selected[:470], self.base["curated_inputs"])
        self.assertEqual(
            [row["path"] for row in selected[470:]], list(core.ADDITION_ORDER)
        )
        self.assertEqual(
            [row["sha256"] for row in selected[470:]],
            [core.ADDITION_PINS[path][1] for path in core.ADDITION_ORDER],
        )
        future = datetime.now(UTC).replace(microsecond=0) + timedelta(hours=1)
        with self.assertRaisesRegex(core.OpenSeedV90Error, "publication time"):
    ...
    def test_database_and_release_contract_recompute_exactly(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix="open-seed-v90-prep-", dir="/private/tmp"
        ) as temporary:
            root = Path(temporary)
            ...
            self.assertEqual(summary["entities_total"], 973)
            self.assertEqual(summary["campuses_total"], 506)
            self.assertEqual(summary["projects_total"], 467)
            self.assertEqual(summary["evidence_total"], 793)
            self.assertEqual(summary["lifecycle_observations_current"], 542)
            self.assertEqual(summary["capacity_estimates_current"], 557)
            self.assertEqual(summary["construction_pipeline_records"], 493)
            self.assertEqual(summary["construction_source_signals"], 392)
    ...
    def test_partial_collision_definition_collision_and_rollback_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory(
            prefix="open-seed-v90-partial-", dir="/private/tmp"
        ) as temporary:
            root = Path(temporary)
            ...
            with (
                ...
                self.assertRaisesRegex(core.OpenSeedV90Error, "partial"),
            ):
                core.build_open_seed_v90()

        with tempfile.TemporaryDirectory(
            prefix="open-seed-v90-collision-", dir="/private/tmp"
        ) as temporary:
            root = Path(temporary)
            ...
            def collide_on_definition(source: Path, destination: Path) -> None:
                if Path(destination) == definition:
                    raise FileExistsError("injected v90 definition collision")
            ...
            with (
                ...
                with self.assertRaisesRegex(FileExistsError, "injected"):
                    core.build_open_seed_v90(recorded_at=target_text)
        ...
        with tempfile.TemporaryDirectory(
            prefix="open-seed-v90-post-validation-", dir="/private/tmp"
        ) as temporary:
            root = Path(temporary)
            sources = root / "sources"
            releases = root / "releases"
            ...
            target = datetime.now(UTC).replace(microsecond=0) + timedelta(seconds=90)
            target_text = target.isoformat(timespec="seconds").replace("+00:00", "Z")
            injected = core.OpenSeedV90Error(
            ...
            with (
                ...
                patch.object(core, "PUBLICATION_LOCK", lock),
                patch.object(core, "_wait_until", return_value=None),
                patch.object(core, "validate_open_seed_v90", side_effect=injected),
            ):
                with self.assertRaisesRegex(core.OpenSeedV90Error, "post-promotion"):
                    core.build_open_seed_v90(recorded_at=target_text)
            self.assertFalse(definition.exists())
            self.assertFalse(release.exists())
            self.assertFalse(lock.exists())
