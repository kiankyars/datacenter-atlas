...
from datacenter_atlas import (
    satellite_unknown037_continuation_acceptance as acceptance,
)


MANIFEST_SHA256 = "0967b139a26f83ecfc7e4b79d7006e316f558295e93cb06e64ee099e7b773a39"
DELTA_SHA256 = "aaa68ed5100a3f0439cbe5e4d3d73e894c74a2f72df78816765be9c6a376cdfd"


@pytest.fixture(scope="module")
def built() -> tuple[dict[str, bytes], dict[str, object], dict[str, object]]:
    files = acceptance.build_unknown037_continuation_acceptance()
...
def test_complete_inventories_and_clone_delta_are_exact(
    ...
    assert len(source_rows) == acceptance.SOURCE_INVENTORY["files"] == 13_584
    assert len(destination_rows) == acceptance.DESTINATION_INVENTORY["files"] == 13_656
    assert sum(row[1] for row in source_rows) == 3_132_875_874
    assert sum(row[1] for row in destination_rows) == 3_146_051_663
    assert (
    ...
    assert delta["preexisting_files"]["non_manifest_byte_identical"] == 13_583
...
def test_bounded_outcomes_no_scene_and_next_pending_are_exact(
    ...
    assert no_scene[0][1]["queue_position"] == acceptance.NO_SCENE_POSITION
