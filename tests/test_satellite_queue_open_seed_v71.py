...
import datacenter_atlas.satellite_queue_v71 as subject
...
class OpenSeedV71SatelliteQueueTests(unittest.TestCase):
    def test_published_final_is_exact_and_frozen(self) -> None:
        manifest = subject.validate_satellite_queue_v71()
    ...
    def test_two_offline_builds_have_exact_hash_counts_and_audited_order(self) -> None:
        failure = AssertionError("v71 queue construction attempted network access")
        with patch.object(socket, "socket", side_effect=failure), patch.object(
            ...
            first = subject._bundle("2026-07-21T13:00:00Z")
            second = subject._bundle("2026-07-21T13:00:00Z")
        ...
        self.assertEqual(first.manifest["counts"]["skipped_missing_coordinates"], 621)
        ...
        self.assertEqual(len(active), 98)
        self.assertEqual(
    ...
    def test_future_private_publication_freezes_and_renames_after_target(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(manifest["counts"]["queue_jobs"], 189)
    ...
    def test_tamper_collision_and_symlink_fail_without_replacement(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            _freeze(queue)
            subject.validate_satellite_queue_v71(
            ...
            with self.assertRaisesRegex(
                subject.SatelliteQueueV71Error, "byte count|SHA-256"
            ):
                subject.validate_satellite_queue_v71(
                    queue, require_final_root_ctime=False
                )
            ...
            with self.assertRaisesRegex(subject.SatelliteQueueV71Error, "occupied"):
                subject._publish_to(
                    occupied,
                    root / ".occupied.lock",
                    generated_at=generated_at,
