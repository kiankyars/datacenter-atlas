...
identity = importlib.import_module(
    "datacenter_atlas.datacenter_atlas.exact_identity_decisions_v13"
)
shim = importlib.import_module("datacenter_atlas.exact_identity_decisions_v13")

ROOT = Path(__file__).resolve().parents[1]
WORKSPACE = ROOT.parent
BUILDER = ROOT / "scripts/build_exact_identity_decisions_v13.py"
...
class ExactIdentityDecisionV13Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.manifest = identity.validate_exact_identity_decision_bundle()

    def test_v13_is_exact_v12_structural_successor(self) -> None:
        ...
        child.update(
            {
                "expected_manifest_sha256": identity.V92_MANIFEST_PIN[1],
    ...
    def test_55_singletons_and_29_explicit_edges_have_release_provenance(self) -> None:
        identity._validate_decision_delta(identity.BUNDLE)
        features, targets, release_for = identity._child_feature_inventory()
        components = identity._csv_rows(identity.BUNDLE / identity.COMPONENTS_FILENAME)
        added = [
            row
            for row in components
            if row["release_id"] == identity.NEW_RELEASE_ID
            and row["stable_key"] in identity.NEW_ENTITY_KEYS
        ]
        self.assertEqual({row["stable_key"] for row in added}, identity.NEW_ENTITY_KEYS)
        self.assertEqual(len({row["component_id"] for row in added}), 55)
        ...
        self.assertTrue(all(row["ambiguous_identity_tokens_json"] == "[]" for row in added))
        self.assertTrue(all(row["snapshot_evidence_id"] for row in added))
        self.assertEqual(
            {row["entity_kind"] for row in added}, {"campus", "project"}
        )
        self.assertEqual(
            {
                name: sum(release_for[row["stable_key"]] == name for row in added)
                for name in identity.RELEASE_DELTAS
            },
            {name: delta[0] for name, delta in identity.RELEASE_DELTAS.items()},
        )
        ...
        expected_pairs = {
            (
                component_for[features[project]["id"]],
                component_for[features[project]["properties"]["target_entity_id"]],
            )
        ...
        relationship_pairs = {
            ...
            in {pair[0] for pair in expected_pairs}
        }
        self.assertEqual(relationship_pairs, expected_pairs)
        self.assertEqual(len(relationship_pairs), 29)
        self.assertEqual(
            {
                name: sum(release_for[key] == name for key in targets)
                for name in identity.RELEASE_DELTAS
            },
            {name: delta[1] for name, delta in identity.RELEASE_DELTAS.items()},
        )
        self.assertEqual(len(identity.carrier.CROSS_ROOT_SINGLETON_TOKENS), 5)
        review = next(
            row
            for row in self.manifest["input_children"]
            if row["release_id"] == "osm-fuzzy-review-v2"
        )
        self.assertTrue(review["review_only"])
        self.assertEqual(review["disposition"], "excluded_review_only")
        self.assertFalse(
            any(row["release_id"] == "osm-fuzzy-review-v2" for row in components)
        )
    ...
    def test_offline_double_replay_is_byte_exact(self) -> None:
        document, raw = identity._regular_document(
            identity.DEFINITION, "identity v13 definition"
        )
        ...
        failure = AssertionError("identity v13 replay attempted network access")
    ...
    def test_future_tamper_symlink_collision_no_replace_and_rollback_fail_closed(
        ...
        with patch.object(
            ...
            with self.assertRaisesRegex(
                ...
                identity.carrier._prepare_bundle(definition)

        with tempfile.TemporaryDirectory(
            prefix="identity-v13-fail-closed-", dir="/private/tmp"
        ) as temporary:
            root = Path(temporary)
    ...
    def test_shim_cli_imports_and_residue(self) -> None:
        ...
        code = (
            "from datacenter_atlas.exact_identity_decisions_v13 import BUNDLE_ID, "
            "EXPECTED_COUNTS; assert BUNDLE_ID.endswith('v13'); "
            "assert EXPECTED_COUNTS['exact_source_record_components']==8551"
        )
        ...
        leftovers = [
            ...
            if path.name.startswith(".identity-v13-private-stage-")
            or path.name == ".exact-identity-v13.lock"
        ]
