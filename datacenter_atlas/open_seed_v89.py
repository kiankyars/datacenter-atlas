"""Build open seed v89 as the strict nine-source append successor to v88."""
...
from . import microsoft_official_current_build_gap_20260721 as official
...
BASE_DEFINITION = ROOT / "sources/open-seed-2026-07-21-v88.json"
BASE_RELEASE = ROOT / "releases/2026-07-21-open-seed-v88"
DEFINITION = ROOT / "sources/open-seed-2026-07-21-v89.json"
RELEASE_ID = "2026-07-21-open-seed-v89"
RELEASE = ROOT / "releases" / RELEASE_ID
PUBLICATION_LOCK = ROOT / ".open-seed-v89.lock"
AS_OF = "2026-07-21"

BASE_RECORDED_AT = "2026-07-22T00:43:03Z"
BASE_DEFINITION_PIN = (
    104_159,
...
BASE_TREE_SHA256 = "aae34cbd2b8111a498ef529b08d0be6937c837b70025bdba6c097735b606073e"
BASE_ENTITIES_PIN = (
    992_569,
...
OFFICIAL_RECORDED_AT = "2026-07-22T00:51:52Z"
OFFICIAL_MANIFEST_PIN = (
    1_752,
    "908c93c82e61627fa938a925b02f4f2f847d54a8641dd6da34ba79d3d58c918d",
)
OFFICIAL_MANIFEST_TREE_SHA256 = (
    "bf28609e293e26f0da988bf2b08dad04e709f022359073aa2a979482ef96b137"
)
OFFICIAL_PHYSICAL_TREE_SHA256 = (
    "6eddae2487586ab2e26502fbd600638b66cbda46140939a41047472ff0643f69"
)
...
ADDITION_PINS = {
    ADDITION_ORDER[0]: (
        4_096,
        "6d62818c21aaa4f4c9db027666b5ff5a81c511c526a5f538da57dd7bb3fc1504",
    ),
    ADDITION_ORDER[1]: (
        4_394,
        "ba6f459381dfa8e165f8b0208f69e6df39a7558618e542cd0015abfa732bb07a",
    ),
    ADDITION_ORDER[2]: (
        3_954,
        "66b14c8fe208b3f0ddceb1531344866aeced677803024526892ada5f5426b872",
    ),
    ADDITION_ORDER[3]: (
        4_069,
        "1b95c26a9f3ebaa4fae409732cb946d185e96e0abb280b531f0263c4b2643098",
    ),
    ADDITION_ORDER[4]: (
        3_987,
        "b8c358c18c9b95224f48ed1634f03e7f1b19c58cb95abeed6f6f0548bd9d0997",
    ),
    ADDITION_ORDER[5]: (
        3_983,
        "bb8b255dfa7a69724f83dfab0bd8d4efd12ca422055a926decab0897a0e3a07c",
    ),
    ADDITION_ORDER[6]: (
        4_079,
        "8ebf76e3ae505f701e48b19b5ce88baef3ff2ac1e6718e6d206e7a6bc59b6f45",
    ),
    ADDITION_ORDER[7]: (
        4_150,
        "dd6aafab5e6732007fd9747df7561f061e8084a7c233494aa3e999fe4266da10",
    ),
    ADDITION_ORDER[8]: (
        4_090,
        "653fddb1d06239192bd7a1356b468f89ab73b9e5f83a0069b3c1167da9fdc22c",
    ),
}
...
ADDED_EVIDENCE_KEYS = frozenset(site.evidence_key for site in official.SITES)
ADDED_EXPORTED_EVIDENCE_KEYS = ADDED_EVIDENCE_KEYS
...
FRESHNESS_README = f"""
Open seed v89 is the exact accepted v88 successor with only the nine
seed-eligible records from `source_artifacts/{official.ARTIFACT_ID}` appended
at input indices 461 through 469. The artifact manifest is
`{OFFICIAL_MANIFEST_PIN[1]}`, logical tree is
`{OFFICIAL_MANIFEST_TREE_SHA256}`, and physical tree is
`{OFFICIAL_PHYSICAL_TREE_SHA256}`.

The append creates eighteen v88-new entities: nine separately named campuses
and nine projects. Microsoft Lévis and L’Ancienne-Lorette reuse the exact
global-open-v3 stable keys identified by the pinned construction-master audit,
but no OSM coordinate or geometry is inherited. Donnacona remains review-only
because its building-complete/internal-and-substation-active mixed state cannot
be preserved by schema 1.1.
...
class OpenSeedV89Error(RuntimeError):
    """Raised when a v89 lineage, claim, or publication guard fails closed."""
...
def _read_json(
    ...
    if path.is_symlink() or not path.is_file() or not stat.S_ISREG(path.stat().st_mode):
        raise OpenSeedV89Error(f"expected ordinary JSON file: {path}")
    if mode is not None and stat.S_IMODE(path.stat().st_mode) != mode:
        raise OpenSeedV89Error(f"file mode differs: {path}")
    raw = path.read_bytes()
    document = json.loads(raw)
    if raw != _canonical(document, sort_keys=sort_keys):
        raise OpenSeedV89Error(f"JSON is not canonical: {path}")
    return raw, document
...
def _validate_official_artifact() -> dict[str, dict[str, Any]]:
    try:
        ...
        raise OpenSeedV89Error(f"official current-build artifact invalid: {error}") from error
    ...
    if (
        ...
        or manifest.get("candidate_assessments") != 10
        or manifest.get("curated_source_records") != 9
        or manifest.get("seed_eligible_candidates") != 9
        ...
        raise OpenSeedV89Error("official current-build artifact pin or boundary differs")
    snapshot = json.loads(
    ...
    if (
        ...
        != {
            ...
            "source_records": 9,
            "seed_eligible_candidates": 9,
            "seed_eligible_source_records": 9,
            "review_only_candidates": 1,
            "distinct_campuses_in_source_records": 9,
            "projects": 9,
            "distinct_entities_in_source_records": 18,
        ...
        raise OpenSeedV89Error("official current-build source snapshot differs")
    ...
    for relative, record in zip(ADDITION_ORDER, records, strict=True):
        ...
        if (len(raw), _sha256(raw)) != ADDITION_PINS[relative] or (
            ...
            raise OpenSeedV89Error(f"v89 source pin differs: {relative}")
    ...
    if (
        ...
        raise OpenSeedV89Error("official current-build normalized claim contract differs")
    return documents
...
def _base_paths(base: Mapping[str, Any]) -> list[Path]:
    ...
    for row in base["curated_inputs"]:
        ...
        if path.is_symlink() or not path.is_file() or v69.sha256(path) != row["sha256"]:
            raise OpenSeedV89Error(f"accepted v88 input pin differs: {row['path']}")
        paths.append(path)
    return paths
...
def selected_inputs(
    ...
    *,
    ...
    if base.get("release_id") != v88.RELEASE_ID:
        raise OpenSeedV89Error("v89 base must be exactly accepted v88")
    ...
    if (
        ...
        raise OpenSeedV89Error("accepted v88 curated inventory differs")
    documents = _validate_official_artifact()
    target = v70.parse_utc(recorded_at, label="v89 recorded_at")
    ...
    if (
        ...
        raise OpenSeedV89Error("v89 publication time precedes an input")
    ...
    if base_evidence & ADDED_EVIDENCE_KEYS:
        raise OpenSeedV89Error("v89 evidence append collides with accepted v88")
    collision = official._collision_witness(documents)
    if (
        collision.get("exact_v88_stable_key_collisions") != []
        or collision.get("exact_v88_evidence_key_collisions") != []
        or collision.get("exact_master_identity_keys_reused")
        != [
            "osm:way/1383040174",
        ...
        raise OpenSeedV89Error("v89 Microsoft collision resolution differs")
    ...
    if (
        selected[:461] != rows
        or [row["path"] for row in selected[461:]] != list(ADDITION_ORDER)
        or tuple(documents) != ADDITION_ORDER
        or len(selected) != 470
        or len({row["path"] for row in selected}) != 470
    ):
        raise OpenSeedV89Error("v89 did not append exactly nine ordered inputs")
    for relative, document in documents.items():
        for index, evidence in enumerate(document["evidence"]):
            ...
            if retrieved > target or retrieved > wall.astimezone(UTC):
                raise OpenSeedV89Error("v89 selected evidence is future-dated")
...
def _validate_database_contract(
    ...
    *,
    ...
    expected_counts = {
        "entities": 959,
        "entity_snapshots": 981,
        "evidence": 784,
        "lifecycle_observations": 555,
    ...
    if actual_counts != expected_counts:
        raise OpenSeedV89Error(f"v89 database counts differ: {actual_counts}")
    with tempfile.TemporaryDirectory(
        prefix="open-seed-v89-base-", dir="/private/tmp"
    ) as temporary:
        prior = v69._populate_database(
            base,
            _base_paths(base),
            Path(temporary) / "v88.sqlite",
        ...
        try:
            for table in (
                ...
                if not _table_state(prior, table) <= _table_state(connection, table):
                    raise OpenSeedV89Error(f"v89 changed a v88 database row: {table}")
            before_entities = {
            ...
            if (
                ...
                raise OpenSeedV89Error("v89 database identity delta differs")
            ...
            if (
                ...
                raise OpenSeedV89Error("v89 database evidence delta differs")
        finally:
            prior.close()
    ...
    if project_targets != expected_targets:
        raise OpenSeedV89Error("v89 Microsoft project/campus boundaries differ")
    ...
    if (
        ...
        raise OpenSeedV89Error("v89 imported claim contract differs")
    ...
    if (
        ...
        or {row["kind"] for row in evidence_rows.values()} != {"company_disclosure"}
        ...
        raise OpenSeedV89Error("v89 imported evidence classification differs")
    ...
    if (
        ...
        raise OpenSeedV89Error("v89 coordinate coverage changed")
...
def _augment_release(documents: Mapping[str, str]) -> dict[str, str]:
    output = v88._augment_release(documents)
    output["README.md"] = (
    ...
    *,
    ...
    if precreated:
        if output.is_symlink() or not output.is_dir() or any(output.iterdir()):
            raise OpenSeedV89Error("precreated v89 release stage must be empty")
    else:
...
def _validate_release_facts(stage: Path, *, recorded_at: str) -> None:
    if stage.is_symlink() or not stage.is_dir():
        raise OpenSeedV89Error("v89 release is not an ordinary directory")
    ...
    if len(entries) != 14 or any(
        ...
        raise OpenSeedV89Error("v89 release file inventory differs")
    ...
    if (
        ...
        raise OpenSeedV89Error("v89 source-input inventory differs")
    ...
    if set(entities) != relevant_keys:
        raise OpenSeedV89Error("v89 relevant entity export differs")
    expected_country = {
        key: site.country
        for site in official.SITES
    ...
    for key in ADDED_ENTITY_KEYS:
        ...
        if (
            ...
            raise OpenSeedV89Error(f"v89 added entity scope differs: {key}")

    expected_status = {
        **{site.campus_key: ("", "") for site in official.SITES},
        **{
    ...
    if {
        ...
        raise OpenSeedV89Error("v89 exported lifecycle facts differ")
    ...
    if set(freshness) != ADDED_PROJECT_KEYS or any(
        ...
        raise OpenSeedV89Error("v89 freshness boundary differs")
    pipeline = {
        row["stable_key"]
    ...
    if pipeline != ADDED_PROJECT_KEYS:
        raise OpenSeedV89Error("v89 construction-pipeline delta differs")
    readme = (stage / "README.md").read_text(encoding="utf-8")
    for marker in (
        "Open seed v89 is the exact accepted v88 successor",
        "Donnacona remains review-only",
        "no OSM coordinate or geometry is inherited",
        "current_status_classification` is `unknown`",
        "current_construction_claim` is `false`",
    ):
        if marker not in readme:
            raise OpenSeedV89Error(f"v89 README guardrail differs: {marker}")
...
def _validate_definition(
    ...
    *,
    ...
    if set(document) != set(base) or document.get("release_id") != RELEASE_ID:
        raise OpenSeedV89Error("v89 definition identity or schema differs")
    build = document.get("build")
    if not isinstance(build, dict) or set(build) != {"as_of", "recorded_at"}:
        raise OpenSeedV89Error("v89 definition build carrier differs")
    if build["as_of"] != AS_OF:
        raise OpenSeedV89Error("v89 as_of differs")
    ...
    if not isinstance(expected_summary, dict) or set(expected_summary) != set(
        ...
        raise OpenSeedV89Error("v89 expected-summary key contract differs")
    recorded = v70.parse_utc(build["recorded_at"], label="v89 recorded_at")
    if (
        validation_wall_clock.tzinfo is None
        or recorded > validation_wall_clock.astimezone(UTC)
    ):
        raise OpenSeedV89Error("v89 recorded_at is later than validation wall clock")
    for key in (
        "epoch_capture",
        ...
        if document.get(key) != base.get(key):
            raise OpenSeedV89Error(f"v89 inherited definition field differs: {key}")
    ...
    *,
    ...
    for path in (definition, release, *release.iterdir()):
        ...
        if max(metadata.st_birthtime, metadata.st_mtime) > target.timestamp() + 1e-6:
            raise OpenSeedV89Error(f"v89 staged inode post-dates recorded_at: {path.name}")
    if require_live:
        if datetime.now(UTC) < target:
            raise OpenSeedV89Error("v89 recorded_at is not live")
        for path in (definition, release):
            if path.stat(follow_symlinks=False).st_ctime + 1e-6 < target.timestamp():
                raise OpenSeedV89Error(
                    f"v89 final root ctime predates recorded_at: {path.name}"
                )
...
def _validate_guard(guard: Mapping[str, Any]) -> None:
    ...
    if (
        v88.DEFINITION != BASE_DEFINITION
        or v88.RELEASE != BASE_RELEASE
        ...
        raise OpenSeedV89Error("accepted v88 base pin differs")
    if (
        ...
        raise OpenSeedV89Error("v89 official-source pin differs")


def validate_open_seed_v89(
    ...
    *,
    ...
    if replay_count != 2:
        raise OpenSeedV89Error("v89 requires exactly two offline replays")
    wall = validation_wall_clock or datetime.now(UTC)
    guard = _guard_state()
    ...
    if definition.get("curated_inputs") != selected:
        raise OpenSeedV89Error("v89 selected input inventory differs")
    if release_path.is_symlink() or not release_path.is_dir():
        raise OpenSeedV89Error("v89 release must be an ordinary directory")
    if require_frozen and stat.S_IMODE(release_path.stat().st_mode) != 0o555:
        raise OpenSeedV89Error("v89 release root is not frozen")
    ...
    if any(path.is_symlink() or not path.is_file() for path in release_files.values()):
        raise OpenSeedV89Error("v89 release contains a non-file")
    if require_frozen and any(
        stat.S_IMODE(path.stat().st_mode) != 0o444
        for path in release_files.values()
    ):
        raise OpenSeedV89Error("v89 release file is not frozen")
    ...
    if _sha256(manifest_raw) != definition["expected_release"].get(
        ...
        raise OpenSeedV89Error("v89 manifest hash differs")
    expected_release = {
        key: value
    ...
    if {key: value for key, value in manifest.items() if key != "files"} != expected_release:
        raise OpenSeedV89Error("v89 expected release facts differ")
    ...
    if {key: summary[key] for key in definition["expected_summary"]} != definition[
        ...
        raise OpenSeedV89Error("v89 expected summary differs")

    for replay in range(replay_count):
        with tempfile.TemporaryDirectory(
            prefix=f"open-seed-v89-replay-{replay + 1}-", dir="/private/tmp"
        ) as temporary:
            root = Path(temporary)
            ...
            if {path.name for path in replay_release.iterdir()} != set(release_files):
                raise OpenSeedV89Error("v89 replay file inventory differs")
            for filename, frozen in release_files.items():
                if (replay_release / filename).read_bytes() != frozen.read_bytes():
                    raise OpenSeedV89Error(f"v89 offline replay differs: {filename}")
    if _guard_state() != guard:
        raise OpenSeedV89Error("v89 validation mutated accepted inputs")
    return manifest
...
def _path_identity(path: Path, *, directory: bool) -> tuple[int, int]:
    ...
    if not expected:
        raise OpenSeedV89Error(f"v89 stage type differs: {path}")
...
def _assert_release_identities(
    ...
    if _path_identity(root, directory=True) != root_identity:
        raise OpenSeedV89Error("v89 release stage root identity changed")
    if _release_identities(root) != dict(members):
        raise OpenSeedV89Error("v89 release stage member identity changed")
...
def _discard_file_stage(path: Path, identity: tuple[int, int]) -> None:
    ...
    if _path_identity(path, directory=False) != identity:
        raise OpenSeedV89Error("refusing substituted v89 definition cleanup")
    path.chmod(0o600)
    path.unlink()
...
def _publication_lock() -> Iterator[None]:
    try:
        ...
        raise OpenSeedV89Error("active v89 publication lock exists") from error
    ...
    try:
        ...
        try:
            ...
            if not stat.S_ISREG(current.st_mode) or (
                ...
                raise OpenSeedV89Error("refusing substituted v89 lock cleanup")
...
def _require_absent(label: str) -> None:
    if DEFINITION.exists() or DEFINITION.is_symlink():
        raise OpenSeedV89Error(f"{label} v89 definition collision")
    if RELEASE.exists() or RELEASE.is_symlink():
        raise OpenSeedV89Error(f"{label} v89 release collision")
...
def _rollback_release(release_identity: tuple[int, int], release_stage: Path) -> None:
    if _path_identity(RELEASE, directory=True) != release_identity:
        raise OpenSeedV89Error("refusing rollback of substituted v89 release")
    if release_stage.exists() or release_stage.is_symlink():
        raise OpenSeedV89Error("v89 release rollback stage is occupied")
...
def _rollback_definition(
    ...
    if _path_identity(DEFINITION, directory=False) != definition_identity:
        raise OpenSeedV89Error("refusing rollback of substituted v89 definition")
    if definition_stage.exists() or definition_stage.is_symlink():
        raise OpenSeedV89Error("v89 definition rollback stage is occupied")
    v69.promote_noreplace(DEFINITION, definition_stage)


def build_open_seed_v89(recorded_at: str | None = None) -> dict[str, Any]:
    """Build and atomically publish the nine-source v88 successor."""

    if (DEFINITION.exists() or DEFINITION.is_symlink()) and (
        RELEASE.exists() or RELEASE.is_symlink()
    ):
        manifest = validate_open_seed_v89()
        return {
    ...
    if (
        ...
        raise OpenSeedV89Error("partial v89 final-path collision")

    guard = _guard_state()
    _validate_guard(guard)
    target = (
        v70.parse_utc(recorded_at, label="v89 recorded_at")
        if recorded_at is not None
        else datetime.now(UTC).replace(microsecond=0) + timedelta(seconds=45)
    )
    if datetime.now(UTC) >= target:
        raise OpenSeedV89Error("v89 recorded_at must be future before staging")
    ...
    with _publication_lock():
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(
                prefix="open-seed-v89-db-", dir="/private/tmp"
            ...
            if _path_identity(definition_stage, directory=False) != definition_identity:
                raise OpenSeedV89Error("v89 definition stage identity changed")
            ...
            if (
                ...
                raise OpenSeedV89Error("v89 private stage changed while waiting")
            _validate_publication_times(
                definition_stage,
                release_stage,
                recorded_at=recorded_at,
            ...
            try:
                ...
                try:
                    ...
                    error.add_note(f"v89 release rollback failed: {rollback_error}")
                raise
            try:
                manifest = validate_open_seed_v89(DEFINITION, RELEASE)
            except BaseException as error:
                rollback_errors = []
                try:
                    ...
                    rollback_errors.append(
                        f"v89 definition rollback failed: {rollback_error}"
                    )
                try:
                    _rollback_release(release_identity, release_stage)
                    published_release = False
                    ...
                    rollback_errors.append(
                        f"v89 release rollback failed: {rollback_error}"
                    )
    ...
    if _guard_state() != guard:
        raise OpenSeedV89Error("v89 build mutated accepted inputs")
    return {
        "definition": str(DEFINITION),
        "definition_sha256": v69.sha256(DEFINITION),
        "manifest_sha256": v69.sha256(RELEASE / "manifest.json"),
...
def main() -> int:
    print(json.dumps(build_open_seed_v89(), indent=2, sort_keys=True))
    return 0
