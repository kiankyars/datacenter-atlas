"""Accept one sealed Unknown038 catalog-only satellite continuation.

Unknown038 is an APFS copy-on-write continuation of accepted Unknown037.  Its
only payload changes are the batch manifest, 23 catalog-result triples, and
two explicit ``unavailable_no_scene`` checkpoints.  Acceptance is offline: this
...
SOURCE = ROOT / "satellite_review_runs/2026-07-22-global-open-v3-unknown-037"
DESTINATION = ROOT / "satellite_review_runs/2026-07-24-global-open-v3-unknown-038"
DESTINATION_LOCK = DESTINATION.with_name(f"{DESTINATION.name}.lock")
SOURCE_ACCEPTANCE = (
    ROOT / "satellite_review_continuations/"
    "2026-07-22-global-open-v3-unknown-037-continued-25"
)

ARTIFACT_ID = "2026-07-24-global-open-v3-unknown-038-continued-25"
OUTPUT = ROOT / "satellite_review_continuations" / ARTIFACT_ID
ACCEPTED_AT = "2026-07-24T21:20:00Z"
...
DESTINATION_BATCH_PIN = (
    6_553_448,
    "42033a50e3ef383613b95487030e21d6b90753c25f6776898b728c9fb6f61733",
)
SOURCE_ACCEPTANCE_MANIFEST_PIN = (
    10_276,
    "0967b139a26f83ecfc7e4b79d7006e316f558295e93cb06e64ee099e7b773a39",
)
SOURCE_ACCEPTANCE_SIDECAR_PIN = (
    93,
    "b7c01ea99272a4e3ee830c7ee0df9524fc53d5ac353212f0053b1b9e37cd8a8a",
)
QUEUE_MANIFEST_PIN = (
    28_269,
...
DESTINATION_INVENTORY = {
    ...
    "directories": 9_330,
    "files": 13_725,
    "sha256": "ac7fef9cb5e01e2f2c241c36be46f877c318607c60b1f69c0c40b63817aaec85",
}
...
DESTINATION_SUMMARY = {
    "jobs_completed": 4_493,
    "jobs_failed": 0,
    "jobs_pending": 1_936,
    "jobs_selected": 6_736,
    "jobs_unavailable_no_scene": 307,
}
EXPECTED_RUN = {
    "budget_exhausted": True,
    "finished_at": "2026-07-24T20:42:59Z",
    ...
    "max_jobs": 25,
    "started_at": "2026-07-24T20:40:44Z",
}
...
POSITION_START = 4_870
POSITION_END = 4_894
NEXT_PENDING_POSITION = 4_895
NEXT_PENDING_QUEUE_ID = "satq-e89e725265ef74b2cf41db13"
SOURCE_NEXT_PENDING_QUEUE_ID = "satq-feda771e469369c01497086b"
EXPECTED_NO_SCENE = {
    "satq-f525005f5ad5b10cb3155b32": {
        "queue_position": 4_889,
        "unavailability": {
            "catalog_error_type": (
            ...
            "query_window": {
                "bbox": "1.8253243,47.8750571,1.8789737,47.9110299",
                "end_date": "2024-07-30",
            ...
            "recorded_at": "2026-07-24T20:42:45Z",
            "window": "baseline",
        },
    },
    "satq-a95f5866731797ff836220c4": {
        "queue_position": 4_894,
        "unavailability": {
            "catalog_error_type": (
                "datacenter_atlas.satellite_catalog.CatalogValidationError"
            ),
            "outcome": "unavailable_no_scene",
            "query_window": {
                "bbox": "101.5022785,3.2050888,101.5383083,3.2410616",
                "end_date": "2024-07-30",
                "provider": "earth-search-v1",
                "start_date": "2024-05-01",
                "target_date": "2024-06-15",
            ...
            "raw_reason": "no scene falls within the baseline temporal window",
            "recorded_at": "2026-07-24T20:42:59Z",
            "window": "baseline",
        },
    },
}
EXPECTED_DELTA = {
    "directories_added": 48,
    "files_added": 69,
    "jobs_completed": 23,
    "jobs_failed": 0,
    "jobs_pending": -25,
    "jobs_selected": 0,
    "jobs_unavailable_no_scene": 2,
    "logical_bytes_added": 11_674_759,
    "preexisting_non_manifest_files_byte_identical": 13_655,
}
...
BUILDER_FILES = (
    "datacenter_atlas/satellite_unknown038_continuation_acceptance.py",
    "scripts/accept_satellite_unknown038_continuation.py",
)


class Unknown038ContinuationAcceptanceError(RuntimeError):
...
def _path_record(path: Path) -> dict[str, Any]:
    if path.is_symlink() or not path.is_file():
        raise Unknown038ContinuationAcceptanceError(
...
def _assert_pin(path: Path, pin: tuple[int, str], label: str) -> bytes:
    if path.is_symlink() or not path.is_file():
        raise Unknown038ContinuationAcceptanceError(f"{label} is not an ordinary file")
    raw = path.read_bytes()
    if (len(raw), _sha256(raw)) != pin:
        raise Unknown038ContinuationAcceptanceError(f"{label} pin changed")
    return raw
...
def _json_object(raw: bytes, label: str) -> dict[str, Any]:
    try:
        ...
        raise Unknown038ContinuationAcceptanceError(f"{label} is not JSON") from error
    if not isinstance(value, dict):
        raise Unknown038ContinuationAcceptanceError(f"{label} is not an object")
    return value


def _timestamp(value: str, label: str) -> datetime:
    if not isinstance(value, str) or not value.endswith("Z"):
        raise Unknown038ContinuationAcceptanceError(f"{label} is not canonical UTC")
    try:
    ...
    if parsed.microsecond:
        raise Unknown038ContinuationAcceptanceError(f"{label} must use whole seconds")
...
def _frozen(root: Path, label: str) -> None:
    ...
    if not files or not directories:
        raise Unknown038ContinuationAcceptanceError(f"{label} is empty")
    for path in files:
...
def _validate_source_acceptance() -> dict[str, Any]:
    _frozen(SOURCE_ACCEPTANCE, "Unknown037 acceptance wrapper")
    raw = _assert_pin(
        SOURCE_ACCEPTANCE / MANIFEST_FILENAME,
        SOURCE_ACCEPTANCE_MANIFEST_PIN,
        "Unknown037 acceptance manifest",
    )
    manifest = _json_object(raw, "Unknown037 acceptance manifest")
    ...
    if (
        ...
        raise Unknown038ContinuationAcceptanceError(
            "Unknown037 acceptance semantics changed"
        )
    sidecar = _assert_pin(
        SOURCE_ACCEPTANCE / SIDECAR_FILENAME,
        SOURCE_ACCEPTANCE_SIDECAR_PIN,
        "Unknown037 acceptance sidecar",
    )
    if sidecar != (
...
def _validate_lock() -> dict[str, Any]:
    if (
        ...
        raise Unknown038ContinuationAcceptanceError("Unknown038 canonical lock changed")
...
def _audit() -> dict[str, Any]:
    ...
    lock = _validate_lock()
    _frozen(SOURCE, "accepted Unknown037 source")
    _frozen(DESTINATION, "accepted Unknown038 destination")
    ...
    if destination_inventory != DESTINATION_INVENTORY:
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 complete canonical inventory changed"
        )
    ...
    if not set(source_files).issubset(destination_files):
        raise Unknown038ContinuationAcceptanceError("Unknown038 removed a source file")
    ...
    if changed_non_manifest:
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 changed inherited non-manifest bytes"
        )
    ...
    if [task["queue_position"] for _queue_id, task in selected] != list(
        ...
        raise Unknown038ContinuationAcceptanceError("Unknown038 task interval changed")
    state_counts = Counter(task["state"] for _queue_id, task in selected)
    if state_counts != Counter({"completed": 23, batch.UNAVAILABLE_NO_SCENE: 2}):
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 bounded task outcomes changed"
        )
    no_scene = [
        (queue_id, task)
    ...
    if {queue_id for queue_id, _task in no_scene} != set(EXPECTED_NO_SCENE):
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 no-scene queue IDs changed"
        )
    for queue_id, task in no_scene:
        expected = EXPECTED_NO_SCENE[queue_id]
        if (
            task["queue_position"] != expected["queue_position"]
            or task["attempts"] != 1
            or task["selected_ids"] is not None
            or task["artifacts"] is not None
            or task["unavailability"] != expected["unavailability"]
            or len(task["failures"]) != 1
            or "no scene falls within the baseline temporal window"
            not in task["failures"][0]["error"]
        ):
            raise Unknown038ContinuationAcceptanceError(
                "Unknown038 no-scene checkpoint changed"
            )
    completed = [
    ...
    if any(
        ...
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 completed task semantics changed"
        )
    for queue_id, source_task in source_document["jobs"].items():
        position = source_task["queue_position"]
        if (
            not POSITION_START <= position <= POSITION_END
            and destination_document["jobs"][queue_id] != source_task
        ):
            raise Unknown038ContinuationAcceptanceError(
                "Unknown038 changed an out-of-range task"
            )
    ...
    if (
        ...
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 exact catalog-only clone delta changed"
        )
    ...
    if next_pending != (NEXT_PENDING_POSITION, NEXT_PENDING_QUEUE_ID):
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 next-pending position changed"
        )
    ...
    if delta_summary != EXPECTED_DELTA:
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 delta arithmetic changed"
        )
    delta = {
    ...
    lock_birth = datetime.fromisoformat(lock["birthtime"])
    lock_birth_whole_second = lock_birth.replace(microsecond=0)
    destination_birth = datetime.fromisoformat(destination_chronology["birthtime"])
    destination_freeze = datetime.fromisoformat(destination_chronology["ctime"])
    if not (
        ...
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 acceptance chronology changed"
        )
    if source_chronology["device"] != destination_chronology["device"]:
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 source and destination are no longer on one device"
        )
...
def build_unknown038_continuation_acceptance() -> dict[str, bytes]:
...
def _closed_tree(directory: Path, expected: Mapping[str, bytes]) -> None:
    if directory.is_symlink() or not directory.is_dir():
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 acceptance wrapper is not a regular directory"
        )
    entries = {path.name: path for path in directory.iterdir()}
    if set(entries) != set(expected) or set(entries) != CLOSED_FILENAMES:
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 acceptance wrapper closed set changed"
        )
    if stat.S_IMODE(directory.stat().st_mode) != 0o555:
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 acceptance wrapper directory is not mode 0555"
        )
    for name, raw in expected.items():
        ...
        if (
            ...
            raise Unknown038ContinuationAcceptanceError(
                f"Unknown038 acceptance wrapper member changed: {name}"
            )


def _publication_paths(root: Path) -> list[Path]:
    files, directories = recovery._regular_tree(root, "Unknown038 wrapper")
...
def _assert_predeclared_stage(root: Path) -> None:
    ...
    for path in _publication_paths(root):
        ...
        if birthtime is None or birthtime > declared or metadata.st_mtime > declared:
            raise Unknown038ContinuationAcceptanceError(
                f"Unknown038 stage chronology is too late: {path}"
            )
...
def _validate_publication_chronology(root: Path) -> None:
    ...
    for path in _publication_paths(root):
        ...
        if (
            ...
            raise Unknown038ContinuationAcceptanceError(
                f"Unknown038 final publication chronology changed: {path}"
            )


def validate_unknown038_continuation_acceptance(
    ...
    if supplied != OUTPUT:
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 acceptance wrapper path changed"
        )
    expected = build_unknown038_continuation_acceptance()
    _closed_tree(supplied, expected)
    _validate_publication_chronology(supplied)
    manifest_raw = expected[MANIFEST_FILENAME]
    manifest = _json_object(manifest_raw, "Unknown038 acceptance manifest")
    if manifest_raw != _canonical_json(manifest):
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 acceptance manifest is not canonical"
        )
    if expected[SIDECAR_FILENAME] != (
        f"{_sha256(manifest_raw)}  {MANIFEST_FILENAME}\n"
    ).encode("ascii"):
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 acceptance sidecar changed"
        )
    if (
        ...
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 acceptance manifest semantics changed"
        )
    return manifest
...
def write_unknown038_continuation_acceptance() -> dict[str, Any]:
    """Write the frozen wrapper with future-clock and no-replace guards."""

    if OUTPUT.exists() or OUTPUT.is_symlink():
        return validate_unknown038_continuation_acceptance(OUTPUT)
    parent = OUTPUT.parent
    if parent.is_symlink() or not parent.is_dir():
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 acceptance output parent is unsafe"
        )
    ...
    if datetime.now(UTC) >= _timestamp(ACCEPTED_AT, "accepted_at"):
        raise Unknown038ContinuationAcceptanceError(
            "Unknown038 stage would postdate the declared acceptance time"
        )
    ...
    try:
        ...
        if OUTPUT.exists() or OUTPUT.is_symlink():
            raise Unknown038ContinuationAcceptanceError(
                "Unknown038 acceptance output appeared before promotion"
            )
        ...
        if (current_parent.st_dev, current_parent.st_ino) != parent_identity:
            raise Unknown038ContinuationAcceptanceError(
                "Unknown038 acceptance parent identity changed"
            )
        if (current_stage.st_dev, current_stage.st_ino) != stage_identity:
            raise Unknown038ContinuationAcceptanceError(
                "Unknown038 acceptance stage identity changed"
            )
        recovery._promote_directory_exclusive(stage, OUTPUT)
        if stage.exists() or stage.is_symlink():
            raise Unknown038ContinuationAcceptanceError(
                "Unknown038 acceptance stage remains after promotion"
            )
...
def main() -> int:
    manifest = write_unknown038_continuation_acceptance()
