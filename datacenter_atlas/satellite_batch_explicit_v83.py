"""Explicit, fail-closed catalog adapter for four new v83 project jobs.

All 104 active-construction jobs remain represented in the checkpoint.  Only
the immutable four-ID receipt is executable; the other 100 jobs stay pending.
...
No imagery is downloaded and no machine or catalog fact changes atlas claims.
"""
...
from .satellite_queue_v83 import (
...
EXPECTED_SELECTED_POSITIONS = (14, 25, 50, 102)


class ExplicitSatelliteBatchV83Error(ValueError):
...
SCHEMA_VERSION = 1
PIPELINE = "satellite_review_catalog_batch_explicit_v83_v1"
RECEIPT_ID = "open-seed-v83-active-explicit-new-projects-4-v1"
...
SCOPE = {
    **carrier.BATCH_SCOPE,
    "selection_mode": "explicit_v83_new_project_queue_ids",
...
def _timestamp(value: str, field: str) -> str:
    try:
        ...
        raise ExplicitSatelliteBatchV83Error(str(error)) from error


def _positive_integer(value: Any, field: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
        raise ExplicitSatelliteBatchV83Error(f"{field} must be a positive integer")
    return value
...
def _carrier_checkpoint() -> None:
    ...
    if path.is_symlink() or not path.is_file() or _sha256(path.read_bytes()) != CARRIER_SHA256:
        raise ExplicitSatelliteBatchV83Error("pinned satellite batch carrier changed")


@dataclass(frozen=True, slots=True)
class ExplicitV83BatchConfig:
    """Runtime settings around the fixed v83 zero-retry, one-attempt policy."""
...
def _queue_state(queue_directory: str | Path) -> tuple[
    ...
    if queue_input.is_symlink() or not queue_input.is_dir():
        raise ExplicitSatelliteBatchV83Error("v83 queue must be a regular directory")
    queue = queue_input.resolve()
    try:
        manifest = validate_satellite_queue_v83(
            queue, require_final_root_ctime=queue == V83_QUEUE.resolve()
        )
    except ValueError as error:
        raise ExplicitSatelliteBatchV83Error(str(error)) from error
    jobs = _read_jobs(queue)
    if any(job.get("review_constraints") != REVIEW_CONSTRAINTS for job in jobs):
        raise ExplicitSatelliteBatchV83Error("v83 queue lost its review-only constraints")
    active = [job for job in jobs if job["priority"]["tier"] == "active_construction"]
    if len(active) != ACTIVE_ENTITY_COUNT:
        raise ExplicitSatelliteBatchV83Error("v83 active job inventory changed")
    if (
        ...
        raise ExplicitSatelliteBatchV83Error("published v83 queue lineage changed")
...
def _receipt(
    ...
    if (
        ...
        or set(SELECTED_QUEUE_IDS) != added_project_ids
        or any(queue_id not in by_id for queue_id in SELECTED_QUEUE_IDS)
        or tuple(by_id[queue_id]["queue_position"] for queue_id in SELECTED_QUEUE_IDS)
        != EXPECTED_SELECTED_POSITIONS
        or list(SELECTED_QUEUE_IDS)
        != sorted(SELECTED_QUEUE_IDS, key=lambda queue_id: by_id[queue_id]["queue_position"])
    ):
        raise ExplicitSatelliteBatchV83Error("explicit v83 project selection changed")
    for queue_id in SELECTED_QUEUE_IDS:
        job = by_id[queue_id]
        expected = ADDED_QUEUE_EXPECTATIONS[queue_id]
        if (
            job["entity"]["id"] != expected["entity_id"]
            or job["entity"]["kind"] != "project"
            or job["priority"]["tier"] != "active_construction"
            or job["priority"]["lifecycle_status"] != "under_construction"
            or job["location"]["center_wgs84"]["latitude"]
            != expected["center_wgs84"]["latitude"]
            or job["location"]["center_wgs84"]["longitude"]
            != expected["center_wgs84"]["longitude"]
        ):
            raise ExplicitSatelliteBatchV83Error(
                f"selected v83 project identity changed: {queue_id}"
            )
    selected_jobs = [
        {
        ...
        for position, queue_id in enumerate(SELECTED_QUEUE_IDS, start=1)
    ]
    return {
        ...
        "selection": {
            "basis": "explicit_v83_new_active_projects_v1",
...
def _write_exclusive_file(path: Path, raw: bytes, mode: int) -> None:
    ...
    try:
        ...
        raise ExplicitSatelliteBatchV83Error(f"control-file collision: {path}") from error
    try:
...
def _validate_receipt(
    ...
    for name, raw in payloads.items():
        ...
        if path.is_symlink() or not path.is_file() or path.read_bytes() != raw:
            raise ExplicitSatelliteBatchV83Error(f"immutable selection receipt changed: {name}")
        if stat.S_IMODE(path.stat().st_mode) != 0o444:
            raise ExplicitSatelliteBatchV83Error(f"selection receipt is not frozen: {name}")
    return {
        "file": SELECTION_RECEIPT_FILENAME,
        "bytes": len(payloads[SELECTION_RECEIPT_FILENAME]),
        "sha256": _sha256(payloads[SELECTION_RECEIPT_FILENAME]),
...
def _write_manifest(output: Path, document: dict[str, Any], updated_at: str) -> None:
    ...
    if path.is_symlink() or (path.exists() and not path.is_file()):
        raise ExplicitSatelliteBatchV83Error("explicit batch manifest path is invalid")
    temporary = output / f".{BATCH_MANIFEST_FILENAME}.explicit-v83-v1.tmp"
    if temporary.exists() or temporary.is_symlink():
        raise ExplicitSatelliteBatchV83Error("stale explicit batch checkpoint exists")
...
def _config_from_document(value: Any) -> ExplicitV83BatchConfig:
    if not isinstance(value, Mapping) or set(value) != set(
        ExplicitV83BatchConfig().as_dict()
    ):
        raise ExplicitSatelliteBatchV83Error("explicit batch configuration schema changed")
    if (
        ...
        raise ExplicitSatelliteBatchV83Error("fixed explicit execution policy changed")
    try:
        config = ExplicitV83BatchConfig(
    ...
    except ExplicitSatelliteBatchV83Error:
        raise
    if config.as_dict() != dict(value):
        raise ExplicitSatelliteBatchV83Error("explicit batch configuration is not canonical")
    return config
...
def _validate_failure(
    ...
    if not isinstance(failure, Mapping) or set(failure) != {"attempt", "at", "error"}:
        raise ExplicitSatelliteBatchV83Error(f"{queue_id} failure schema changed")
    attempt = _positive_integer(failure.get("attempt"), f"{queue_id} failure attempt")
    if attempt > attempts:
        raise ExplicitSatelliteBatchV83Error(f"{queue_id} failure exceeds attempts")
    _timestamp(failure.get("at"), f"{queue_id} failure timestamp")
    if not isinstance(failure.get("error"), str) or not failure["error"]:
        raise ExplicitSatelliteBatchV83Error(f"{queue_id} failure error is invalid")
    return attempt
...
def _validate_document(
    ...
    *,
    ...
    if not isinstance(document, dict) or set(document) != expected_keys:
        raise ExplicitSatelliteBatchV83Error("explicit batch manifest schema changed")
    if (
        ...
        raise ExplicitSatelliteBatchV83Error("explicit batch identity or lineage changed")
    ...
    if not isinstance(tasks, dict) or set(tasks) != set(expected_jobs):
        raise ExplicitSatelliteBatchV83Error("explicit batch represented job inventory changed")
    ...
    for queue_id, task in tasks.items():
        if not isinstance(task, dict) or set(task) != task_keys:
            raise ExplicitSatelliteBatchV83Error(f"task schema changed: {queue_id}")
        ...
        if any(task.get(field) != expected[field] for field in immutable):
            raise ExplicitSatelliteBatchV83Error(f"task identity changed: {queue_id}")
        ...
        if state not in {
            ...
            raise ExplicitSatelliteBatchV83Error(f"task state is invalid: {queue_id}")
        attempts = task.get("attempts")
        if isinstance(attempts, bool) or not isinstance(attempts, int) or not 0 <= attempts <= 1:
            raise ExplicitSatelliteBatchV83Error(f"task attempts are invalid: {queue_id}")
        failures = task.get("failures")
        if not isinstance(failures, list) or len(failures) > attempts:
            raise ExplicitSatelliteBatchV83Error(f"task failures are invalid: {queue_id}")
        failure_attempts = [
            _validate_failure(failure, queue_id=queue_id, attempts=attempts)
            for failure in failures
        ]
        if failure_attempts != sorted(set(failure_attempts)):
            raise ExplicitSatelliteBatchV83Error(f"task failures are unordered: {queue_id}")
        ...
        if state == "completed":
            if attempts != 1:
                raise ExplicitSatelliteBatchV83Error(f"completed task attempts differ: {queue_id}")
            ...
            if not isinstance(task.get("selected_ids"), dict) or not isinstance(
                ...
                raise ExplicitSatelliteBatchV83Error(f"completed task provenance absent: {queue_id}")
            if task.get("unavailability") is not None:
                raise ExplicitSatelliteBatchV83Error(f"completed task unavailable: {queue_id}")
        elif any(
            ...
            raise ExplicitSatelliteBatchV83Error(f"non-completed task has provenance: {queue_id}")
        if state == "failed" and failure_attempts != [1]:
            raise ExplicitSatelliteBatchV83Error(f"failed task lacks final failure: {queue_id}")
        if state == carrier.UNAVAILABLE_NO_SCENE:
            ...
            raise ExplicitSatelliteBatchV83Error(f"unexpected unavailability: {queue_id}")
    expected_summary = _summary(document)
    if document.get("summary") != expected_summary:
        raise ExplicitSatelliteBatchV83Error("explicit batch summary changed")
    selected_terminal = (
    ...
    if document.get("state") != expected_state:
        raise ExplicitSatelliteBatchV83Error("explicit selection state changed")
    ...
    if reserved > LIFETIME_HTTP_ATTEMPT_CAP or document.get("lifetime_budget") != expected_lifetime:
        raise ExplicitSatelliteBatchV83Error("explicit lifetime HTTP budget changed")
...
def _validate_last_run(value: Any, *, reserved: int) -> None:
    ...
    if not isinstance(value, Mapping) or set(value) != keys:
        raise ExplicitSatelliteBatchV83Error("explicit last_run schema changed")
    ...
    for field in (
        ...
        if isinstance(number, bool) or not isinstance(number, int) or number < 0:
            raise ExplicitSatelliteBatchV83Error(f"explicit last_run {field} is invalid")
    if (
        value["max_jobs"] <= 0
        ...
        raise ExplicitSatelliteBatchV83Error("explicit last_run budget changed")
    if value["finished_at"] is not None and (
        ...
        raise ExplicitSatelliteBatchV83Error("explicit last_run outcomes do not reconcile")
...
def _validate_directory_chain(output: Path, directory: Path) -> None:
    ...
    try:
        ...
        raise ExplicitSatelliteBatchV83Error("catalog parent escapes explicit output") from error
    current = output
    if output.is_symlink() or not output.is_dir():
        raise ExplicitSatelliteBatchV83Error("explicit output root changed type")
    for part in relative.parts:
        ...
        if resolved != output_resolved and output_resolved not in resolved.parents:
            raise ExplicitSatelliteBatchV83Error("catalog parent escapes explicit output")


def _catalog_result(
    job: Mapping[str, Any], directory: Path, config: ExplicitV83BatchConfig
) -> dict[str, Any]:
    try:
...
def _terminal_artifacts(
    ...
    *,
    ...
    for queue_id, task in document["jobs"].items():
        ...
        transaction_pattern = f".{final.name}.explicit-v83-v1-stage-*"
    ...
    for path in output.rglob("*"):
        if path not in allowed:
            raise ExplicitSatelliteBatchV83Error(f"unexpected explicit batch artifact: {path}")


def _read_manifest(path: Path) -> dict[str, Any]:
    if path.is_symlink() or not path.is_file():
        raise ExplicitSatelliteBatchV83Error("explicit batch manifest is not a regular file")
    raw = path.read_bytes()
    try:
        document = json.loads(raw)
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise ExplicitSatelliteBatchV83Error("explicit batch manifest is invalid JSON") from error
    if not isinstance(document, dict) or raw != _manifest_raw(document):
        raise ExplicitSatelliteBatchV83Error("explicit batch manifest is not canonical JSON")
    return document


def validate_explicit_satellite_batch_v83(
    queue_directory: str | Path,
    output_directory: str | Path,
    *,
    config: ExplicitV83BatchConfig | None = None,
) -> dict[str, Any]:
    """Validate the receipt, all 104 tasks, budget, and terminal outputs offline."""
    ...
    if output_input.is_symlink() or not output_input.is_dir():
        raise ExplicitSatelliteBatchV83Error("explicit batch output is not a regular directory")
    ...
    if config is not None and config != saved_config:
        raise ExplicitSatelliteBatchV83Error("saved explicit configuration differs")
...
def _ensure_output(output_directory: str | Path, queue: Path) -> Path:
    ...
    if output == queue or queue in output.parents:
        raise ExplicitSatelliteBatchV83Error("explicit output must be separate from queue")
    if output.is_symlink():
        raise ExplicitSatelliteBatchV83Error("explicit output may not be a symlink")
    output.mkdir(parents=True, exist_ok=True)
    if output.is_symlink() or not output.is_dir():
        raise ExplicitSatelliteBatchV83Error("explicit output is not a regular directory")
...
def _transaction(output: Path, final: Path) -> tuple[Path, tuple[int, int]]:
    ...
    if final.exists() or final.is_symlink():
        raise ExplicitSatelliteBatchV83Error(f"catalog final path collision: {final}")
    transaction = Path(
        tempfile.mkdtemp(
            prefix=f".{final.name}.explicit-v83-v1-stage-", dir=final.parent
        )
    )
...
def _discard_transaction(
    ...
    if (
        ...
        raise ExplicitSatelliteBatchV83Error("refusing substituted job transaction cleanup")
    paths = sorted(transaction.rglob("*"), key=lambda path: len(path.parts), reverse=True)
    if any(path.is_symlink() for path in paths):
        raise ExplicitSatelliteBatchV83Error("refusing symlink-contaminated job cleanup")
    for path in paths:
        ...
        if stat.S_ISREG(metadata.st_mode):
            ...
            raise ExplicitSatelliteBatchV83Error("refusing non-regular job cleanup")
...
def _prune_empty_parents(path: Path, output: Path) -> None:
    ...
    while current != output:
        if current.is_symlink() or not current.is_dir():
            raise ExplicitSatelliteBatchV83Error("catalog parent changed during cleanup")
        try:
            current.rmdir()
        except OSError:
            return
...
def execute_explicit_satellite_batch_v83(
    queue_directory: str | Path,
    output_directory: str | Path,
    *,
    config: ExplicitV83BatchConfig = ExplicitV83BatchConfig(),
    ...
    """Attempt only the immutable four-ID v83 project selection under a lifetime cap."""

    if not isinstance(config, ExplicitV83BatchConfig):
        raise ExplicitSatelliteBatchV83Error("config must be ExplicitV83BatchConfig")
    ...
    if max_jobs > SELECTED_JOB_COUNT:
        raise ExplicitSatelliteBatchV83Error("max_jobs exceeds the four-job selection")
    if max_http_attempts > LIFETIME_HTTP_ATTEMPT_CAP:
        raise ExplicitSatelliteBatchV83Error("max_http_attempts exceeds lifetime cap 8")
    ...
    return validate_explicit_satellite_batch_v83(
