"""Exact-identity carrier for federation-v4 geometry guardrails.

The wire format and deterministic identity policy remain unchanged. This
isolated carrier accepts only the explicit false geometry-inference field
introduced by open-seed v86, while preserving freshness and source-scoped
identity validation from the frozen v3 carrier.
"""
...
def _federation_input(
    ...
    try:
        index = federation_v4.validate_federated_release_index(
            directory,
            child_release_paths=child_paths,
        )
    except (federation_v4.FederatedReleaseError, OSError) as error:
        raise ExactIdentityDecisionError(
            f"federated v4 index validation failed: {error}"
        ) from error
    manifest_checkpoint = legacy._checkpoint(directory / MANIFEST_FILENAME)
    index_checkpoint = legacy._checkpoint(
        directory / federation_v4.INDEX_FILENAME
    )
    if (
    ...
    if (
        index.get("schema_version") != federation_v4.INDEX_SCHEMA_VERSION
        or index.get("format") != federation_v4.INDEX_FORMAT
    ):
        raise ExactIdentityDecisionError("federated v4 index format is unsupported")
    generated_at = legacy._timestamp(
        index.get("generated_at"), "federated v4 generated_at"
    )
    if datetime.fromisoformat(generated_at.replace("Z", "+00:00")) >= (
        datetime.fromisoformat(definition.recorded_at.replace("Z", "+00:00"))
    ):
        raise ExactIdentityDecisionError(
            "decision recorded_at must follow the federated v4 index"
        )
    return dict(index), {
...
def _inspect_children(
    ...
    if not isinstance(descriptors, list):
        raise ExactIdentityDecisionError(
            "federated v4 release descriptors are invalid"
        )
    by_id = {
...
def validate_exact_identity_decision_bundle(
    ...
    *,
    ...
    """Validate a closed bundle and optionally replay all v4-bound inputs."""
...
def write_exact_identity_decision_bundle(
    ...
    """Double-rebuild, freeze, and atomically publish one v4-bound bundle."""
