"""Exact-identity carrier for federation-v5 append-only child manifests.

The wire format and deterministic identity policy remain unchanged. This
isolated carrier consumes the append-only lineage extension introduced by
open-seed v92 through the frozen federation-v5 projection, while preserving
freshness, geometry non-inference, and source-scoped identity validation.
"""
...
from . import federated_release_v4 as federation_v4
from . import federated_release_v5 as federation_v5
...
CROSS_ROOT_SINGLETON_TOKENS = frozenset(
    {
        "openstreetmap:way/1288894119",
        "openstreetmap:way/1319701990",
        "openstreetmap:way/1383040174",
        "openstreetmap:way/1383040176",
        "wikidata:Q136745002",
    }
)
...
def _preserve_cross_root_singletons(
    ...
    """Suppress only the v92-side token in five explicit cross-root collisions."""
    ...
    if set(collisions) != CROSS_ROOT_SINGLETON_TOKENS:
        raise ExactIdentityDecisionError(
            "federation-v5 cross-root singleton collision inventory differs"
        )

    for token, members in collisions.items():
        if (
            {item.release_id for item in members}
            != {"epoch-official-open-seed-v92", "global-open-v3"}
            or not any(
                item.release_id == "epoch-official-open-seed-v92"
                for item in members
            )
            or not any(item.release_id == "global-open-v3" for item in members)
        ):
            raise ExactIdentityDecisionError(
                f"federation-v5 cross-root singleton witness differs: {token}"
            )
        expected_basis = (
            "wikidata_stable_key"
        ...
        successors = [
            item
            for item in members
            if item.release_id == "epoch-official-open-seed-v92"
        ]
        if any(
            len(
                [
            ...
            raise ExactIdentityDecisionError(
                f"federation-v5 suppression basis differs: {token}"
            )
    suppressed_occurrences = {
        occurrence.occurrence_id
        for occurrence in occurrences
        if occurrence.release_id == "epoch-official-open-seed-v92"
        and any(
            identity.token in CROSS_ROOT_SINGLETON_TOKENS
            for identity in occurrence.identities
        )
    }
    if len(suppressed_occurrences) != 9:
        raise ExactIdentityDecisionError(
            "federation-v5 cross-root successor occurrence inventory differs"
        )

    normalized = []
    for occurrence in occurrences:
        if occurrence.occurrence_id not in suppressed_occurrences:
...
def _federation_input(
    ...
    try:
        index = federation_v5.validate_federated_release_index(
            directory,
            child_release_paths=child_paths,
        )
    except (federation_v5.FederatedReleaseError, OSError) as error:
        raise ExactIdentityDecisionError(
            f"federated v5 index validation failed: {error}"
        ) from error
    manifest_checkpoint = legacy._checkpoint(directory / MANIFEST_FILENAME)
    index_checkpoint = legacy._checkpoint(
        directory / federation_v5.INDEX_FILENAME
    )
    if (
        ...
        raise ExactIdentityDecisionError("federated v5 index hash drifted")
    if (
        index.get("schema_version") != federation_v5.INDEX_SCHEMA_VERSION
        or index.get("format") != federation_v5.INDEX_FORMAT
    ):
        raise ExactIdentityDecisionError("federated v5 index format is unsupported")
    generated_at = legacy._timestamp(
        index.get("generated_at"), "federated v5 generated_at"
    )
    if datetime.fromisoformat(generated_at.replace("Z", "+00:00")) >= (
        datetime.fromisoformat(definition.recorded_at.replace("Z", "+00:00"))
    ):
        raise ExactIdentityDecisionError(
            "decision recorded_at must follow the federated v5 index"
        )
    return dict(index), {
...
def _inspect_children(
    ...
    if not isinstance(descriptors, list):
        raise ExactIdentityDecisionError(
            "federated v5 release descriptors are invalid"
        )
    by_id = {
    ...
    for value in definition.children:
        ...
        try:
            geometry = federation_v4._geometry_contract(
                manifest,
                label=f"{release_id} child manifest",
            )
            descriptor_geometry = federation_v4._geometry_contract(
                descriptor_manifest,
                label=f"{release_id} federation descriptor",
            )
        except federation_v4.FederatedReleaseError as error:
            raise ExactIdentityDecisionError(
                f"child geometry descriptor is invalid: {release_id}: {error}"
            ) from error
        if descriptor_geometry != geometry:
            raise ExactIdentityDecisionError(
                f"child geometry descriptor does not reconcile: {release_id}"
            )
...
def validate_exact_identity_decision_bundle(
    ...
    *,
    ...
    """Validate a closed bundle and optionally replay all v5-bound inputs."""
...
def write_exact_identity_decision_bundle(
    ...
    """Double-rebuild, freeze, and atomically publish one v5-bound bundle."""
