"""Geometry-complete, candidate-independent industrial/grid OSM auxiliary v2.
...
OUTPUT_DIRECTORY = (
    PROJECT_ROOT / "source_cache" / "osm-blind-tile-auxiliary-260713-v2"
)
...
PIPELINE = "osm_blind_tile_stable_industrial_grid_extraction_v2"
SCHEMA_VERSION = 2
GENERATOR = "DataCenterAtlas/0.1 blind-tile-stable-industrial-grid-v2"
...
PROCESSOR_FILES = (
    ...
    "datacenter_atlas/osm_blind_tile_auxiliary_v2.py",
    "scripts/extract_osm_blind_tile_auxiliary_v2.py",
)
...
PRESELECTION_REASONS = frozenset(
    {
        "explicit_area_no",
        "relation_empty",
        "relation_missing_outer_member",
        "relation_missing_outer_way_member",
    }
)
...
SCAN_STATISTIC_KEYS = frozenset(
    {
        ...
        "preselection_relation_missing_outer_member",
        "preselection_relation_missing_outer_way_member",
    }
)
...
class OsmBlindTileAuxiliaryV2Error(ValueError):
    """Raised when v2 selection, geometry, lineage, or publication fails closed."""
...
def _decode_json(raw: bytes, label: str) -> Any:
    try:
        ...
        raise OsmBlindTileAuxiliaryV2Error(f"{label} must be UTF-8") from error

    def reject_constant(value: str) -> None:
        raise OsmBlindTileAuxiliaryV2Error(
    ...
    try:
        ...
        raise OsmBlindTileAuxiliaryV2Error(f"{label} is invalid JSON") from error


def _timestamp(value: Any, label: str) -> datetime:
    if not isinstance(value, str) or not value:
        raise OsmBlindTileAuxiliaryV2Error(f"{label} must be an RFC 3339 timestamp")
    try:
    ...
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise OsmBlindTileAuxiliaryV2Error(f"{label} must include a timezone")
...
def _normalize(path: str | Path, label: str) -> Path:
    try:
        ...
        raise OsmBlindTileAuxiliaryV2Error(f"{label} path cannot be normalized") from error


def _regular_file(path: Path, label: str) -> os.stat_result:
    if path.is_symlink():
        raise OsmBlindTileAuxiliaryV2Error(f"{label} must not be a symlink")
    try:
        status = path.stat()
    except FileNotFoundError as error:
        raise OsmBlindTileAuxiliaryV2Error(f"{label} is missing") from error
    if not stat.S_ISREG(status.st_mode):
        raise OsmBlindTileAuxiliaryV2Error(f"{label} must be a regular file")
    return status
...
def _checkpoint(path: Path, label: str, *, include_path: bool = True) -> dict[str, Any]:
    ...
    if status.st_size <= 0:
        raise OsmBlindTileAuxiliaryV2Error(f"{label} is empty")
    record: dict[str, Any] = {
...
def resolve_osmium(executable: str = "osmium") -> str:
    try:
        ...
        raise OsmBlindTileAuxiliaryV2Error(str(error)) from error
...
def _run_text(runner: Runner, command: Sequence[str], label: str) -> tuple[str, str]:
    try:
        ...
        raise OsmBlindTileAuxiliaryV2Error(f"could not execute {label}: {error}") from error
...
def _tool_lineage(executable: str, runner: Runner = subprocess.run) -> dict[str, Any]:
    ...
    if stderr:
        raise OsmBlindTileAuxiliaryV2Error("osmium version emitted stderr")
    lines = stdout.splitlines()
    if len(lines) < 2 or lines[0] != EXPECTED_OSMIUM_FIRST_LINE or lines[1] != EXPECTED_LIBOSMIUM_LINE:
        raise OsmBlindTileAuxiliaryV2Error("osmium/libosmium version is not pinned")
...
def _assert_execution_lineage(
    ...
    *,
    ...
    if _processor_files() != processor.get("files"):
        raise OsmBlindTileAuxiliaryV2Error("processor files drifted during extraction")
    if _runtime_lineage() != processor.get("runtime"):
        raise OsmBlindTileAuxiliaryV2Error("Python/platform runtime drifted during extraction")
    if _tool_lineage(str(tool["executable"]["path"]), runner) != tool:
        raise OsmBlindTileAuxiliaryV2Error("osmium runtime drifted during extraction")
...
def _element_tags(element: ET.Element) -> dict[str, str]:
    ...
    for child in element:
        ...
        if key is None or value is None or key in tags:
            raise OsmBlindTileAuxiliaryV2Error("OSM XML contains an invalid or duplicate tag")
        tags[key] = value
    return tags
...
def _industrial_preselection_reason(
    ...
    if not any(member.attrib.get("role", "") == "outer" for member in members):
        return "relation_missing_outer_member"
    if not any(
        member.attrib.get("role", "") == "outer"
        and member.attrib.get("type") == "way"
        for member in members
    ):
        return "relation_missing_outer_way_member"
    return None
...
def scan_osm_xml(
    ...
    try:
        ...
        try:
            ...
            raise OsmBlindTileAuxiliaryV2Error("osmium broad stream is empty") from error
        if first_event != "start" or root.tag.rsplit("}", 1)[-1] != "osm":
            raise OsmBlindTileAuxiliaryV2Error("osmium broad stream has no OSM root")
        for event, element in iterator:
            ...
            try:
                ...
                raise OsmBlindTileAuxiliaryV2Error("OSM XML object has an invalid ID") from error
        ...
        raise OsmBlindTileAuxiliaryV2Error("osmium broad stream is invalid XML") from error
    if counts["broad_objects"] <= 0 or counts["candidate_objects"] <= 0:
        raise OsmBlindTileAuxiliaryV2Error("broad scan or stable candidate inventory is empty")
    if counts["preselected_industrial"] <= 0:
        raise OsmBlindTileAuxiliaryV2Error("preselected industrial inventory is empty")
...
def filter_contract_document() -> dict[str, Any]:
    ...
    document["geometry_v2"] = {
        ...
        "relation_requires_member_and_outer_role": True,
    ...
    *,
    ...
    with paths.candidate_ids.open("x", encoding="utf-8", newline="\n") as ids_output, \
        ...
        try:
            ...
            raise OsmBlindTileAuxiliaryV2Error(f"could not start osmium scan: {error}") from error
        if process.stdout is None:
            process.terminate()
            process.wait()
            raise OsmBlindTileAuxiliaryV2Error("osmium scan has no stdout stream")
        try:
...
def _run_getid(
    ...
    *,
    ...
    if output.exists() or output.is_symlink():
        raise OsmBlindTileAuxiliaryV2Error("refusing to overwrite an extraction intermediate")
    ...
    if stdout or stderr:
        raise OsmBlindTileAuxiliaryV2Error("osmium getid emitted unexpected output")
...
def _id_key(reference: str) -> tuple[int, int]:
    ...
    if match is None:
        raise OsmBlindTileAuxiliaryV2Error(f"invalid OSM reference {reference!r}")
    ...
    with path.open("r", encoding="utf-8", newline="") as source:
        for line in source:
            if not line.endswith("\n"):
                raise OsmBlindTileAuxiliaryV2Error(f"{label} has an unterminated line")
            value = line[:-1]
            order = _id_key(value)
            if previous is not None and order <= previous:
                raise OsmBlindTileAuxiliaryV2Error(f"{label} is duplicate or unordered")
            previous = order
            result.append(value)
    if not result:
        raise OsmBlindTileAuxiliaryV2Error(f"{label} is empty")
...
def _decode_area_id(value: Any) -> str:
    if not isinstance(value, str):
        raise OsmBlindTileAuxiliaryV2Error("polygon export feature ID is not a string")
    match = re.fullmatch(r"a([1-9][0-9]*)", value)
    if match is None:
        raise OsmBlindTileAuxiliaryV2Error("polygon export feature ID is not an osmium area ID")
    ...
    if area_id % 2 == 0:
        ...
        letter = "r"
    if osm_id <= 0:
        raise OsmBlindTileAuxiliaryV2Error("polygon export area ID decodes to an invalid OSM ID")
    ...
    *,
    ...
    with stderr_path.open("xb") as stderr_output:
        ...
        if process.stdout is None:
            process.terminate()
            process.wait()
            raise OsmBlindTileAuxiliaryV2Error("polygon export has no stdout stream")
        try:
            for raw_line in process.stdout:
                ...
                if not isinstance(feature, Mapping) or feature.get("type") != "Feature":
                    raise OsmBlindTileAuxiliaryV2Error("polygon export emitted a non-feature")
...
def _candidate_records(path: Path) -> Iterable[dict[str, Any]]:
    ...
    with path.open("r", encoding="utf-8", newline="") as source:
        for index, line in enumerate(source, 1):
            if not line.endswith("\n"):
                raise OsmBlindTileAuxiliaryV2Error("candidate ledger line is unterminated")
            ...
            if not isinstance(value, dict) or set(value) != CANDIDATE_RECORD_KEYS:
                raise OsmBlindTileAuxiliaryV2Error("candidate ledger schema is invalid")
            if line != canonical_json_line(value):
                raise OsmBlindTileAuxiliaryV2Error("candidate ledger is not canonical")
            reference = f"{TYPE_LETTERS.get(value.get('osm_type'), '?')}{value.get('osm_id')}"
            order = _id_key(reference)
            if previous is not None and order <= previous:
                raise OsmBlindTileAuxiliaryV2Error("candidate ledger is unordered")
            previous = order
            yield value
...
def _geometry_index(path: Path) -> tuple[
    ...
    try:
        ...
        raise OsmBlindTileAuxiliaryV2Error("unresolved reference XML is invalid") from error
    ...
    for element in root:
        ...
        try:
            ...
            raise OsmBlindTileAuxiliaryV2Error("unresolved XML object ID is invalid") from error
        if kind == "node":
            try:
                longitude = float(element.attrib["lon"])
                latitude = float(element.attrib["lat"])
            except (KeyError, ValueError) as error:
                raise OsmBlindTileAuxiliaryV2Error("unresolved node location is invalid") from error
            if not math.isfinite(longitude) or not math.isfinite(latitude):
                raise OsmBlindTileAuxiliaryV2Error("unresolved node location is non-finite")
            ...
            for child in element:
                if child.tag.rsplit("}", 1)[-1] == "nd":
                    try:
                        ...
                        raise OsmBlindTileAuxiliaryV2Error("unresolved way reference is invalid") from error
            ...
            for child in element:
                ...
                if member_type not in TYPE_LETTERS:
                    raise OsmBlindTileAuxiliaryV2Error("unresolved relation member type is invalid")
                try:
                    member_id = int(child.attrib["ref"])
                except (KeyError, ValueError) as error:
                    raise OsmBlindTileAuxiliaryV2Error("unresolved relation member ID is invalid") from error
...
def _referenced_positions(
    ...
    if key in active:
        raise OsmBlindTileAuxiliaryV2Error("unresolved relation references contain a cycle")
    if object_type == "node":
        if osm_id not in nodes:
            raise OsmBlindTileAuxiliaryV2Error("unresolved node reference is missing")
        return [nodes[osm_id]]
    if object_type == "way":
        if osm_id not in ways:
            raise OsmBlindTileAuxiliaryV2Error("unresolved way reference is missing")
        result: list[tuple[float, float]] = []
        for node_id in ways[osm_id]:
            if node_id not in nodes:
                raise OsmBlindTileAuxiliaryV2Error("unresolved way node is missing")
            result.append(nodes[node_id])
        return result
    if osm_id not in relations:
        raise OsmBlindTileAuxiliaryV2Error("unresolved relation reference is missing")
    active.add(key)
    result = []
...
def _build_ledgers(
    ...
    *,
    ...
    if set(polygon_missing) - set(unresolved_candidates):
        raise OsmBlindTileAuxiliaryV2Error("polygon-missing ID has no candidate record")
    ...
    if final_counts["selected_objects"] <= 0 or final_counts["selected_industrial"] <= 0:
        raise OsmBlindTileAuxiliaryV2Error("final selection inventory is empty")
...
def _normalize_fileinfo(stdout: str, pbf: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise OsmBlindTileAuxiliaryV2Error("osmium fileinfo must be an object")
    file_section = value.get("file")
    data = value.get("data")
    header = value.get("header")
    if not all(isinstance(section, Mapping) for section in (file_section, data, header)):
        raise OsmBlindTileAuxiliaryV2Error("osmium fileinfo sections are missing")
    ...
    if str(file_section.get("format", "")).upper() != "PBF" or file_section.get(
        ...
        raise OsmBlindTileAuxiliaryV2Error("osmium fileinfo does not describe the final PBF")
    count = data.get("count")
    if not isinstance(count, Mapping):
        raise OsmBlindTileAuxiliaryV2Error("osmium fileinfo counts are missing")
    counts: dict[str, int] = {}
    for key in ("nodes", "ways", "relations"):
        number = count.get(key)
        if isinstance(number, bool) or not isinstance(number, int) or number < 0:
            raise OsmBlindTileAuxiliaryV2Error("osmium fileinfo count is invalid")
        counts[key] = number
    return {
...
def _fileinfo(runner: Runner, executable: str, pbf: Path) -> dict[str, Any]:
    ...
    if stderr:
        raise OsmBlindTileAuxiliaryV2Error("osmium fileinfo emitted stderr")
...
def _validate_closed_tree(root: Path) -> None:
    if root.is_symlink() or not root.is_dir():
        raise OsmBlindTileAuxiliaryV2Error("v2 output is not a regular directory")
    if stat.S_IMODE(root.stat().st_mode) != FROZEN_DIRECTORY_MODE:
        raise OsmBlindTileAuxiliaryV2Error("v2 output directory mode must be 0555")
    actual = {path.name for path in root.iterdir()}
    if actual != OUTPUT_NAMES:
        raise OsmBlindTileAuxiliaryV2Error("v2 output inventory is not closed")
    for path in root.iterdir():
        status = _regular_file(path, f"v2 artifact {path.name}")
        if stat.S_IMODE(status.st_mode) != FROZEN_FILE_MODE:
            raise OsmBlindTileAuxiliaryV2Error(
                f"v2 artifact mode must be 0444: {path.name}"
            )
...
def _validate_selection(
    ...
    with ledger_path.open("r", encoding="utf-8", newline="") as source:
        for index, (reference, line) in enumerate(zip(ids, source, strict=True), 1):
            if not line.endswith("\n"):
                raise OsmBlindTileAuxiliaryV2Error("selection ledger line is unterminated")
            ...
            if not isinstance(record, dict) or set(record) != SELECTION_RECORD_KEYS:
                raise OsmBlindTileAuxiliaryV2Error("selection ledger schema is invalid")
            if line != canonical_json_line(record):
                raise OsmBlindTileAuxiliaryV2Error("selection ledger is not canonical")
            if reference != f"{TYPE_LETTERS.get(record.get('osm_type'), '?')}{record.get('osm_id')}":
                raise OsmBlindTileAuxiliaryV2Error("selection ID and ledger identity differ")
            signals = record.get("signals")
            if signals not in (["industrial"], ["grid"], ["industrial", "grid"]):
                raise OsmBlindTileAuxiliaryV2Error("selection signals are invalid")
            status = record.get("geometry_status")
            if "industrial" in signals and status != "polygon_export_verified":
                raise OsmBlindTileAuxiliaryV2Error("industrial selection lacks polygon verification")
            if status == "grid_preserved_industrial_unresolved" and signals != ["grid"]:
                raise OsmBlindTileAuxiliaryV2Error("mixed unresolved grid status is invalid")
            if status not in {
                "polygon_export_verified",
                "grid_geometry_preserved",
                "grid_preserved_industrial_unresolved",
            }:
                raise OsmBlindTileAuxiliaryV2Error("selection geometry status is invalid")
            voltages = record.get("parsed_voltages_v")
            if not isinstance(voltages, list) or any(
                isinstance(value, bool) or not isinstance(value, int) or value <= 0
                for value in voltages
            ):
                raise OsmBlindTileAuxiliaryV2Error("selection voltages are invalid")
            if "grid" in signals and not any(
                value >= v1.MINIMUM_GRID_VOLTAGE_V for value in voltages
            ):
                raise OsmBlindTileAuxiliaryV2Error("grid selection is below 110 kV")
            if not isinstance(record.get("voltage_tags"), dict):
                raise OsmBlindTileAuxiliaryV2Error("selection voltage tags are invalid")
            if any(
            ...
            if power is not None and (not isinstance(power, str) or not power):
                raise OsmBlindTileAuxiliaryV2Error("selection power value is invalid")
...
def _validate_bounds(value: Any) -> list[float] | None:
    ...
    if not isinstance(value, list) or len(value) != 4:
        raise OsmBlindTileAuxiliaryV2Error("unresolved bounds are invalid")
    result: list[float] = []
    for item in value:
        if isinstance(item, bool) or not isinstance(item, (int, float)):
            raise OsmBlindTileAuxiliaryV2Error("unresolved bounds must be numeric")
        number = float(item)
        if not math.isfinite(number):
            raise OsmBlindTileAuxiliaryV2Error("unresolved bounds must be finite")
        result.append(number)
    if not -180 <= result[0] <= result[2] <= 180 or not -90 <= result[1] <= result[3] <= 90:
        raise OsmBlindTileAuxiliaryV2Error("unresolved bounds are outside WGS84")
    return result
...
def _validate_unresolved(
    ...
    with path.open("r", encoding="utf-8", newline="") as source:
        for index, line in enumerate(source, 1):
            if not line.endswith("\n"):
                raise OsmBlindTileAuxiliaryV2Error("unresolved ledger line is unterminated")
            ...
            if not isinstance(record, dict) or set(record) != UNRESOLVED_RECORD_KEYS:
                raise OsmBlindTileAuxiliaryV2Error("unresolved ledger schema is invalid")
            if line != canonical_json_line(record):
                raise OsmBlindTileAuxiliaryV2Error("unresolved ledger is not canonical")
            reference = record.get("source_reference")
            if reference != f"{TYPE_LETTERS.get(record.get('osm_type'), '?')}{record.get('osm_id')}":
                raise OsmBlindTileAuxiliaryV2Error("unresolved source identity is invalid")
            order = _id_key(reference)
            if previous is not None and order <= previous:
                raise OsmBlindTileAuxiliaryV2Error("unresolved ledger is unordered")
            previous = order
            reason = record.get("reason")
            stage = record.get("stage")
            if stage == "preselection_structure":
                if reason not in PRESELECTION_REASONS:
                    raise OsmBlindTileAuxiliaryV2Error("preselection unresolved reason is invalid")
                counts["preselection_geometry_rejections"] += 1
            elif stage == "polygon_export_completeness":
                if reason != POSTEXPORT_REASON:
                    raise OsmBlindTileAuxiliaryV2Error("polygon unresolved reason is invalid")
                counts["polygon_export_unresolved"] += 1
            else:
                raise OsmBlindTileAuxiliaryV2Error("unresolved stage is invalid")
            if record.get("rejected_signal") != "industrial":
                raise OsmBlindTileAuxiliaryV2Error("unresolved rejected signal is invalid")
            accepted = record.get("accepted_signals")
            if accepted not in ([], ["grid"]):
                raise OsmBlindTileAuxiliaryV2Error("unresolved accepted signals are invalid")
            ...
            if not isinstance(location, Mapping) or set(location) != {
                ...
                raise OsmBlindTileAuxiliaryV2Error("unresolved location schema is invalid")
    ...
    if not records:
        raise OsmBlindTileAuxiliaryV2Error("unresolved ledger is empty")
...
def _semantic_polygon_validation(
    ...
    *,
    ...
    if not expected:
        raise OsmBlindTileAuxiliaryV2Error("final industrial inventory is empty")
    with tempfile.TemporaryDirectory(prefix="osm-blind-v2-validate-") as temporary:
        root = Path(temporary)
    ...
    if diagnostics != {
        ...
        raise OsmBlindTileAuxiliaryV2Error("final polygon export emitted diagnostics")
...
def _validate_statistics(
    ...
    if not isinstance(value, Mapping) or set(value) != {"final", "scan"}:
        raise OsmBlindTileAuxiliaryV2Error("selection statistics schema is invalid")
    scan = value.get("scan")
    if not isinstance(scan, Mapping) or set(scan) != SCAN_STATISTIC_KEYS or any(
        isinstance(number, bool) or not isinstance(number, int) or number < 0
        for number in scan.values()
    ):
        raise OsmBlindTileAuxiliaryV2Error("scan statistics are invalid")
    if (
        scan["broad_objects"]
        ...
        + scan["preselection_relation_missing_outer_member"]
        + scan["preselection_relation_missing_outer_way_member"]
    ):
        raise OsmBlindTileAuxiliaryV2Error("scan statistics do not reconcile")
    expected_final = {
    ...
    if set(expected_final) != FINAL_STATISTIC_KEYS or value.get("final") != expected_final:
        raise OsmBlindTileAuxiliaryV2Error("final statistics do not reproduce ledgers")
    if (
        ...
        raise OsmBlindTileAuxiliaryV2Error("final statistics do not reconcile")
...
def validate_bundle(
    ...
    *,
    ...
    document = _decode_json(raw, "v2 extraction manifest")
    if not isinstance(document, dict) or set(document) != MANIFEST_KEYS:
        raise OsmBlindTileAuxiliaryV2Error("v2 manifest top-level schema is invalid")
    if raw != canonical_json(document):
        raise OsmBlindTileAuxiliaryV2Error("v2 extraction manifest is not canonical")
    digest = hashlib.sha256(raw).hexdigest()
    if sidecar_path.read_bytes() != f"{digest}  {MANIFEST_FILENAME}\n".encode("ascii"):
        raise OsmBlindTileAuxiliaryV2Error("v2 manifest sidecar mismatch")
    if (
        ...
        raise OsmBlindTileAuxiliaryV2Error("v2 manifest identity is invalid")
    if document.get("candidate_independence") != _candidate_independence():
        raise OsmBlindTileAuxiliaryV2Error("candidate-independence contract changed")
    if document.get("downstream_contract") != DOWNSTREAM_CONTRACT:
        raise OsmBlindTileAuxiliaryV2Error("downstream unresolved contract changed")
    if document.get("filter") != filter_contract_document():
        raise OsmBlindTileAuxiliaryV2Error("v2 filter contract changed")
    if document.get("commands") != plan.command_contract():
        raise OsmBlindTileAuxiliaryV2Error("v2 command contract changed")
    if document.get("rights") != _rights():
        raise OsmBlindTileAuxiliaryV2Error("v2 rights changed")
    started_at = _timestamp(document.get("started_at"), "v2 started_at")
    finished_at = _timestamp(document.get("finished_at"), "v2 finished_at")
    if finished_at < started_at:
        raise OsmBlindTileAuxiliaryV2Error("v2 extraction finished before it started")
    ...
    if not isinstance(invocation, list) or any(
        ...
        raise OsmBlindTileAuxiliaryV2Error("v2 invocation schema is invalid")
    processor = document.get("processor")
    if processor != _processor_lineage():
        raise OsmBlindTileAuxiliaryV2Error("v2 processor/runtime lineage changed")
    tool = document.get("tool")
    if tool != _tool_lineage(executable, runner):
        raise OsmBlindTileAuxiliaryV2Error("v2 osmium lineage changed")
    ...
    if deep_source_hash:
        ...
        if source_document != verified.manifest_document() or source_document != expected_source_document:
            raise OsmBlindTileAuxiliaryV2Error("v2 source checkpoint changed")
    ...
    if outputs != _output_checkpoints(paths):
        raise OsmBlindTileAuxiliaryV2Error("v2 output checkpoints changed")
    ...
    if not isinstance(completeness, Mapping) or set(completeness) != {
        ...
        raise OsmBlindTileAuxiliaryV2Error("geometry completeness schema is invalid")
    ...
    if (
        ...
        raise OsmBlindTileAuxiliaryV2Error("geometry completeness counts do not reconcile")
    ...
    if not isinstance(diagnostics, Mapping) or set(diagnostics) != {
        ...
        raise OsmBlindTileAuxiliaryV2Error("initial polygon diagnostics are invalid")
    if (
    ...
    if document.get("integrity") != {
        ...
        raise OsmBlindTileAuxiliaryV2Error("v2 integrity schema is invalid")
    ...
    if (
        ...
        raise OsmBlindTileAuxiliaryV2Error("scan stderr checkpoint is invalid")
    ...
    if semantic != {
        ...
        raise OsmBlindTileAuxiliaryV2Error("offline polygon validation counts changed")
    if document.get("fileinfo") != _fileinfo(runner, executable, paths.final_pbf):
        raise OsmBlindTileAuxiliaryV2Error("v2 final PBF fileinfo changed")
...
def extract_auxiliary(
    ...
    *,
    ...
    if plan.output_directory.parent.is_symlink():
        raise OsmBlindTileAuxiliaryV2Error("output parent must not be a symlink")
    staging = Path(
        tempfile.mkdtemp(
    ...
    try:
        ...
        if _source_stat(plan.source_path) != source_stat_before:
            raise OsmBlindTileAuxiliaryV2Error("Planet source metadata drifted during extraction")
        if recheck_source_hash_after:
            ...
            if verified_after != verified:
                raise OsmBlindTileAuxiliaryV2Error("Planet source hash drifted during extraction")
        _assert_execution_lineage(processor, tool, runner=runner)
        ...
        if plan.output_directory.exists() or plan.output_directory.is_symlink():
            raise OsmBlindTileAuxiliaryV2Error("output appeared before atomic publication")
