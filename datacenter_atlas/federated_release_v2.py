"""Versioned federation index with publication-v4 freshness semantics.

The legacy federation module remains unchanged. This successor reuses its
closed validation primitives while extending child manifest descriptors with
the exact publication-v4 lifecycle-freshness contract.
"""
...
def _inspect_child(definition: _ChildDefinition) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(
        prefix="federated-v2-child-", dir="/private/tmp"
    ) as temporary:
...
def validate_federated_release_index(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="federated-v2-index-", dir="/private/tmp"
    ) as temporary:
