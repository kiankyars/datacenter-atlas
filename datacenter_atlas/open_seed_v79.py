"""Build the strict two-source append-only successor to open seed v78."""
...
RELEASE_ID = "2026-07-21-open-seed-v79"
RELEASE = ROOT / "releases" / RELEASE_ID
PUBLICATION_LOCK = ROOT / ".open-seed-v79.lock"
AS_OF = "2026-07-21"

BASE_RECORDED_AT = "2026-07-21T16:35:37Z"
BASE_DEFINITION_PIN = (
    93_532,
...
EXCLUDED_SOURCE_PATHS = frozenset(
    {
        "sources/curated-official-2026-07-21-telcosub-costa-del-este-review-only.json"
    }
)
...
ADDED_ENTITY_KEYS = frozenset(
    {
        "curated:hive-yguazu-paraguay-campus",
        "curated:hive-yguazu-paraguay-campus:2026-100mw-expansion",
        "curated:puntonet-epicentro-quito-campus",
        "curated:puntonet-epicentro-quito-campus:current-build",
    }
)
ADDED_PROJECT_KEYS = frozenset(key for key in ADDED_ENTITY_KEYS if key.count(":") >= 2)
ADDED_EVIDENCE_KEYS = frozenset(
    {
        "hive-yguazu-fy2026-10k-construction-captured-2026-07-21",
        "hive-yguazu-100mw-agreement-captured-2026-07-21",
        "hive-yguazu-current-operations-page-captured-2026-07-21",
        "puntonet-epicentro-summit-specification-captured-2026-07-21",
        "puntonet-epicentro-ceo-construction-update-captured-2026-07-21",
    }
)
ADDED_SOURCE_FAMILIES = frozenset(
    {
        "sec_edgar_hive_annual_filings",
        "hive_company_news_releases",
        "hive_current_operations_pages",
        "puntonet_epicentro_pages",
        "puntonet_executive_official_updates",
    }
)
...
FRESHNESS_README = f"""
Open seed v79 is the exact accepted v78 successor with only the two
seed-eligible source records from
`source_artifacts/global-official-builds-latam-caribbean-gap-2026-07-21-v1`
appended in frozen artifact order. It preserves all 419 v78 inputs, then
appends HIVE Yguazú and Puntonet Epicentro Quito for 421 inputs total.
The frozen official-source manifest is `{OFFICIAL_MANIFEST_PIN[1]}` and its
physical tree is `{OFFICIAL_PHYSICAL_TREE_SHA256}`.

TELCOSUB Costa del Este, Gualeguaychú, and the Trinidad and Tobago modular
data-center program remain review-only and contribute no v79 input or claim.
HIVE's 100 MW row is contracted grid connection only, not critical IT, gross
facility demand, generation, current draw, consumption, or annual energy. Its
crypto-mining workload is direct company disclosure. Puntonet's ambiguous 6 MW
label and rack counts remain narrative, and AI-ready capability creates no AI
workload. No coordinate, operating model, PUE, current-load, or current-status
claim is inferred.
...
class OpenSeedV79Error(RuntimeError):
    """Raised when a v79 lineage, claim, or publication guard fails closed."""
...
def _read_json(
    ...
    if path.is_symlink() or not path.is_file() or not stat.S_ISREG(path.stat().st_mode):
        raise OpenSeedV79Error(f"expected ordinary JSON file: {path}")
    if mode is not None and stat.S_IMODE(path.stat().st_mode) != mode:
        raise OpenSeedV79Error(f"file mode differs: {path}")
    raw = path.read_bytes()
    document = json.loads(raw)
    if raw != _canonical(document, sort_keys=sort_keys):
        raise OpenSeedV79Error(f"JSON is not canonical: {path}")
    return raw, document
...
def _validate_official_artifact() -> dict[str, Any]:
    ...
    if (
        ...
        raise OpenSeedV79Error(
            "official LatAm-Caribbean-gap artifact is not exact and frozen"
        )
    if any(
        path.is_symlink()
        ...
        raise OpenSeedV79Error("official artifact member mode differs")
    ...
    if (len(manifest_raw), _sha256(manifest_raw)) != OFFICIAL_MANIFEST_PIN:
        raise OpenSeedV79Error("official artifact manifest pin differs")
    if (
        ...
        raise OpenSeedV79Error("official artifact boundary differs")
    listed = manifest.get("files")
    if not isinstance(listed, list):
        raise OpenSeedV79Error("official artifact inventory differs")
    actual = {
    ...
    if actual != expected or manifest.get("closed_file_set") != sorted(expected):
        raise OpenSeedV79Error("official artifact is not a closed file set")
    for row in listed:
        raw = (artifact / row["path"]).read_bytes()
        if (len(raw), _sha256(raw)) != (row["bytes"], row["sha256"]):
            raise OpenSeedV79Error(f"official artifact file pin differs: {row['path']}")
    if _sha256(_canonical(listed)) != OFFICIAL_MANIFEST_TREE_SHA256:
        raise OpenSeedV79Error("official artifact manifest tree differs")
    if (artifact / "manifest.sha256").read_text() != (
        f"{OFFICIAL_MANIFEST_PIN[1]}  manifest.json\n"
    ):
        raise OpenSeedV79Error("official artifact sidecar differs")
    ...
    if (
        ...
        raise OpenSeedV79Error("official artifact publication time differs")
    ...
    if (len(snapshot_raw), _sha256(snapshot_raw)) != OFFICIAL_SNAPSHOT_PIN:
        raise OpenSeedV79Error("official source snapshot pin differs")
    if (len(assessment_raw), _sha256(assessment_raw)) != OFFICIAL_ASSESSMENT_PIN:
        raise OpenSeedV79Error("official assessment pin differs")
    if (len(rights_raw), _sha256(rights_raw)) != OFFICIAL_RIGHTS_PIN:
        raise OpenSeedV79Error("official rights pin differs")
    if (
        ...
        raise OpenSeedV79Error("official source-rights boundary differs")
...
def _validate_additions(
    ...
    if tuple(ADDITION_PINS) != ADDITION_ORDER or len(ADDITION_PINS) != 2:
        raise OpenSeedV79Error("v79 requires exactly two ordered additions")
    carrier = _validate_official_artifact()
    target = v70.parse_utc(recorded_at, label="v79 recorded_at")
    wall = validation_wall_clock or datetime.now(UTC)
    if wall.tzinfo is None or target > wall.astimezone(UTC):
        raise OpenSeedV79Error("v79 recorded_at is later than validation wall clock")
    if v70.parse_utc(OFFICIAL_RECORDED_AT, label="official recorded_at") > target:
        raise OpenSeedV79Error("official artifact post-dates v79")
    ...
    if (
        ...
        or snapshot.get("totals", {}).get("review_only_source_records") != 1
        ...
        or snapshot.get("totals", {}).get("workload_observations") != 1
        ...
        raise OpenSeedV79Error("official source-snapshot boundary differs")
    record_by_path = {row.get("path"): row for row in records}
    expected_record_order = (*ADDITION_ORDER, *EXCLUDED_SOURCE_PATHS)
    if (
        len(record_by_path) != 3
        or tuple(row.get("path") for row in records) != expected_record_order
    ):
        raise OpenSeedV79Error("official source-snapshot paths collide")
    accepted_records = {
    ...
    if {
        ...
        raise OpenSeedV79Error("official seed-eligible source inventory differs")
    excluded_records = {
    ...
    if (
        ...
        raise OpenSeedV79Error("LatAm-Caribbean source inventory differs")
    ...
    if (
        ...
        or {
            ...
            for key in (
                "telcosub-telconet-costa-del-este-edge-dc",
                "gualeguaychu-municipal-cpd-finishing-stage",
                "gortt-tier-iv-rated-4-modular-data-center",
            )
        }
        != {
            "telcosub-telconet-costa-del-este-edge-dc": "review_only_stale_physical_start_no_later_site_status",
            "gualeguaychu-municipal-cpd-finishing-stage": "review_only_historical_completed_by_2026_05_22",
            "gortt-tier-iv-rated-4-modular-data-center": "review_watch_cutoff_state_and_location_unresolved",
        }
        or any(
        ...
        or candidates["telcosub-telconet-costa-del-este-edge-dc"].get(
            "seed_eligible"
        )
        is not False
        or candidates["telcosub-telconet-costa-del-este-edge-dc"].get(
        ...
        raise OpenSeedV79Error("LatAm-Caribbean disposition boundary differs")
    ...
    for relative, expected_pin in EXCLUDED_SOURCE_PINS.items():
        ...
        if (len(raw), _sha256(raw)) != expected_pin:
            raise OpenSeedV79Error(f"v79 excluded source byte pin differs: {relative}")
        if max(metadata.st_birthtime, metadata.st_mtime) > target.timestamp() + 1e-6:
            raise OpenSeedV79Error(f"v79 excluded source post-dates publication: {relative}")
        if (
            ...
            raise OpenSeedV79Error("v79 TELCOSUB review-only boundary differs")
    for relative in ADDITION_ORDER:
        ...
        if (len(raw), _sha256(raw)) != expected_pin:
            raise OpenSeedV79Error(f"v79 source byte pin differs: {relative}")
        if max(metadata.st_birthtime, metadata.st_mtime) > target.timestamp() + 1e-6:
            raise OpenSeedV79Error(f"v79 source post-dates publication: {relative}")
        if document.get("schema_version") != "1.1":
            raise OpenSeedV79Error(f"v79 source schema differs: {relative}")
        documents[relative] = document
        for row in document.get("evidence", []):
            key = row.get("key")
            if key in evidence:
                raise OpenSeedV79Error(f"v79 duplicate evidence key: {key}")
            ...
            if retrieved > target or retrieved > wall.astimezone(UTC):
                raise OpenSeedV79Error("v79 source evidence is future-dated")
            ...
            if (
                ...
                raise OpenSeedV79Error(f"v79 evidence lineage differs: {key}")
            evidence[key] = row
        for entity_name in ("campus", "project"):
            row = document.get(entity_name)
            if not isinstance(row, dict) or row.get("stable_key") in entities:
                raise OpenSeedV79Error(f"v79 entity inventory differs: {relative}")
            if set(row) != {
                "address",
                "as_of_date",
                "confidence",
                ...
                raise OpenSeedV79Error(
                    f"v79 entity classification or role boundary differs: {relative}"
                )
            ...
            if expected_point is None:
                if row.get("coordinates") is not None or row.get("geometry") is not None:
                    raise OpenSeedV79Error(
                        f"v79 source gained unsupported coordinates: {relative}"
                    )
            elif (
                row.get("coordinates")
                ...
                raise OpenSeedV79Error(f"v79 official coordinate differs: {relative}")
        ...
        for row in document.get("capacities", []):
            ...
            if not float(row["low"]) == float(row["base"]) == float(row["high"]):
                raise OpenSeedV79Error("v79 source capacity range differs")
    ...
    if set(entities) != ADDED_ENTITY_KEYS or set(evidence) != ADDED_EVIDENCE_KEYS:
        raise OpenSeedV79Error("v79 identity or evidence contract differs")
    if (
        ...
        raise OpenSeedV79Error(
            "v79 lifecycle, capacity, model, or workload contract differs"
        )
    hive = documents[ADDITION_ORDER[0]]
    puntonet = documents[ADDITION_ORDER[1]]
    if (
        ...
        or len(hive["workloads"]) != 1
        or hive["workloads"][0].get("value") != "crypto_mining"
        or hive["capacities"][0].get("entity") != "project"
        or hive["capacities"][0].get("metric") != "grid_connection_mw"
        or hive["capacities"][0].get("stage") != "contracted"
        or "not IT load" not in hive["capacities"][0].get("notes", "")
        or "as IT load"
        not in hive["evidence"][0]["metadata"].get("capacity_guardrail", "")
        or hive["campus"].get("coordinates") is not None
        or hive["project"].get("coordinates") is not None
        or puntonet["capacities"] != []
        or puntonet["workloads"] != []
        or puntonet["evidence"][0]["metadata"].get("power_label_as_reported")
        != "6MWATTS de energía"
        or "creates no normalized capacity"
        not in puntonet["evidence"][0]["metadata"].get("capacity_guardrail", "")
        or "creates no active AI workload"
        not in puntonet["evidence"][0]["metadata"].get("workload_guardrail", "")
        or puntonet["campus"].get("coordinates") is not None
        or puntonet["project"].get("coordinates") is not None
        or any("telcosub" in relative for relative in ADDITION_ORDER)
    ):
        raise OpenSeedV79Error("v79 LatAm-Caribbean claim guardrail differs")
    return documents
...
def _base_paths(base: Mapping[str, Any]) -> list[Path]:
    ...
    for row in base["curated_inputs"]:
        ...
        if (
            ...
            raise OpenSeedV79Error(f"accepted v78 input pin differs: {row['path']}")
        paths.append(path)
    return paths
...
def selected_inputs(
    ...
    *,
    ...
    if base.get("release_id") != "2026-07-21-open-seed-v78":
        raise OpenSeedV79Error("v79 base must be exactly accepted v78")
    rows = base.get("curated_inputs")
    if not isinstance(rows, list) or len(rows) != 419:
        raise OpenSeedV79Error("accepted v78 curated inventory differs")
    if any(not isinstance(row, dict) or set(row) != {"path", "sha256"} for row in rows):
        raise OpenSeedV79Error("accepted v78 curated rows differ")
    base_names = [row["path"] for row in rows]
    if len(set(base_names)) != 419:
        raise OpenSeedV79Error("accepted v78 input paths collide")
    ...
    if set(base_names) & (set(ADDITION_PINS) | EXCLUDED_SOURCE_PATHS):
        raise OpenSeedV79Error("v78 already contains a LatAm-Caribbean source")
    ...
    if base_entity_keys & ADDED_ENTITY_KEYS or base_evidence_keys & ADDED_EVIDENCE_KEYS:
        raise OpenSeedV79Error("v79 addition collides with accepted v78 semantics")
    ...
    if (
        selected[:419] != rows
        or [row["path"] for row in selected[419:]] != list(ADDITION_ORDER)
        or tuple(documents) != ADDITION_ORDER
        or len(selected) != 421
        or set(row["path"] for row in selected) & EXCLUDED_SOURCE_PATHS
    ):
        raise OpenSeedV79Error("v79 did not preserve v78 and append exactly two inputs")
...
def _validate_database_contract(
    ...
    *,
    ...
    if actual_counts != expected_counts:
        raise OpenSeedV79Error(f"v79 database counts differ: {actual_counts}")
    with tempfile.TemporaryDirectory(
        prefix="open-seed-v79-base-", dir="/private/tmp"
    ) as td:
        prior = v69._populate_database(
            base, _base_paths(base), Path(td) / "v78.sqlite", recorded_at=recorded_at
        )
        try:
            tables = (
                "entities",
            ...
            for table in tables:
                if not _table_state(prior, table) <= _table_state(connection, table):
                    raise OpenSeedV79Error(f"v79 changed a prior database row: {table}")
            ...
            if after_keys - before_keys != ADDED_ENTITY_KEYS or before_keys - after_keys:
                raise OpenSeedV79Error("v79 database identity delta differs")
            ...
            if (
                ...
                raise OpenSeedV79Error("v79 database evidence delta differs")
        finally:
            prior.close()
    ...
    if {key for key, _ in kinds} != ADDED_ENTITY_KEYS or {
        ...
        raise OpenSeedV79Error("v79 entity-kind contract differs")
    ...
    if (
        ...
        raise OpenSeedV79Error("v79 imported claim contract differs")
    for stable_key, latitude, longitude, geometry in connection.execute(
        ...
        if expected_point is None:
            if latitude is not None or longitude is not None or geometry not in {
                ...
                raise OpenSeedV79Error(
                    f"v79 imported unsupported coordinates: {stable_key}"
                )
        elif (
            ...
            raise OpenSeedV79Error(f"v79 imported coordinate differs: {stable_key}")
    ...
    *,
    ...
    if precreated:
        if output.is_symlink() or not output.is_dir() or any(output.iterdir()):
            raise OpenSeedV79Error("precreated v79 release stage must be empty")
    else:
...
CSV_DELTA_COUNT_CONTRACT = {
    "entities.csv": (862, 4, 0),
    "evidence.csv": (551, 3, 0),
    "capacity_estimates.csv": (536, 1, 0),
    "construction_pipeline.csv": (442, 2, 0),
    "construction_source_signals.csv": (344, 2, 0),
    "resolution_candidates.csv": (7, 0, 0),
    "lifecycle_freshness.csv": (484, 2, 0),
}
...
def _validate_release_delta(stage: Path, *, recorded_at: str) -> None:
    ...
    for filename, expected in CSV_DELTA_COUNT_CONTRACT.items():
        ...
        if actual != expected:
            raise OpenSeedV79Error(
                f"v79 public CSV delta differs: {filename}: {actual}"
            )
    ...
    if set(after_entities) - set(before_entities) != ADDED_ENTITY_KEYS:
        raise OpenSeedV79Error("v79 public identity delta differs")
    if set(before_entities) - set(after_entities) or any(
        after_entities[key] != row for key, row in before_entities.items()
    ):
        raise OpenSeedV79Error("v79 changed an accepted v78 public entity")
    for stable_key in ADDED_ENTITY_KEYS:
        ...
        if expected_point is None:
            if row["latitude"] or row["longitude"] or row["geometry_json"] != "null":
                raise OpenSeedV79Error("v79 public addition gained unsupported coordinates")
        elif (
            ...
            raise OpenSeedV79Error("v79 public official coordinate differs")
        ...
        if expected_workload is None:
            if workloads != []:
                raise OpenSeedV79Error("v79 public addition gained a workload")
        elif (
            len(workloads) != 1
            or workloads[0].get("workload") != expected_workload
            or workloads[0].get("as_of_date") != "2026-06-01"
            or workloads[0].get("method") != "company_disclosure"
        ):
            raise OpenSeedV79Error("v79 public HIVE workload differs")
        capacities = json.loads(row.get("capacity_estimates_json", "null"))
        expected_capacity = {
            "curated:hive-yguazu-paraguay-campus:2026-100mw-expansion": 100.0,
        }.get(stable_key)
        if expected_capacity is None:
            if capacities != []:
                raise OpenSeedV79Error("v79 public addition gained unsupported capacity")
        elif (
            len(capacities) != 1
            or capacities[0].get("metric") != "grid_connection_mw"
            or capacities[0].get("stage") != "contracted"
            or capacities[0].get("unit") != "MW"
            or float(capacities[0].get("base")) != expected_capacity
        ):
            raise OpenSeedV79Error("v79 public HIVE grid capacity differs")
    ...
    if set(before_evidence) - set(after_evidence) or any(
        ...
        raise OpenSeedV79Error("v79 changed accepted v78 public evidence")
    added_evidence = [
        row for key, row in after_evidence.items() if key not in before_evidence
    ]
    if (
        len(added_evidence) != 3
        ...
        raise OpenSeedV79Error("v79 public evidence projection differs")
    ...
    if (
        ...
        raise OpenSeedV79Error("v79 construction-pipeline delta differs")
    before_signals = {
    ...
    if (
        ...
        raise OpenSeedV79Error("v79 construction-signal delta differs")
    signal_by_key = {
    ...
    for stable_key, row in signal_by_key.items():
        ...
        if expected_point is None:
            if row["representative_latitude"] or row["representative_longitude"]:
                raise OpenSeedV79Error("v79 signal gained unsupported coordinates")
        elif (
            float(row["representative_latitude"]) != expected_point[0]
            or float(row["representative_longitude"]) != expected_point[1]
        ):
            raise OpenSeedV79Error("v79 signal official coordinate differs")
    ...
    if _normalize_timestamps(before_resolution, recorded_at) != _normalize_timestamps(
        ...
        raise OpenSeedV79Error("v79 changed the resolution advisory")
    for filename in ("resolution_candidates.csv", "resolution_candidates.json"):
        if (stage / filename).read_bytes() != (BASE_RELEASE / filename).read_bytes():
            raise OpenSeedV79Error(f"v79 changed invariant release file: {filename}")
    ...
    if (
        ...
        raise OpenSeedV79Error("v79 source-input projection differs")
    ...
    if set(after_features) - set(before_features) != ADDED_ENTITY_KEYS or any(
        ...
        raise OpenSeedV79Error("v79 GeoJSON base preservation differs")
    for stable_key in ADDED_ENTITY_KEYS:
        expected_point = COORDINATE_CONTRACT.get(stable_key)
        expected_geometry = (
            None
        ...
        if after_features[stable_key]["geometry"] != expected_geometry:
            raise OpenSeedV79Error("v79 GeoJSON coordinate boundary differs")
...
def _validate_release_facts(stage: Path, *, recorded_at: str) -> None:
    ...
    expected_summary = {
        ...
        "capacity_estimates_current": 537,
        "construction_pipeline_records": 444,
        "construction_source_signals": 346,
        "entities_total": 866,
        "entities_with_coordinates": 200,
        "evidence_total": 690,
        "lifecycle_observations_current": 486,
        "projects_total": 411,
        "recorded_at": recorded_at,
    }
    if {key: summary.get(key) for key in expected_summary} != expected_summary:
        raise OpenSeedV79Error(
            f"v79 summary facts differ: "
            f"{ {key: summary.get(key) for key in expected_summary} }"
        )
    if (
        summary.get("entities_by_status", {}).get("under_construction") != 335
        or summary.get("entities_by_status", {}).get("shell") != 30
    ):
        raise OpenSeedV79Error("v79 last-observed status summary differs")
    if (
        ...
        raise OpenSeedV79Error("v79 typed-capacity summary differs")
    ...
    expected_summary_document["country_assignment_counts"]["not_evaluated"] = 866
    expected_summary_document["country_source_claims_by_method"][
        "source_explicit_country_tag"
    ] = 866
    expected_summary_document["country_source_tag_fallbacks"] = 866
    expected_summary_document["entities_by_country"].update(
        {"Ecuador": 2, "Paraguay": 2}
    )
    expected_summary_document["entities_by_kind"] = {"campus": 455, "project": 411}
    expected_summary_document["entities_by_status"].update(
        {"under_construction": 335}
    )
    expected_summary_document["evidence_by_kind"].update(
        {"company_disclosure": 501}
    )
    expected_summary_document["capacity_estimates_by_metric"]["grid_connection_mw"] = 24
    expected_summary_document["capacity_estimates_by_stage"]["contracted"] = 19
    if summary != expected_summary_document:
        raise OpenSeedV79Error("v79 full summary delta differs")

    manifest = json.loads((stage / "manifest.json").read_text())
    expected_manifest = {
        "entities": 866,
        "evidence_records": 554,
        "capacity_estimates": 537,
        "construction_pipeline_records": 444,
        "construction_source_signals": 346,
        "resolution_candidates": 7,
        "lifecycle_freshness_records": 486,
    ...
    if {key: manifest.get(key) for key in expected_manifest} != expected_manifest:
        raise OpenSeedV79Error("v79 release manifest facts differ")
    ...
    if (
        ...
        or len(manifest["source_families"]) != 322
    ):
        raise OpenSeedV79Error("v79 public source-family delta differs")
    ...
    expected_manifest_document.update(
        {
            "entities": 866,
            "entities_by_kind": {"campus": 455, "project": 411},
            "evidence_records": 554,
            "capacity_estimates": 537,
            "construction_pipeline_records": 444,
            "construction_source_signals": 346,
    ...
    if (
        ...
        raise OpenSeedV79Error("v79 full manifest delta differs")
    ...
    if (
        ...
        or classes
        != {
            "recent_0_90_days": 252,
            "aging_91_365_days": 202,
        ...
        raise OpenSeedV79Error("v79 freshness/current-status boundary differs")
    expected_freshness = {
        "curated:hive-yguazu-paraguay-campus:2026-100mw-expansion": (
            "under_construction",
            "2026-06-01",
    ...
    for stable_key, expected in expected_freshness.items():
        ...
        if actual != expected:
            raise OpenSeedV79Error(f"v79 source freshness differs: {stable_key}")
    ...
    if projected_capacities != {
        ...
        raise OpenSeedV79Error("v79 public capacity boundary differs")

    readme = (stage / "README.md").read_text()
    for marker in (
        OFFICIAL_MANIFEST_PIN[1],
        OFFICIAL_PHYSICAL_TREE_SHA256,
        "TELCOSUB Costa del Este",
        "Gualeguaychú",
        "Trinidad and Tobago modular\ndata-center program remain review-only",
        "100 MW row is contracted grid connection only",
        "crypto-mining workload is direct company disclosure",
        "ambiguous 6 MW\nlabel and rack counts remain narrative",
        "AI-ready capability creates no AI\nworkload",
        "current_status_classification`\nremains `unknown",
        "current_construction_claim` remains `false",
    ):
        if marker not in readme:
            raise OpenSeedV79Error(f"v79 README guardrail differs: {marker}")
    if len(list(stage.iterdir())) != 14:
        raise OpenSeedV79Error("v79 release file inventory count differs")
...
def _validate_definition(
    ...
    *,
    ...
    if set(document) != set(base) or document.get("release_id") != RELEASE_ID:
        raise OpenSeedV79Error("v79 definition identity or schema differs")
    build = document.get("build")
    if not isinstance(build, dict) or set(build) != {"as_of", "recorded_at"}:
        raise OpenSeedV79Error("v79 definition build carrier differs")
    if build["as_of"] != AS_OF:
        raise OpenSeedV79Error("v79 as_of differs")
    recorded = v70.parse_utc(build["recorded_at"], label="v79 recorded_at")
    if (
    ...
    for key in (
        ...
        if document.get(key) != base.get(key):
            raise OpenSeedV79Error(f"v79 inherited definition field differs: {key}")
...
def _validate_publication_times(
    ...
    *,
    ...
    for path in (definition, release, *release.iterdir()):
        ...
        if max(metadata.st_birthtime, metadata.st_mtime) > target.timestamp() + 1e-6:
            raise OpenSeedV79Error(f"v79 staged inode post-dates recorded_at: {path.name}")
    if require_live:
        if datetime.now(UTC) < target:
            raise OpenSeedV79Error("v79 recorded_at is not live")
        for path in (definition, release):
            if path.stat(follow_symlinks=False).st_ctime + 1e-6 < target.timestamp():
                raise OpenSeedV79Error(
                    f"v79 final root ctime predates recorded_at: {path.name}"
                )


def validate_open_seed_v79(
    ...
    *,
    ...
    if replay_count != 2:
        raise OpenSeedV79Error("v79 requires exactly two offline replays")
    wall = validation_wall_clock or datetime.now(UTC)
    guard = _guard_state()
    if guard["base_definition"] != BASE_DEFINITION_PIN:
        raise OpenSeedV79Error("accepted v78 definition pin differs")
    if (
        guard["base_manifest"] != BASE_MANIFEST_SHA256
        or guard["base_tree"] != BASE_TREE_SHA256
    ):
        raise OpenSeedV79Error("accepted v78 release pin differs")
    if guard["excluded"] != EXCLUDED_SOURCE_PINS:
        raise OpenSeedV79Error("v79 excluded source pin differs")
    ...
    if definition.get("curated_inputs") != selected:
        raise OpenSeedV79Error("v79 selected input inventory differs")
    if release_path.is_symlink() or not release_path.is_dir():
        raise OpenSeedV79Error("v79 release must be an ordinary directory")
    if require_frozen and stat.S_IMODE(release_path.stat().st_mode) != 0o555:
        raise OpenSeedV79Error("v79 release root is not frozen")
    ...
    if any(path.is_symlink() or not path.is_file() for path in release_files.values()):
        raise OpenSeedV79Error("v79 release contains a non-file")
    if require_frozen and any(
        stat.S_IMODE(path.stat().st_mode) != 0o444 for path in release_files.values()
    ):
        raise OpenSeedV79Error("v79 release file is not frozen")
    ...
    if _sha256(manifest_raw) != definition["expected_release"].get(
        ...
        raise OpenSeedV79Error("v79 manifest hash differs")
    expected_release = {
        key: value
    ...
    if {key: summary[key] for key in definition["expected_summary"]} != definition[
        ...
        raise OpenSeedV79Error("v79 expected summary differs")

    for replay in range(replay_count):
        with tempfile.TemporaryDirectory(
            prefix=f"open-seed-v79-replay-{replay + 1}-", dir="/private/tmp"
        ) as td:
            root = Path(td)
            ...
            if {path.name for path in replay_release.iterdir()} != set(release_files):
                raise OpenSeedV79Error("v79 replay file inventory differs")
            for filename, frozen in release_files.items():
                if (replay_release / filename).read_bytes() != frozen.read_bytes():
                    raise OpenSeedV79Error(f"v79 offline replay differs: {filename}")
    if _guard_state() != guard:
        raise OpenSeedV79Error("v79 validation mutated accepted inputs")
    return manifest
...
def _path_identity(path: Path, *, directory: bool) -> tuple[int, int]:
    ...
    if not expected:
        raise OpenSeedV79Error(f"v79 stage type differs: {path}")
...
def _assert_release_identities(
    ...
    if _path_identity(root, directory=True) != root_identity:
        raise OpenSeedV79Error("v79 release stage root identity changed")
    if _release_identities(root) != dict(members):
        raise OpenSeedV79Error("v79 release stage member identity changed")
...
def _discard_file_stage(path: Path, identity: tuple[int, int]) -> None:
    ...
    if _path_identity(path, directory=False) != identity:
        raise OpenSeedV79Error("refusing substituted v79 definition cleanup")
    path.chmod(0o600)
    path.unlink()
...
def _publication_lock() -> Iterator[None]:
    try:
        ...
        raise OpenSeedV79Error("active v79 publication lock exists") from error
    ...
    try:
        ...
        try:
            ...
            if not stat.S_ISREG(current.st_mode) or (
                ...
                raise OpenSeedV79Error("refusing substituted v79 lock cleanup")
...
def _require_absent(label: str) -> None:
    if DEFINITION.exists() or DEFINITION.is_symlink():
        raise OpenSeedV79Error(f"{label} v79 definition collision")
    if RELEASE.exists() or RELEASE.is_symlink():
        raise OpenSeedV79Error(f"{label} v79 release collision")
...
def _rollback_release(release_identity: tuple[int, int], release_stage: Path) -> None:
    if _path_identity(RELEASE, directory=True) != release_identity:
        raise OpenSeedV79Error("refusing rollback of substituted v79 release")
    if release_stage.exists() or release_stage.is_symlink():
        raise OpenSeedV79Error("v79 release rollback stage is occupied")
    v69.promote_noreplace(RELEASE, release_stage)


def build_open_seed_v79(recorded_at: str | None = None) -> dict[str, Any]:
    """Build and atomically publish the two-source v78 successor."""

    if (DEFINITION.exists() or DEFINITION.is_symlink()) and (
        RELEASE.exists() or RELEASE.is_symlink()
    ):
        manifest = validate_open_seed_v79()
        return {
    ...
    if (
        ...
        raise OpenSeedV79Error("partial v79 final-path collision")
    ...
    if guard["excluded"] != EXCLUDED_SOURCE_PINS:
        raise OpenSeedV79Error("v79 excluded source pin differs")
    target = (
        v70.parse_utc(recorded_at, label="v79 recorded_at")
        if recorded_at is not None
        else datetime.now(UTC).replace(microsecond=0) + timedelta(seconds=45)
    )
    if datetime.now(UTC) >= target:
        raise OpenSeedV79Error("v79 recorded_at must be future before staging")
    ...
    with _publication_lock():
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(
                prefix="open-seed-v79-db-", dir="/private/tmp"
            ) as td:
                connection = _build_database(
            ...
            if _path_identity(definition_stage, directory=False) != definition_identity:
                raise OpenSeedV79Error("v79 definition stage identity changed")
            ...
            if (
                ...
                raise OpenSeedV79Error("v79 private stage changed while waiting")
            _validate_publication_times(
                definition_stage,
                release_stage,
                recorded_at=recorded_at,
            ...
            try:
                ...
                try:
                    ...
                    error.add_note(f"v79 release rollback failed: {rollback_error}")
                raise
            manifest = validate_open_seed_v79(DEFINITION, RELEASE)
        finally:
    ...
    if _guard_state() != guard:
        raise OpenSeedV79Error("v79 build mutated accepted inputs")
    return {
        "definition": str(DEFINITION),
        "definition_sha256": v69.sha256(DEFINITION),
        "manifest_sha256": v69.sha256(RELEASE / "manifest.json"),
...
def main() -> int:
    print(json.dumps(build_open_seed_v79(), indent=2, sort_keys=True))
    return 0
