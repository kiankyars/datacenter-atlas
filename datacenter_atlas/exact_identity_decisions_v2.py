"""Exact-identity decisions over a publication-v4 federation.

The legacy identity implementation remains byte-pinned. This successor keeps
its identity and topology semantics unchanged while validating federation-v2
descriptors, including their last-observed lifecycle contract, against every
hash-pinned child release before producing any output.
"""
...
def _federation_input(
    ...
    try:
        index = federation_v2.validate_federated_release_index(
            directory,
            child_release_paths=child_paths,
        )
    except (federation_v2.FederatedReleaseError, OSError) as error:
        raise ExactIdentityDecisionError(
            f"federated v2 index validation failed: {error}"
        ) from error
    manifest_checkpoint = legacy._checkpoint(directory / MANIFEST_FILENAME)
    index_checkpoint = legacy._checkpoint(
        directory / federation_v2.INDEX_FILENAME
    )
    if (
        ...
        raise ExactIdentityDecisionError("federated v2 index hash drifted")
    if (
        index.get("schema_version") != federation_v2.INDEX_SCHEMA_VERSION
        or index.get("format") != federation_v2.INDEX_FORMAT
    ):
        raise ExactIdentityDecisionError("federated v2 index format is unsupported")
    generated_at = legacy._timestamp(
        index.get("generated_at"), "federated v2 generated_at"
    )
    if datetime.fromisoformat(generated_at.replace("Z", "+00:00")) >= (
        datetime.fromisoformat(definition.recorded_at.replace("Z", "+00:00"))
    ):
        raise ExactIdentityDecisionError(
            "decision recorded_at must follow the federated v2 index"
        )
    return dict(index), {
...
def _inspect_children(
    ...
    if not isinstance(descriptors, list):
        raise ExactIdentityDecisionError(
            "federated v2 release descriptors are invalid"
        )
    by_id = {
    ...
    for value in definition.children:
        ...
        try:
            freshness = federation_v2._freshness_contract(
                manifest,
                label=f"{release_id} child manifest",
            )
            descriptor_freshness = federation_v2._freshness_contract(
                descriptor_manifest,
                label=f"{release_id} federation descriptor",
            )
        except federation_v2.FederatedReleaseError as error:
            raise ExactIdentityDecisionError(
        ...
        if freshness is not None:
            try:
                federation_v2._validate_freshness_csv(
                    directory,
...
def validate_exact_identity_decision_bundle(
    ...
    *,
    ...
    """Validate a closed bundle and optionally replay all v2-bound inputs."""
...
def write_exact_identity_decision_bundle(
    ...
    """Double-rebuild, freeze, and atomically publish one v2-bound bundle."""
