"""Accept one sealed Unknown036 catalog-only satellite continuation.

Unknown036 is an APFS copy-on-write continuation of accepted Unknown035.  Its
...
SOURCE = ROOT / "satellite_review_runs/2026-07-22-global-open-v3-unknown-035"
DESTINATION = ROOT / "satellite_review_runs/2026-07-22-global-open-v3-unknown-036"
DESTINATION_LOCK = DESTINATION.with_name(f"{DESTINATION.name}.lock")
SOURCE_ACCEPTANCE = (
    ROOT / "satellite_review_continuations/"
    "2026-07-22-global-open-v3-unknown-035-continued-25"
)

ARTIFACT_ID = "2026-07-22-global-open-v3-unknown-036-continued-25"
OUTPUT = ROOT / "satellite_review_continuations" / ARTIFACT_ID
ACCEPTED_AT = "2026-07-22T05:52:00Z"
...
SOURCE_ACCEPTANCE_MANIFEST_PIN = (
    9_766,
    "1fd9851e7ab43bc7865c62c47abddb009c4fc69058ef8dd86db5b9da88b1e9b9",
)
SOURCE_ACCEPTANCE_SIDECAR_PIN = (
    93,
    "b9a09ead0ad4c6c55164725a6c1ebd24024524f16724b870d375b84b54a59dec",
)
QUEUE_MANIFEST_PIN = (
    28_269,
...
EXPECTED_RUN = {
    ...
    "finished_at": "2026-07-22T05:37:57Z",
    ...
    "max_jobs": 25,
    "started_at": "2026-07-22T05:36:23Z",
}
...
POSITION_START = 4_820
POSITION_END = 4_844
NEXT_PENDING_POSITION = 4_845
NEXT_PENDING_QUEUE_ID = "satq-e5631eb90d392d5c6074b2cf"
NO_SCENE_QUEUE_ID = "satq-5373102bb99ed55904d02412"
EXPECTED_UNAVAILABILITY = {
    ...
    "query_window": {
        "bbox": "113.8900339,22.477747,113.9289693,22.5137198",
        "end_date": "2024-07-30",
    ...
    "recorded_at": "2026-07-22T05:36:51Z",
    "window": "baseline",
}
EXPECTED_DELTA = {
    ...
    "jobs_selected": 0,
    "jobs_unavailable_no_scene": 1,
    "logical_bytes_added": 12_721_278,
    "preexisting_non_manifest_files_byte_identical": 13_511,
}
...
BUILDER_FILES = (
    "datacenter_atlas/satellite_unknown036_continuation_acceptance.py",
    "scripts/accept_satellite_unknown036_continuation.py",
)


class Unknown036ContinuationAcceptanceError(RuntimeError):
...
def _path_record(path: Path) -> dict[str, Any]:
    if path.is_symlink() or not path.is_file():
        raise Unknown036ContinuationAcceptanceError(
...
def _assert_pin(path: Path, pin: tuple[int, str], label: str) -> bytes:
    if path.is_symlink() or not path.is_file():
        raise Unknown036ContinuationAcceptanceError(f"{label} is not an ordinary file")
    raw = path.read_bytes()
    if (len(raw), _sha256(raw)) != pin:
        raise Unknown036ContinuationAcceptanceError(f"{label} pin changed")
    return raw
...
def _json_object(raw: bytes, label: str) -> dict[str, Any]:
    try:
        ...
        raise Unknown036ContinuationAcceptanceError(f"{label} is not JSON") from error
    if not isinstance(value, dict):
        raise Unknown036ContinuationAcceptanceError(f"{label} is not an object")
    return value


def _timestamp(value: str, label: str) -> datetime:
    if not isinstance(value, str) or not value.endswith("Z"):
        raise Unknown036ContinuationAcceptanceError(f"{label} is not canonical UTC")
    try:
    ...
    if parsed.microsecond:
        raise Unknown036ContinuationAcceptanceError(f"{label} must use whole seconds")
...
def _frozen(root: Path, label: str) -> None:
    ...
    if not files or not directories:
        raise Unknown036ContinuationAcceptanceError(f"{label} is empty")
    for path in files:
...
def _validate_source_acceptance() -> dict[str, Any]:
    _frozen(SOURCE_ACCEPTANCE, "Unknown035 acceptance wrapper")
    raw = _assert_pin(
        SOURCE_ACCEPTANCE / MANIFEST_FILENAME,
        SOURCE_ACCEPTANCE_MANIFEST_PIN,
        "Unknown035 acceptance manifest",
    )
    manifest = _json_object(raw, "Unknown035 acceptance manifest")
    ...
    if (
        ...
        != {"queue_id": NO_SCENE_QUEUE_ID, "queue_position": POSITION_START}
    ):
        raise Unknown036ContinuationAcceptanceError(
            "Unknown035 acceptance semantics changed"
        )
    sidecar = _assert_pin(
        SOURCE_ACCEPTANCE / SIDECAR_FILENAME,
        SOURCE_ACCEPTANCE_SIDECAR_PIN,
        "Unknown035 acceptance sidecar",
    )
    if sidecar != (
...
def _validate_lock() -> dict[str, Any]:
    if (
        ...
        raise Unknown036ContinuationAcceptanceError("Unknown036 canonical lock changed")
...
def _audit() -> dict[str, Any]:
    ...
    lock = _validate_lock()
    _frozen(SOURCE, "accepted Unknown035 source")
    _frozen(DESTINATION, "accepted Unknown036 destination")
    ...
    if not set(source_files).issubset(destination_files):
        raise Unknown036ContinuationAcceptanceError("Unknown036 removed a source file")
    ...
    if changed_non_manifest:
        raise Unknown036ContinuationAcceptanceError(
            "Unknown036 changed inherited non-manifest bytes"
        )
    ...
    if [task["queue_position"] for _queue_id, task in selected] != list(
        ...
        raise Unknown036ContinuationAcceptanceError("Unknown036 task interval changed")
    ...
    if state_counts != Counter({"completed": 24, batch.UNAVAILABLE_NO_SCENE: 1}):
        raise Unknown036ContinuationAcceptanceError(
            "Unknown036 bounded task outcomes changed"
        )
    no_scene = [
        (queue_id, task)
    ...
    if (
        ...
        or no_scene[0][1]["queue_position"] != POSITION_START
        ...
        raise Unknown036ContinuationAcceptanceError(
            "Unknown036 no-scene checkpoint changed"
        )
    completed = [
    ...
    if any(
        ...
        raise Unknown036ContinuationAcceptanceError(
            "Unknown036 completed task semantics changed"
        )
    for queue_id, source_task in source_document["jobs"].items():
        ...
        if not POSITION_START <= position <= POSITION_END:
            if destination_document["jobs"][queue_id] != source_task:
                raise Unknown036ContinuationAcceptanceError(
                    "Unknown036 changed an out-of-range task"
                )
    ...
    if (
        ...
        raise Unknown036ContinuationAcceptanceError(
            "Unknown036 exact catalog-only clone delta changed"
        )
    ...
    if next_pending != (NEXT_PENDING_POSITION, NEXT_PENDING_QUEUE_ID):
        raise Unknown036ContinuationAcceptanceError(
            "Unknown036 next-pending position changed"
        )
    ...
    if delta_summary != EXPECTED_DELTA:
        raise Unknown036ContinuationAcceptanceError(
            "Unknown036 delta arithmetic changed"
        )
    delta = {
    ...
    if not (
        ...
        raise Unknown036ContinuationAcceptanceError(
            "Unknown036 acceptance chronology changed"
        )
    if source_chronology["device"] != destination_chronology["device"]:
        raise Unknown036ContinuationAcceptanceError(
            "Unknown036 source and destination are no longer on one device"
        )
...
def build_unknown036_continuation_acceptance() -> dict[str, bytes]:
...
def _closed_tree(directory: Path, expected: Mapping[str, bytes]) -> None:
    if directory.is_symlink() or not directory.is_dir():
        raise Unknown036ContinuationAcceptanceError(
            "Unknown036 acceptance wrapper is not a regular directory"
        )
    entries = {path.name: path for path in directory.iterdir()}
    if set(entries) != set(expected) or set(entries) != CLOSED_FILENAMES:
        raise Unknown036ContinuationAcceptanceError(
            "Unknown036 acceptance wrapper closed set changed"
        )
    if stat.S_IMODE(directory.stat().st_mode) != 0o555:
        raise Unknown036ContinuationAcceptanceError(
            "Unknown036 acceptance wrapper directory is not mode 0555"
        )
    for name, raw in expected.items():
        ...
        if (
            ...
            raise Unknown036ContinuationAcceptanceError(
                f"Unknown036 acceptance wrapper member changed: {name}"
            )


def _publication_paths(root: Path) -> list[Path]:
    files, directories = recovery._regular_tree(root, "Unknown036 wrapper")
...
def _assert_predeclared_stage(root: Path) -> None:
    ...
    for path in _publication_paths(root):
        ...
        if birthtime is None or birthtime > declared or metadata.st_mtime > declared:
            raise Unknown036ContinuationAcceptanceError(
                f"Unknown036 stage chronology is too late: {path}"
            )
...
def _validate_publication_chronology(root: Path) -> None:
    ...
    for path in _publication_paths(root):
        ...
        if (
            ...
            raise Unknown036ContinuationAcceptanceError(
                f"Unknown036 final publication chronology changed: {path}"
            )


def validate_unknown036_continuation_acceptance(
    ...
    if supplied != OUTPUT:
        raise Unknown036ContinuationAcceptanceError(
            "Unknown036 acceptance wrapper path changed"
        )
    expected = build_unknown036_continuation_acceptance()
    _closed_tree(supplied, expected)
    _validate_publication_chronology(supplied)
    manifest_raw = expected[MANIFEST_FILENAME]
    manifest = _json_object(manifest_raw, "Unknown036 acceptance manifest")
    ...
    if (
        ...
        raise Unknown036ContinuationAcceptanceError(
            "Unknown036 acceptance manifest semantics changed"
        )
    return manifest
...
def write_unknown036_continuation_acceptance() -> dict[str, Any]:
    """Write the frozen wrapper with future-clock and no-replace guards."""

    if OUTPUT.exists() or OUTPUT.is_symlink():
        return validate_unknown036_continuation_acceptance(OUTPUT)
    parent = OUTPUT.parent
    if parent.is_symlink() or not parent.is_dir():
        raise Unknown036ContinuationAcceptanceError(
            "Unknown036 acceptance output parent is unsafe"
        )
    ...
    if datetime.now(UTC) >= _timestamp(ACCEPTED_AT, "accepted_at"):
        raise Unknown036ContinuationAcceptanceError(
            "Unknown036 stage would postdate the declared acceptance time"
        )
    ...
    try:
        ...
        if OUTPUT.exists() or OUTPUT.is_symlink():
            raise Unknown036ContinuationAcceptanceError(
                "Unknown036 acceptance output appeared before promotion"
            )
        ...
        if (current_parent.st_dev, current_parent.st_ino) != parent_identity:
            raise Unknown036ContinuationAcceptanceError(
                "Unknown036 acceptance parent identity changed"
            )
        if (current_stage.st_dev, current_stage.st_ino) != stage_identity:
            raise Unknown036ContinuationAcceptanceError(
                "Unknown036 acceptance stage identity changed"
            )
        recovery._promote_directory_exclusive(stage, OUTPUT)
        if stage.exists() or stage.is_symlink():
            raise Unknown036ContinuationAcceptanceError(
                "Unknown036 acceptance stage remains after promotion"
            )
...
def main() -> int:
    manifest = write_unknown036_continuation_acceptance()
