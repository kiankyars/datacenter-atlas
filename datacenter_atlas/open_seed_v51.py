...
V51 derives only from the accepted frozen v49 definition and release. It
retains all 297 inherited input pins and adds four accepted official-source
records. The release is source scoped and makes no completeness, parity, or
...
PUBLICATION_LOCK = ROOT / ".open-seed-v51.lock"
...
def validate_temporal_contract(
    ...
    *,
    ...
    """Validate the fixed recorded time against inputs and the build clock.

    A historical reproduction is valid at any time on or after ``RECORDED_AT``.
    A clock before that instant is rejected because it would make the recorded
    time a future assertion.
    """
...
def publication_lock() -> Iterator[None]:
    """Hold an exclusive v51 publication lock without replacing any file."""
...
def _validate_base_definition(base: dict[str, object]) -> None:
    """Prove that accepted v49, and no other seed, is the lineage root."""
    ...
    if BASE_DEFINITION != accepted_definition or BASE_RELEASE != accepted_release:
        raise SystemExit("v51 base paths must select exactly accepted v49")
...
def selected_inputs(base: dict[str, object]) -> tuple[dict[str, str], list[Path]]:
    """Return all inherited v49 pins plus exactly four accepted additions."""
    ...
    if set(base_pins) & set(ADDITIONS):
        raise SystemExit("one or more v51 additions already occurs in v49")

    pins = dict(base_pins)
    pins.update(ADDITIONS)
    if len(pins) != 301:
        raise SystemExit(f"expected 301 v51 inputs, found {len(pins)}")
    ...
    if {path: pins[path] for path in ADDITIONS} != ADDITIONS:
        raise SystemExit("v51 addition inventory changed")
...
def build_open_seed_v51() -> dict[str, object]:
    """Build and freeze v51 once, refusing every publication collision."""
    ...
    with publication_lock():
        ...
        try:
            with tempfile.TemporaryDirectory(
                prefix="open-seed-v51-db-", dir=staging_root
            ) as temporary:
...
def main() -> int:
    """Build v51 and print the frozen release facts."""

    print(json.dumps(build_open_seed_v51(), indent=2, sort_keys=True))
    return 0
