"""Build open seed v90 as the strict seven-source append successor to v89."""
...
from . import google_official_current_build_gap_20260722 as official
...
BASE_DEFINITION = ROOT / "sources/open-seed-2026-07-21-v89.json"
BASE_RELEASE = ROOT / "releases/2026-07-21-open-seed-v89"
DEFINITION = ROOT / "sources/open-seed-2026-07-21-v90.json"
RELEASE_ID = "2026-07-21-open-seed-v90"
RELEASE = ROOT / "releases" / RELEASE_ID
PUBLICATION_LOCK = ROOT / ".open-seed-v90.lock"
AS_OF = "2026-07-21"

BASE_RECORDED_AT = "2026-07-22T00:57:44Z"
BASE_DEFINITION_PIN = (
    105_962,
...
BASE_TREE_SHA256 = "83b721b2066c3be6cbf3cf3bfb255abb8ed4427ad5ebd17da8dcc5214551fdad"
BASE_ENTITIES_PIN = (
    1_004_639,
...
OFFICIAL_RECORDED_AT = "2026-07-22T01:12:16Z"
OFFICIAL_MANIFEST_PIN = (
    1_748,
    "76335b0c2c24856ae9d3e01c46215d701ac5d38ae439fd0780c553f8b1628673",
)
OFFICIAL_MANIFEST_TREE_SHA256 = (
    "c2a4b7441ab22ca290d9b8f68fdcffe06b50fc94a1be10c4b745cbabcce96c69"
)
OFFICIAL_PHYSICAL_TREE_SHA256 = (
    "c3be59989fe098f3cc2da063d6c6475c43d1f73bd71e6eca665bd0a8abe71476"
)
...
ADDITION_PINS = {
    ADDITION_ORDER[0]: (
        8_768,
        "ec1ca923b7893a0bb7c3294bfc44995b015bea242ea17f63908fd44fbee1e48c",
    ),
    ADDITION_ORDER[1]: (
        8_783,
        "dc71b0379768b5c0e2c04c5e2e463c5f763e975dd5487f060b4183f558f8e109",
    ),
    ADDITION_ORDER[2]: (
        4_070,
        "06fb28d0cb81e0ef6fefdca725b8f9702d6339a8dc6767a5db693369ea56de31",
    ),
    ADDITION_ORDER[3]: (
        6_393,
        "1dfdd07a5d6d3b6eef98cfeda757365bd30c65a704b652ae74eb70dea801bfd3",
    ),
    ADDITION_ORDER[4]: (
        3_869,
        "b050c05327259f9a34339d08d3a300ecc48a760481be5d16e66586705beaf231",
    ),
    ADDITION_ORDER[5]: (
        3_762,
        "cbe7219232567add85c104631959281f5dcc7e0aed5c2068ae43890d70d85a5d",
    ),
    ADDITION_ORDER[6]: (
        4_134,
        "9c7ade6f256e68a22b71e4fdfdf5f666f0f9e9f753e8ebdc498d0ef8e2a24671",
    ),
}
...
FRESHNESS_README = f"""
Open seed v90 is the exact accepted v89 successor with only the seven
seed-eligible records from `source_artifacts/{official.ARTIFACT_ID}` appended
at input indices 470 through 476. The artifact manifest is
`{OFFICIAL_MANIFEST_PIN[1]}`, logical tree is
`{OFFICIAL_MANIFEST_TREE_SHA256}`, and physical tree is
`{OFFICIAL_PHYSICAL_TREE_SHA256}`.

The append creates fourteen v89-new entities: seven separately named campuses
and seven projects. Five stable keys for the two Dorchester campus/project
pairs and the Elmina campus reuse exact global-open-v3 identities.
No source coordinate or geometry is inherited. A second Skien data center,
Project Spade, Michigan City, and Morgan County remain review-only.
...
class OpenSeedV90Error(RuntimeError):
    """Raised when a v90 lineage, claim, or publication guard fails closed."""
...
def _read_json(
    ...
    if path.is_symlink() or not path.is_file() or not stat.S_ISREG(path.stat().st_mode):
        raise OpenSeedV90Error(f"expected ordinary JSON file: {path}")
    if mode is not None and stat.S_IMODE(path.stat().st_mode) != mode:
        raise OpenSeedV90Error(f"file mode differs: {path}")
    raw = path.read_bytes()
    document = json.loads(raw)
    if raw != _canonical(document, sort_keys=sort_keys):
        raise OpenSeedV90Error(f"JSON is not canonical: {path}")
    return raw, document
...
def _validate_official_artifact() -> dict[str, dict[str, Any]]:
    try:
        ...
        raise OpenSeedV90Error(f"official current-build artifact invalid: {error}") from error
    ...
    if (
        ...
        or manifest.get("curated_source_records") != 7
        or manifest.get("seed_eligible_candidates") != 7
        ...
        raise OpenSeedV90Error("official current-build artifact pin or boundary differs")
    snapshot = json.loads(
    ...
    if (
        ...
        != {
            ...
            "source_records": 7,
            "seed_eligible_candidates": 7,
            "seed_eligible_source_records": 7,
            "review_only_candidates": 4,
            "distinct_campuses_in_source_records": 7,
            "projects": 7,
            "distinct_entities_in_source_records": 14,
        ...
        raise OpenSeedV90Error("official current-build source snapshot differs")
    ...
    for relative, record in zip(ADDITION_ORDER, records, strict=True):
        ...
        if (len(raw), _sha256(raw)) != ADDITION_PINS[relative] or (
            ...
            raise OpenSeedV90Error(f"v90 source pin differs: {relative}")
    ...
    if (
        ...
        raise OpenSeedV90Error("official current-build normalized claim contract differs")
    return documents
...
def _base_paths(base: Mapping[str, Any]) -> list[Path]:
    ...
    for row in base["curated_inputs"]:
        ...
        if path.is_symlink() or not path.is_file() or v69.sha256(path) != row["sha256"]:
            raise OpenSeedV90Error(f"accepted v89 input pin differs: {row['path']}")
        paths.append(path)
    return paths
...
def selected_inputs(
    ...
    *,
    ...
    if base.get("release_id") != v89.RELEASE_ID:
        raise OpenSeedV90Error("v90 base must be exactly accepted v89")
    ...
    if (
        ...
        raise OpenSeedV90Error("accepted v89 curated inventory differs")
    documents = _validate_official_artifact()
    target = v70.parse_utc(recorded_at, label="v90 recorded_at")
    ...
    if (
        ...
        raise OpenSeedV90Error("v90 publication time precedes an input")
    ...
    if base_evidence & ADDED_EVIDENCE_KEYS:
        raise OpenSeedV90Error("v90 evidence append collides with accepted v89")
    collision = official._collision_witness(documents)
    if (
        collision.get("exact_v89_stable_key_collisions") != []
        or collision.get("exact_v89_evidence_key_collisions") != []
        ...
        raise OpenSeedV90Error("v90 Google collision resolution differs")
    ...
    if (
        selected[:470] != rows
        or [row["path"] for row in selected[470:]] != list(ADDITION_ORDER)
        or tuple(documents) != ADDITION_ORDER
        or len(selected) != 477
        or len({row["path"] for row in selected}) != 477
    ):
        raise OpenSeedV90Error("v90 did not append exactly seven ordered inputs")
    for relative, document in documents.items():
        for index, evidence in enumerate(document["evidence"]):
            ...
            if retrieved > target or retrieved > wall.astimezone(UTC):
                raise OpenSeedV90Error("v90 selected evidence is future-dated")
...
def _validate_database_contract(
    ...
    *,
    ...
    expected_counts = {
        "entities": 973,
        "entity_snapshots": 995,
        "evidence": 793,
        "lifecycle_observations": 562,
    ...
    if actual_counts != expected_counts:
        raise OpenSeedV90Error(f"v90 database counts differ: {actual_counts}")
    with tempfile.TemporaryDirectory(
        prefix="open-seed-v90-base-", dir="/private/tmp"
    ) as temporary:
        prior = v69._populate_database(
            base,
            _base_paths(base),
            Path(temporary) / "v89.sqlite",
        ...
        try:
            for table in (
                ...
                if not _table_state(prior, table) <= _table_state(connection, table):
                    raise OpenSeedV90Error(f"v90 changed a v89 database row: {table}")
            before_entities = {
            ...
            if (
                ...
                raise OpenSeedV90Error("v90 database identity delta differs")
            ...
            if (
                ...
                raise OpenSeedV90Error("v90 database evidence delta differs")
        finally:
            prior.close()
    ...
    if project_targets != expected_targets:
        raise OpenSeedV90Error("v90 Google project/campus boundaries differ")
    ...
    if (
        ...
        raise OpenSeedV90Error("v90 imported claim contract differs")
    ...
    if (
        ...
        != {
            "dorchester_county_economic_development",
            "farciennes_municipality",
            "google_company_blog",
            "lagrange_city_financial_report",
            "mida_official_news",
            "skien_municipality",
        }
    ):
        raise OpenSeedV90Error("v90 imported evidence classification differs")
    ...
    if (
        ...
        raise OpenSeedV90Error("v90 coordinate coverage changed")
...
def _augment_release(documents: Mapping[str, str]) -> dict[str, str]:
    output = v89._augment_release(documents)
    output["README.md"] = (
    ...
    *,
    ...
    if precreated:
        if output.is_symlink() or not output.is_dir() or any(output.iterdir()):
            raise OpenSeedV90Error("precreated v90 release stage must be empty")
    else:
...
def _validate_release_facts(stage: Path, *, recorded_at: str) -> None:
    if stage.is_symlink() or not stage.is_dir():
        raise OpenSeedV90Error("v90 release is not an ordinary directory")
    ...
    if len(entries) != 14 or any(
        ...
        raise OpenSeedV90Error("v90 release file inventory differs")
    ...
    if (
        ...
        raise OpenSeedV90Error("v90 source-input inventory differs")
    ...
    if set(entities) != relevant_keys:
        raise OpenSeedV90Error("v90 relevant entity export differs")
    expected_country = {
        key: site.country
        for site in official.SITES
    ...
    for key in ADDED_ENTITY_KEYS:
        ...
        if (
            ...
            raise OpenSeedV90Error(f"v90 added entity scope differs: {key}")
    ...
    if {
        ...
        raise OpenSeedV90Error("v90 exported lifecycle facts differ")
    ...
    if set(freshness) != ADDED_PROJECT_KEYS or any(
        ...
        raise OpenSeedV90Error("v90 freshness boundary differs")
    pipeline = {
        row["stable_key"]
    ...
    if pipeline != ADDED_PROJECT_KEYS:
        raise OpenSeedV90Error("v90 construction-pipeline delta differs")
    readme = (stage / "README.md").read_text(encoding="utf-8")
    for marker in (
        "Open seed v90 is the exact accepted v89 successor",
        "Project Spade, Michigan City, and Morgan County remain review-only",
        ...
        if marker not in readme:
            raise OpenSeedV90Error(f"v90 README guardrail differs: {marker}")
...
def _validate_definition(
    ...
    *,
    ...
    if set(document) != set(base) or document.get("release_id") != RELEASE_ID:
        raise OpenSeedV90Error("v90 definition identity or schema differs")
    build = document.get("build")
    if not isinstance(build, dict) or set(build) != {"as_of", "recorded_at"}:
        raise OpenSeedV90Error("v90 definition build carrier differs")
    if build["as_of"] != AS_OF:
        raise OpenSeedV90Error("v90 as_of differs")
    ...
    if not isinstance(expected_summary, dict) or set(expected_summary) != set(
        ...
        raise OpenSeedV90Error("v90 expected-summary key contract differs")
    recorded = v70.parse_utc(build["recorded_at"], label="v90 recorded_at")
    if (
        validation_wall_clock.tzinfo is None
        or recorded > validation_wall_clock.astimezone(UTC)
    ):
        raise OpenSeedV90Error("v90 recorded_at is later than validation wall clock")
    for key in (
        "epoch_capture",
        ...
        if document.get(key) != base.get(key):
            raise OpenSeedV90Error(f"v90 inherited definition field differs: {key}")
    ...
    *,
    ...
    for path in (definition, release, *release.iterdir()):
        ...
        if max(metadata.st_birthtime, metadata.st_mtime) > target.timestamp() + 1e-6:
            raise OpenSeedV90Error(f"v90 staged inode post-dates recorded_at: {path.name}")
    if require_live:
        if datetime.now(UTC) < target:
            raise OpenSeedV90Error("v90 recorded_at is not live")
        for path in (definition, release):
            if path.stat(follow_symlinks=False).st_ctime + 1e-6 < target.timestamp():
                raise OpenSeedV90Error(
                    f"v90 final root ctime predates recorded_at: {path.name}"
                )
...
def _validate_guard(guard: Mapping[str, Any]) -> None:
    ...
    if (
        v89.DEFINITION != BASE_DEFINITION
        or v89.RELEASE != BASE_RELEASE
        ...
        raise OpenSeedV90Error("accepted v89 base pin differs")
    if (
        ...
        raise OpenSeedV90Error("v90 official-source pin differs")


def validate_open_seed_v90(
    ...
    *,
    ...
    if replay_count != 2:
        raise OpenSeedV90Error("v90 requires exactly two offline replays")
    wall = validation_wall_clock or datetime.now(UTC)
    guard = _guard_state()
    ...
    if definition.get("curated_inputs") != selected:
        raise OpenSeedV90Error("v90 selected input inventory differs")
    if release_path.is_symlink() or not release_path.is_dir():
        raise OpenSeedV90Error("v90 release must be an ordinary directory")
    if require_frozen and stat.S_IMODE(release_path.stat().st_mode) != 0o555:
        raise OpenSeedV90Error("v90 release root is not frozen")
    ...
    if any(path.is_symlink() or not path.is_file() for path in release_files.values()):
        raise OpenSeedV90Error("v90 release contains a non-file")
    if require_frozen and any(
        stat.S_IMODE(path.stat().st_mode) != 0o444
        for path in release_files.values()
    ):
        raise OpenSeedV90Error("v90 release file is not frozen")
    ...
    if _sha256(manifest_raw) != definition["expected_release"].get(
        ...
        raise OpenSeedV90Error("v90 manifest hash differs")
    expected_release = {
        key: value
    ...
    if {key: value for key, value in manifest.items() if key != "files"} != expected_release:
        raise OpenSeedV90Error("v90 expected release facts differ")
    ...
    if {key: summary[key] for key in definition["expected_summary"]} != definition[
        ...
        raise OpenSeedV90Error("v90 expected summary differs")

    for replay in range(replay_count):
        with tempfile.TemporaryDirectory(
            prefix=f"open-seed-v90-replay-{replay + 1}-", dir="/private/tmp"
        ) as temporary:
            root = Path(temporary)
            ...
            if {path.name for path in replay_release.iterdir()} != set(release_files):
                raise OpenSeedV90Error("v90 replay file inventory differs")
            for filename, frozen in release_files.items():
                if (replay_release / filename).read_bytes() != frozen.read_bytes():
                    raise OpenSeedV90Error(f"v90 offline replay differs: {filename}")
    if _guard_state() != guard:
        raise OpenSeedV90Error("v90 validation mutated accepted inputs")
    return manifest
...
def _path_identity(path: Path, *, directory: bool) -> tuple[int, int]:
    ...
    if not expected:
        raise OpenSeedV90Error(f"v90 stage type differs: {path}")
...
def _assert_release_identities(
    ...
    if _path_identity(root, directory=True) != root_identity:
        raise OpenSeedV90Error("v90 release stage root identity changed")
    if _release_identities(root) != dict(members):
        raise OpenSeedV90Error("v90 release stage member identity changed")
...
def _discard_file_stage(path: Path, identity: tuple[int, int]) -> None:
    ...
    if _path_identity(path, directory=False) != identity:
        raise OpenSeedV90Error("refusing substituted v90 definition cleanup")
    path.chmod(0o600)
    path.unlink()
...
def _publication_lock() -> Iterator[None]:
    try:
        ...
        raise OpenSeedV90Error("active v90 publication lock exists") from error
    ...
    try:
        ...
        try:
            ...
            if not stat.S_ISREG(current.st_mode) or (
                ...
                raise OpenSeedV90Error("refusing substituted v90 lock cleanup")
...
def _require_absent(label: str) -> None:
    if DEFINITION.exists() or DEFINITION.is_symlink():
        raise OpenSeedV90Error(f"{label} v90 definition collision")
    if RELEASE.exists() or RELEASE.is_symlink():
        raise OpenSeedV90Error(f"{label} v90 release collision")
...
def _rollback_release(release_identity: tuple[int, int], release_stage: Path) -> None:
    if _path_identity(RELEASE, directory=True) != release_identity:
        raise OpenSeedV90Error("refusing rollback of substituted v90 release")
    if release_stage.exists() or release_stage.is_symlink():
        raise OpenSeedV90Error("v90 release rollback stage is occupied")
...
def _rollback_definition(
    ...
    if _path_identity(DEFINITION, directory=False) != definition_identity:
        raise OpenSeedV90Error("refusing rollback of substituted v90 definition")
    if definition_stage.exists() or definition_stage.is_symlink():
        raise OpenSeedV90Error("v90 definition rollback stage is occupied")
    v69.promote_noreplace(DEFINITION, definition_stage)


def build_open_seed_v90(recorded_at: str | None = None) -> dict[str, Any]:
    """Build and atomically publish the seven-source v89 successor."""

    if (DEFINITION.exists() or DEFINITION.is_symlink()) and (
        RELEASE.exists() or RELEASE.is_symlink()
    ):
        manifest = validate_open_seed_v90()
        return {
    ...
    if (
        ...
        raise OpenSeedV90Error("partial v90 final-path collision")

    guard = _guard_state()
    _validate_guard(guard)
    target = (
        v70.parse_utc(recorded_at, label="v90 recorded_at")
        if recorded_at is not None
        else datetime.now(UTC).replace(microsecond=0) + timedelta(seconds=45)
    )
    if datetime.now(UTC) >= target:
        raise OpenSeedV90Error("v90 recorded_at must be future before staging")
    ...
    with _publication_lock():
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(
                prefix="open-seed-v90-db-", dir="/private/tmp"
            ...
            if _path_identity(definition_stage, directory=False) != definition_identity:
                raise OpenSeedV90Error("v90 definition stage identity changed")
            ...
            if (
                ...
                raise OpenSeedV90Error("v90 private stage changed while waiting")
            _validate_publication_times(
                definition_stage,
                release_stage,
                recorded_at=recorded_at,
            ...
            try:
                ...
                try:
                    ...
                    error.add_note(f"v90 release rollback failed: {rollback_error}")
                raise
            try:
                manifest = validate_open_seed_v90(DEFINITION, RELEASE)
            except BaseException as error:
                rollback_errors = []
                try:
                    ...
                    rollback_errors.append(
                        f"v90 definition rollback failed: {rollback_error}"
                    )
                try:
                    _rollback_release(release_identity, release_stage)
                    published_release = False
                    ...
                    rollback_errors.append(
                        f"v90 release rollback failed: {rollback_error}"
                    )
    ...
    if _guard_state() != guard:
        raise OpenSeedV90Error("v90 build mutated accepted inputs")
    return {
        "definition": str(DEFINITION),
        "definition_sha256": v69.sha256(DEFINITION),
        "manifest_sha256": v69.sha256(RELEASE / "manifest.json"),
...
def main() -> int:
    print(json.dumps(build_open_seed_v90(), indent=2, sort_keys=True))
    return 0
