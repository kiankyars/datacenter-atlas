"""Coverage-audit carrier accepting federation-v4 geometry non-inference.
...
v4, which accepts the explicit geometry non-inference field introduced by the
open-seed v86 child.
"""
...
def _successor_replacement(source: str, old: str, new: str, count: int) -> str:
    ...
    if actual != count:
        raise ImportError(
            "coverage_audit_v4 accepted-v30 boundary changed: "
...
if _carrier_hashlib.sha256(_raw).hexdigest() != _BASE_SOURCE_SHA256:
    raise ImportError("coverage_audit_v2 changed; refusing v31 carrier load")
_source = _raw.decode("utf-8")
for _old, _new, _count in (
    ("federated_release_v2", "federated_release_v4", 1),
    ("federation_v2", "federation_v4", 4),
    (
        "federated v2 input validation failed",
        "federated v4 input validation failed",
        1,
    ),
):
