"""Strict validator for the frozen open-seed v61 successor.

V61 remains on the 2026-07-20 local research day. It is the exact frozen-v60
successor that adds only the Batelco/Qareeb Beyon Data Oasis record. Applied
Digital PF1 Building 2 Phase 1 and next-local-day Pure AMS01 remain outside
the release.
...
from . import open_seed_release_v5 as v5
...
V60_DEFINITION = "sources/open-seed-2026-07-20-v60.json"
V60_RELEASE = "releases/2026-07-20-open-seed-v60"
V60_DEFINITION_SHA256 = (
...
V60_TREE_SHA256 = "1d58652f691b8030717b7af6c61d2717b2b3427520f7d1b402b75b7670beb138"

RELEASE_ID = "2026-07-20-open-seed-v61"
AS_OF = "2026-07-20"
...
ADDITION_PINS = {
    "sources/curated-official-2026-07-20-batelco-qareeb-beyon-data-oasis.json": (
        "80799190bd0e57ce9274a4f480a0108a2c65d907a0edcdd2a4f27e4ec04b3a5a"
    )
}
...
OUT_OF_SCOPE_EXCLUSIONS = frozenset(
    {
...
class OpenSeedReleaseV6Error(ValueError):
    """Raised when the v61 definition, inputs, or release differ."""


def freshness_contract() -> dict[str, Any]:
    """Return the exact v61 last-observed and calendar-boundary contract."""

    return v5.freshness_contract()


def validate_frozen_v60(project_root: Path) -> dict[str, Any]:
    """Pin and fully validate the sole permitted v61 base."""

    definition = project_root / V60_DEFINITION
    release = project_root / V60_RELEASE
    if v2._sha256(v2._ordinary_file(definition, "v60 definition")) != (
        V60_DEFINITION_SHA256
    ):
        raise OpenSeedReleaseV6Error("frozen v60 definition hash differs")
    if v2._sha256(v2._ordinary_file(release / "manifest.json", "v60 manifest")) != (
        V60_MANIFEST_SHA256
    ):
        raise OpenSeedReleaseV6Error("frozen v60 manifest hash differs")
    if v2._tree_digest(release) != V60_TREE_SHA256:
        raise OpenSeedReleaseV6Error("frozen v60 release tree differs")
    try:
        return v5.validate_open_seed_release_v5(definition, release)
    except ValueError as error:
        raise OpenSeedReleaseV6Error("frozen v60 validation failed") from error
...
def _definition(path: str | Path) -> tuple[dict[str, Any], Path, Path]:
    ...
    if set(document) != expected_keys or document["schema_version"] != 1:
        raise OpenSeedReleaseV6Error("definition schema is invalid")
    if document["publication_contract_version"] != 4:
        raise OpenSeedReleaseV6Error("publication contract version must remain 4")
    if definition_path.parent.name != "sources":
        raise OpenSeedReleaseV6Error("definition must reside in the sources directory")
    project_root = definition_path.parent.parent
    if document["release_id"] != RELEASE_ID:
        raise OpenSeedReleaseV6Error("release_id must identify frozen v61")
    if document["scope"] != {
        ...
        raise OpenSeedReleaseV6Error("definition scope guardrails are invalid")
    if document["build"] != {"as_of": AS_OF, "recorded_at": RECORDED_AT}:
        raise OpenSeedReleaseV6Error("v61 build timestamp contract differs")
    if document["freshness_contract"] != freshness_contract():
        raise OpenSeedReleaseV6Error("v61 freshness contract differs")
    return document, definition_path, project_root


def _expected_v61_rows(project_root: Path) -> list[dict[str, str]]:
    raw = v2._ordinary_file(project_root / V60_DEFINITION, "v60 definition")
    base = v2._json_object(raw, "v60 definition", canonical=True)
    rows = base.get("curated_inputs")
    if not isinstance(rows, list) or len(rows) != 347:
        raise OpenSeedReleaseV6Error("v60 curated inventory differs")
    selected: dict[str, str] = {}
    for row in rows:
        if not isinstance(row, dict) or set(row) != {"path", "sha256"}:
            raise OpenSeedReleaseV6Error("v60 curated row is invalid")
        path, digest = row["path"], row["sha256"]
        if not isinstance(path, str) or not isinstance(digest, str) or path in selected:
            raise OpenSeedReleaseV6Error("v60 curated inventory is invalid")
        selected[path] = digest
    for path, digest in ADDITION_PINS.items():
        if path in selected:
            raise OpenSeedReleaseV6Error("a v61 addition already occurs in v60")
        if v2._sha256(v2._ordinary_file(project_root / path, path)) != digest:
            raise OpenSeedReleaseV6Error(f"accepted v61 source hash differs: {path}")
        selected[path] = digest
    excluded = STALE_EXCLUSIONS | PENDING_NEXT_DAY_EXCLUSIONS | OUT_OF_SCOPE_EXCLUSIONS
    if excluded & set(selected):
        raise OpenSeedReleaseV6Error("an excluded stale or next-day source was selected")
    if len(selected) != 348:
        raise OpenSeedReleaseV6Error(
            f"expected 348 unique v61 inputs, found {len(selected)}"
        )
...
def _validate_inputs(
    ...
    base = v2._json_object(
        v2._ordinary_file(project_root / V60_DEFINITION, "v60 definition"),
        "v60 definition",
        canonical=True,
    )
    epoch = definition["epoch_capture"]
    if epoch != base["epoch_capture"]:
        raise OpenSeedReleaseV6Error("v61 must preserve the frozen v60 Epoch capture")
    archive = v2._repo_path(project_root, epoch["archive"], "Epoch archive")
    map_path = v2._repo_path(project_root, epoch["map"], "Epoch map")
    if definition["curated_inputs"] != _expected_v61_rows(project_root):
        raise OpenSeedReleaseV6Error("curated inputs are not the exact v61 successor")
    paths = [row["path"] for row in definition["curated_inputs"]]
    if paths != sorted(paths) or len(paths) != len(set(paths)):
        raise OpenSeedReleaseV6Error("curated paths must be sorted and unique")
    ...
    for record in definition["curated_inputs"]:
        ...
        if v2._sha256(raw) != record["sha256"]:
            raise OpenSeedReleaseV6Error(f"curated input changed: {record['path']}")
        source = v2._json_object(raw, f"curated input {record['path']}", canonical=False)
        version = source.get("schema_version")
        if version not in schema_counts:
            raise OpenSeedReleaseV6Error(f"unsupported curated schema: {version!r}")
        schema_counts[version] += 1
        timestamps.extend(
            v2._evidence_timestamps(source, cutoff=cutoff, label=record["path"])
        )
        parsed.append((path, source))
    if schema_counts != {"1.0": 316, "1.1": 32}:
        raise OpenSeedReleaseV6Error(f"v61 schema inventory differs: {schema_counts}")
...
def _rebuild_documents(
    ...
    from .open_seed_v61 import augment_release_documents

    with tempfile.TemporaryDirectory(prefix="open-seed-v61-offline-", dir="/private/tmp") as temporary:
        ...
        try:
            ...
            if json.loads(json.dumps(asdict(epoch_result))) != definition["expected_epoch_result"]:
                raise OpenSeedReleaseV6Error("Epoch import result or warning set differs")
...
def validate_open_seed_release_v6(
    definition_path: str | Path,
    release_directory: str | Path,
    *,
    require_frozen: bool = True,
) -> dict[str, Any]:
    """Validate v60 lineage and reconstruct every v61 output byte twice."""

    definition, _, project_root = _definition(definition_path)
    validate_frozen_v60(project_root)
    archive, map_path, curated, timestamps = _validate_inputs(definition, project_root)
    if not timestamps:
        raise OpenSeedReleaseV6Error("retrieval inventory must not be empty")
    ...
    if not release.is_dir() or (
        ...
        raise OpenSeedReleaseV6Error("release directory identity differs")
    entries = list(release.iterdir())
    if any(path.is_symlink() or not path.is_file() for path in entries):
        raise OpenSeedReleaseV6Error("release may contain only ordinary files")
    ...
    if not isinstance(expected_release, dict) or "manifest_sha256" not in expected_release:
        raise OpenSeedReleaseV6Error("expected_release definition is invalid")
    if v2._sha256(manifest_raw) != expected_release["manifest_sha256"]:
        raise OpenSeedReleaseV6Error("release manifest hash differs")
    for key, value in expected_release.items():
        if key != "manifest_sha256" and manifest.get(key) != value:
            raise OpenSeedReleaseV6Error(f"release manifest fact differs: {key}")
    files = manifest.get("files")
    if not isinstance(files, dict):
        raise OpenSeedReleaseV6Error("release manifest file inventory is invalid")
    expected_files = set(files) | {"manifest.json"}
    if {path.name for path in entries} != expected_files:
        raise OpenSeedReleaseV6Error("release file set differs from its manifest")
    for filename, record in files.items():
        ...
        if not isinstance(record, dict) or record != {
            ...
            raise OpenSeedReleaseV6Error(f"release file changed: {filename}")
    ...
    if not isinstance(expected_summary, dict) or any(
        ...
        raise OpenSeedReleaseV6Error("release summary facts differ")
    ...
    if first != second:
        raise OpenSeedReleaseV6Error("two offline reconstructions differ")
    if set(first) != expected_files:
        raise OpenSeedReleaseV6Error("offline reconstruction file set differs")
    ...
    if require_frozen:
        if stat.S_IMODE(release.stat().st_mode) != FROZEN_DIRECTORY_MODE:
            raise OpenSeedReleaseV6Error("release root must have mode 0555")
        wrong_modes = [
            path.name
            for path in entries
