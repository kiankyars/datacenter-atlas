"""Build and validate the publication-mechanics-v4 mosaic preparation release.

The scientific derivation remains pinned to the byte-preserved v2 definition.
V4 succeeds rejected v3 only for publication mechanics. Failure recovery uses
atomic no-clobber renames and deliberately performs no unlink or rmdir. It also
keeps an exact-path ledger for every name that can retain state, including a
generated quarantine name whose identity cannot be proved after a rename.
"""
...
DEFINITION_FORMAT = "datacenter-atlas-satellite-mosaic-v3-preparation-definition-v4"
RELEASE_FORMAT = "datacenter-atlas-satellite-mosaic-v3-preparation-v4"
SCHEMA_VERSION = 4
PUBLICATION_MECHANICS_VERSION = 4
...
REJECTED_PREDECESSOR_PATH = (
    "sources/satellite-mosaic-preparation-2026-07-19-algorithm-v2-blocked-v3.json"
)
BUILDER_PATHS = {
    "cli": "scripts/build_satellite_mosaic_preparation_v4.py",
    "legacy_scientific_module": "datacenter_atlas/satellite_mosaic_preparation.py",
    "module": "datacenter_atlas/satellite_mosaic_preparation_v4.py",
    "rejected_v3_module": "datacenter_atlas/satellite_mosaic_preparation_v3.py",
}
PUBLICATION_CONTRACT = {
    "all_namespace_mutations_parent_fsynced": True,
    "all_possible_leftover_exact_paths_reported": True,
    ...
    "generated_quarantine_path_reported_if_unverified": True,
    ...
    "quarantine_ownership_claim_requires_identity": True,
...
def _read_definition(
    ...
    definition = v2._read_json(definition_path, "v4 definition", canonical=True)
    if v2._mode(definition_path) != "0444":
        raise SatelliteMosaicPreparationError("v4 definition mode must be 0444")
    if set(definition) != {
        "base_definition",
        "claim_constraints",
        "expected_partition",
        ...
        raise SatelliteMosaicPreparationError("v4 definition schema drift")
    if (
        definition["format"] != DEFINITION_FORMAT
        or definition["schema_version"] != SCHEMA_VERSION
    ):
        raise SatelliteMosaicPreparationError("v4 definition format mismatch")
    ...
    if output_dir is not None:
        ...
        if v2._is_relative(output, definition_path) or v2._is_relative(
            ...
            raise SatelliteMosaicPreparationError("output overlaps v4 definition")
...
def _builder_pins(root: Path) -> dict[str, Any]:
    ...
    for name, relative in sorted(BUILDER_PATHS.items()):
        path = v2._inside(root, relative, f"v4 builder {name}")
        if not path.is_file() or path.is_symlink():
            raise SatelliteMosaicPreparationError(f"v4 builder {name} is invalid")
...
def _readme(definition: Mapping[str, Any]) -> bytes:
    return (
        ...
        "Version 4 supersedes rejected versions 1 through 3 only for publication "
        "mechanics and remains a candidate pending a fresh independent audit. Failure "
        "recovery uses descriptor-relative probes and atomic no-clobber renames. It "
        "never calls unlink or rmdir. Every possible retained staging, target, or "
        "generated quarantine pathname is reported exactly; an unprobeable path is "
        "labelled possible with unverified ownership. Every successful namespace "
        "mutation is followed by a parent-directory fsync, and probe, rollback, "
        "recovery, and sync errors are appended to the primary failure.\n\n"
...
def _display_path(parent_path: Path, name: str) -> str:
    return os.fspath(parent_path / name)
...
def _restore_unexpected_move(
    *,
    ...
    original_known, original = _probe_at(
        parent_fd, original_name, f"{label} original {original_name}", errors
    )
    moved_known, moved = _probe_at(
        parent_fd, moved_name, f"{label} moved {moved_name}", errors
    )
...
def _move_owned_entry(
    *,
    ...
    known, moved = _probe_at(
        parent_fd,
        destination_name,
        f"{label} destination {destination_name}",
        errors,
    )
    ...
    if known and moved is not None:
        errors.append(
            f"{label} moved an entry with unexpected identity "
            f"{_identity_text(_identity(moved))}; it will be restored or preserved, "
            "never deleted"
        )
    else:
...
def _report_possible_leftovers(
    *,
    parent_path: Path,
    parent_fd: int,
    possible_names: Mapping[str, str],
    tree_identity: tuple[int, int] | None,
    errors: list[str],
) -> None:
    """Report every observed or unprobeable exact path without overstating ownership."""

    for name, provenance in possible_names.items():
        display = _display_path(parent_path, name)
        known, value = _probe_at(
            parent_fd, name, f"final leftover check {display}", errors
        )
        if not known:
            errors.append(
                "possible leftover path (unverified ownership; descriptor-relative "
                f"state unavailable; {provenance}): {display}"
            )
        elif value is not None:
            if tree_identity is not None and _identity(value) == tree_identity:
                ownership = "verified writer-created identity"
            else:
                ownership = "observed entry; not verified as writer-created"
            errors.append(
                f"leftover path observed ({ownership}; {provenance}; "
                f"{_identity_text(_identity(value))}): {display}"
            )
...
def _recover_publication_failure(
    *,
    ...
    if staging_name is not None:
        possible_names[staging_name] = "writer-generated staging name"
        _sync_after_mutation(
            parent_fd, "staging creation recovery parent fsync", errors
        )
    possible_names[target_name] = "requested publication target"
    ...
    names = list(possible_names)
    states: dict[str, tuple[bool, os.stat_result | None]] = {
        name: _probe_at(
            parent_fd,
            name,
            f"recovery {_display_path(parent_path, name)}",
            errors,
        )
        for name in names
    }
    if tree_identity is None:
        errors.append(
            "writer-created tree identity is unavailable; no recovery mutation attempted"
        )
        ...
        if len(owned_names) > 1:
            ...
            if owned_name == target_name and staging_name is not None:
                ...
                if staging_known and staging_value is None:
                    ...
                    errors.append(
                        "rollback destination is occupied; rollback not attempted"
                    )

            known, current = _probe_at(
                parent_fd,
                owned_name,
                f"pre-quarantine owned entry {owned_name}",
                errors,
            )
            if not known or current is None or _identity(current) != tree_identity:
                errors.append(
                    "owned entry changed before quarantine; no deletion attempted"
                )
            else:
                try:
                    ...
                    errors.append(
                        "recovery quarantine name generation failed: "
                        f"{_error_text(error)}"
                    )
                else:
                    quarantine = f".{target_name}.recovery-{token}"
                    possible_names[quarantine] = "generated recovery-quarantine name"
                    quarantine_known, quarantine_value = _probe_at(
                        parent_fd,
                        quarantine,
                        f"recovery quarantine {_display_path(parent_path, quarantine)}",
                        errors,
                    )
                    if not quarantine_known or quarantine_value is not None:
                        ...
                        errors.append(
                            "writer-created tree identity verified and preserved without "
                            f"deletion at {_display_path(parent_path, quarantine)}"
                        )

    _report_possible_leftovers(
...
def validate_satellite_mosaic_preparation_v4(
    ...
    try:
        ...
        for name in RELEASE_FILES:
            if actual[name] != expected[name]:
                raise SatelliteMosaicPreparationError(
                    f"{name} differs from deterministic v4 reproduction"
                )
        parsed = v2._strict_json(actual["manifest.json"], "v4 release manifest")
        if not isinstance(parsed, dict) or actual["manifest.json"] != v2.canonical_json(
            parsed
        ):
            raise SatelliteMosaicPreparationError(
                "v4 release manifest is not canonical JSON"
            )
        if parsed != manifest:
            raise SatelliteMosaicPreparationError("v4 manifest semantic mismatch")
...
def write_satellite_mosaic_preparation_v4(
    output_dir: Path, *, definition_path: Path
) -> dict[str, Any]:
    root = v2._project_root(definition_path)
    ...
    try:
        try:
            ...
            for _ in range(100):
                ...
                # Record the exact name before mkdir: a wrapper or asynchronous
                # failure can create the entry and then raise before returning.
            ...
            validated = validate_satellite_mosaic_preparation_v4(
                staging_path, definition_path=definition_path
            )
            if validated != manifest:
                raise SatelliteMosaicPreparationError(
                    "frozen staging validation returned a differing v4 manifest"
                )
