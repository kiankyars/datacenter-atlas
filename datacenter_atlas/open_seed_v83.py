"""Build the strict eight-source append-only successor to open seed v82."""
...
RELEASE_ID = "2026-07-21-open-seed-v83"
RELEASE = ROOT / "releases" / RELEASE_ID
PUBLICATION_LOCK = ROOT / ".open-seed-v83.lock"
AS_OF = "2026-07-21"

BASE_RECORDED_AT = "2026-07-21T17:29:03Z"
BASE_DEFINITION_PIN = (
    97_043,
...
BASE_TREE_SHA256 = "7948ab06c0018827de799170d6bd85089902e188674b4a20e7040c48efd21da0"
BASE_ENTITIES_PIN = (
    945_021,
...
OFFICIAL_ARTIFACT = (
    ROOT
    / "source_artifacts/global-official-builds-oceania-gap-2026-07-21-v1"
)
OFFICIAL_RECORDED_AT = "2026-07-21T17:22:11Z"
...
OFFICIAL_SNAPSHOT_PIN = (
    8_725,
    "a3528e89f90965af5a5a1081ed5cedd6a483b93769aa2d3df31dee0a27007f6b",
)
OFFICIAL_ASSESSMENT_PIN = (
    10_999,
    "2299fd31b4e78de9cec90d26530c5773d0187e08d82adce0d1eb4ba2bb506c2e",
)
OFFICIAL_RIGHTS_PIN = (
    1_192,
    "7bbbcc4eab7f0f87ce834e01a41c663a3e6b3136f6a6ebcf98be520eb3b63173",
)
...
REVIEW_ONLY_CANDIDATE_IDS = frozenset(
    {
        ...
        "bounded-official-sweep-french-polynesia",
    }
)
...
ADDED_EVIDENCE_KEYS = frozenset(
    {
        "airtrunk-syd3-cooling-delivery-captured-2026-07-21",
        "airtrunk-syd3-current-facility-page-captured-2026-07-21",
        "airtrunk-syd3-original-design-announcement-captured-2026-07-21",
        "cdc-eastern-creek-ec5-current-status-captured-2026-07-21",
        "cdc-eastern-creek-ec5-nearing-operational-captured-2026-07-21",
        "cdc-eastern-creek-ec6-current-status-captured-2026-07-21",
        "cdc-eastern-creek-ec6-nearing-operational-captured-2026-07-21",
        "cdc-marsden-park-current-status-captured-2026-07-21",
        "cdc-laverton-current-status-captured-2026-07-21",
        "cdc-brooklyn-remaining-facilities-current-status-captured-2026-07-21",
        "cdc-maddington-structural-shell-captured-2026-07-21",
        "cdc-maddington-heavy-precast-installation-captured-2026-07-21",
        "cdc-maddington-current-campus-page-captured-2026-07-21",
        "gta-gu3-april-opening-announcement-captured-2026-07-21",
        "gta-gu3-current-about-identity-captured-2026-07-21",
        "gta-gu3-alupang-running-on-genset-captured-2026-07-21",
        "gta-alupang-facility-design-captured-2026-07-21",
    }
)
ADDED_SOURCE_FAMILIES = frozenset(
    {
        "airtrunk_official_updates",
        "airtrunk_current_location_pages",
        "airtrunk_company_news",
        "cdc_current_location_pages",
        "infratil_market_announcements",
        "cica_official_updates",
        "wa_universal_project_portfolio",
        "gta_official_updates",
        "gta_current_newsroom_about_sections",
        "guam_joint_information_center_releases",
        "gta_company_news",
    }
)
...
ADDED_CANDIDATE_DECISIONS = {
    **{
        ...
        if key != "curated:gta-gu3-alupang-data-center:initial-build"
    },
    "curated:gta-gu3-alupang-data-center:initial-build":
...
FRESHNESS_README = f"""
Open seed v83 is the exact accepted v82 successor with only the eight
seed-eligible source records from
`source_artifacts/global-official-builds-oceania-gap-2026-07-21-v1`
appended in frozen artifact order. It preserves all 433 v82 inputs, then
appends AirTrunk SYD3, CDC EC5, CDC EC6, Marsden Park, Laverton, Brooklyn,
Maddington, and GTA GU3 for 441 inputs. EC5 and EC6 share one campus identity.
The frozen official-source manifest is `{OFFICIAL_MANIFEST_PIN[1]}` and its
physical tree is `{OFFICIAL_PHYSICAL_TREE_SHA256}`.

All 17 other carrier candidates remain review-only and contribute no v83 input
or claim. AirTrunk's conflicting 320+ MW IT-load and 400+ MW total-capacity
labels, plus CDC's campus, regional, and untyped figures, remain narrative and
are never normalized. GU3 contributes only 4 MW gross facility capacity at
design stage and generic colocation; no workload is inferred. Its dynamic About
text remains retrieval-date identity corroboration, while the Guam JIC record
anchors the dated operational observation. No coordinate, PUE, consumption,
annual-energy, current-load, or current-status claim is inferred.
...
class OpenSeedV83Error(RuntimeError):
    """Raised when a v83 lineage, claim, or publication guard fails closed."""
...
def _read_json(
    ...
    if path.is_symlink() or not path.is_file() or not stat.S_ISREG(path.stat().st_mode):
        raise OpenSeedV83Error(f"expected ordinary JSON file: {path}")
    if mode is not None and stat.S_IMODE(path.stat().st_mode) != mode:
        raise OpenSeedV83Error(f"file mode differs: {path}")
    raw = path.read_bytes()
    document = json.loads(raw)
    if raw != _canonical(document, sort_keys=sort_keys):
        raise OpenSeedV83Error(f"JSON is not canonical: {path}")
    return raw, document
...
def _validate_official_artifact() -> dict[str, Any]:
    ...
    if (
        ...
        raise OpenSeedV83Error("official Oceania-gap artifact is not exact and frozen")
    if any(
        path.is_symlink()
        or stat.S_IMODE(path.stat().st_mode) != (0o555 if path.is_dir() else 0o444)
        for path in artifact.rglob("*")
    ):
        raise OpenSeedV83Error("official artifact member mode differs")
    ...
    if (len(manifest_raw), _sha256(manifest_raw)) != OFFICIAL_MANIFEST_PIN:
        raise OpenSeedV83Error("official artifact manifest pin differs")
    if (
        manifest.get("recorded_at") != OFFICIAL_RECORDED_AT
        or manifest.get("tree_sha256") != OFFICIAL_MANIFEST_TREE_SHA256
        or manifest.get("candidate_assessments") != 25
        or manifest.get("curated_source_records") != 8
        or manifest.get("seed_eligible_source_records") != 8
        or manifest.get("review_only_candidates") != 17
        or manifest.get("successful_http_200_body_captures") != 20
        ...
        raise OpenSeedV83Error("official artifact boundary differs")
    listed = manifest.get("files")
    if not isinstance(listed, list):
        raise OpenSeedV83Error("official artifact inventory differs")
    actual = {
    ...
    if actual != expected or manifest.get("closed_file_set") != sorted(expected):
        raise OpenSeedV83Error("official artifact is not a closed file set")
    for row in listed:
        raw = (artifact / row["path"]).read_bytes()
        if (len(raw), _sha256(raw)) != (row["bytes"], row["sha256"]):
            raise OpenSeedV83Error(f"official artifact file pin differs: {row['path']}")
    if _sha256(_canonical(listed)) != OFFICIAL_MANIFEST_TREE_SHA256:
        raise OpenSeedV83Error("official artifact manifest tree differs")
    if (artifact / "manifest.sha256").read_text() != (
        f"{OFFICIAL_MANIFEST_PIN[1]}  manifest.json\n"
    ):
        raise OpenSeedV83Error("official artifact sidecar differs")
    ...
    if (
        ...
        raise OpenSeedV83Error("official artifact publication time differs")
    ...
    if (len(snapshot_raw), _sha256(snapshot_raw)) != OFFICIAL_SNAPSHOT_PIN:
        raise OpenSeedV83Error("official source snapshot pin differs")
    if (len(assessment_raw), _sha256(assessment_raw)) != OFFICIAL_ASSESSMENT_PIN:
        raise OpenSeedV83Error("official assessment pin differs")
    if (len(rights_raw), _sha256(rights_raw)) != OFFICIAL_RIGHTS_PIN:
        raise OpenSeedV83Error("official rights pin differs")
    if (
        ...
        raise OpenSeedV83Error("official source-rights boundary differs")
...
def _validate_additions(
    ...
    if tuple(ADDITION_PINS) != ADDITION_ORDER or len(ADDITION_PINS) != 8:
        raise OpenSeedV83Error("v83 requires exactly eight ordered additions")
    carrier = _validate_official_artifact()
    target = v70.parse_utc(recorded_at, label="v83 recorded_at")
    wall = validation_wall_clock or datetime.now(UTC)
    if wall.tzinfo is None or target > wall.astimezone(UTC):
        raise OpenSeedV83Error("v83 recorded_at is later than validation wall clock")
    if v70.parse_utc(OFFICIAL_RECORDED_AT, label="official recorded_at") > target:
        raise OpenSeedV83Error("official artifact post-dates v83")
    ...
    if (
        ...
        or len(records) != 8
        or snapshot.get("totals", {}).get("candidate_assessments") != 25
        or snapshot.get("totals", {}).get("source_records") != 8
        or snapshot.get("totals", {}).get("seed_eligible_source_records") != 8
        or snapshot.get("totals", {}).get("review_only_candidates") != 17
        or snapshot.get("totals", {}).get("distinct_campuses") != 7
        or snapshot.get("totals", {}).get("projects") != 8
        or snapshot.get("totals", {}).get("entity_snapshots") != 16
        or snapshot.get("totals", {}).get("unique_evidence_records") != 17
    ...
    if {
        ...
        raise OpenSeedV83Error("official seed-eligible source inventory differs")
    excluded_records = {
    ...
    if (
        ...
        raise OpenSeedV83Error("Oceania source inventory differs")
    ...
    if (
        assessment.get("candidate_count") != 25
        or assessment.get("seed_eligible_count") != 8
        ...
        or sum(row.get("seed_eligible") is True for row in candidates.values()) != 8
    ):
        raise OpenSeedV83Error("Oceania disposition boundary differs")
    ...
    for relative, expected_pin in EXCLUDED_SOURCE_PINS.items():
        ...
        if (len(raw), _sha256(raw)) != expected_pin:
            raise OpenSeedV83Error(f"v83 excluded source byte pin differs: {relative}")
        if max(metadata.st_birthtime, metadata.st_mtime) > target.timestamp() + 1e-6:
            raise OpenSeedV83Error(f"v83 excluded source post-dates publication: {relative}")
        if (
            ...
            raise OpenSeedV83Error("v83 review-only source gained a normalized claim")
    for relative in ADDITION_ORDER:
        ...
        if (len(raw), _sha256(raw)) != expected_pin:
            raise OpenSeedV83Error(f"v83 source byte pin differs: {relative}")
        if max(metadata.st_birthtime, metadata.st_mtime) > target.timestamp() + 1e-6:
            raise OpenSeedV83Error(f"v83 source post-dates publication: {relative}")
        if document.get("schema_version") != "1.1":
            raise OpenSeedV83Error(f"v83 source schema differs: {relative}")
        documents[relative] = document
        for row in document.get("evidence", []):
            key = row.get("key")
            if key in evidence:
                raise OpenSeedV83Error(f"v83 duplicate evidence key: {key}")
            ...
            if retrieved > target or retrieved > wall.astimezone(UTC):
                raise OpenSeedV83Error("v83 source evidence is future-dated")
            ...
            if (
                ...
                raise OpenSeedV83Error(f"v83 evidence lineage differs: {key}")
            evidence[key] = row
        for entity_name in ("campus", "project"):
            row = document.get(entity_name)
            if not isinstance(row, dict):
                raise OpenSeedV83Error(f"v83 entity inventory differs: {relative}")
            if set(row) != {
                "address",
                "as_of_date",
                "confidence",
                ...
                raise OpenSeedV83Error(
                    f"v83 entity classification or role boundary differs: {relative}"
                )
            ...
            if stable_key in entities:
                ...
                if (
                    stable_key != "curated:cdc-eastern-creek-campus"
                    or entity_name != "campus"
                    or entity_occurrences[stable_key] != 1
                    or {
                        key: entities[stable_key][key]
                        for key in shared_identity_fields
                    }
                    != {key: row[key] for key in shared_identity_fields}
                ):
                    raise OpenSeedV83Error(
                        f"v83 entity inventory differs: {relative}"
                    )
            expected_point = COORDINATE_CONTRACT.get(stable_key)
            if expected_point is None:
                if row.get("coordinates") is not None or row.get("geometry") is not None:
                    raise OpenSeedV83Error(
                        f"v83 source gained unsupported coordinates: {relative}"
                    )
            elif (
                row.get("coordinates")
                ...
                raise OpenSeedV83Error(f"v83 official coordinate differs: {relative}")
        ...
        for row in document.get("capacities", []):
            ...
            if not float(row["low"]) == float(row["base"]) == float(row["high"]):
                raise OpenSeedV83Error("v83 source capacity range differs")
    ...
    if (
        ...
        or entity_occurrences
        != Counter(
            {
                key: 2 if key == "curated:cdc-eastern-creek-campus" else 1
        ...
        raise OpenSeedV83Error("v83 identity or evidence contract differs")
    if (
        ...
        raise OpenSeedV83Error(
            "v83 lifecycle, capacity, model, or workload contract differs"
        )
    airtrunk = documents[ADDITION_ORDER[0]]
    ec5 = documents[ADDITION_ORDER[1]]
    ec6 = documents[ADDITION_ORDER[2]]
    marsden = documents[ADDITION_ORDER[3]]
    laverton = documents[ADDITION_ORDER[4]]
    brooklyn = documents[ADDITION_ORDER[5]]
    maddington = documents[ADDITION_ORDER[6]]
    gu3 = documents[ADDITION_ORDER[7]]
    if (
        ...
        or any(
            document["capacities"] or document["operating_models"]
            for document in documents.values()
            if document is not gu3
        )
        or airtrunk["evidence"][1]["metadata"].get("capacity_label_as_reported")
        != "400+MW total capacity"
        or airtrunk["evidence"][2]["metadata"].get("capacity_label_as_reported")
        != "320+ MW of IT load"
        or airtrunk["evidence"][1]["metadata"].get(
            "operating_type_as_reported"
        )
        != "hyperscale campus"
        or ec5["campus"]["stable_key"] != ec6["campus"]["stable_key"]
        or ec5["evidence"][0]["metadata"].get(
            "campus_capacity_label_as_reported"
        )
        != "over 200 MW"
        or ec6["evidence"][0]["metadata"].get(
            "campus_capacity_label_as_reported"
        )
        != "over 200 MW"
        or marsden["evidence"][0]["metadata"].get("capacity_label_as_reported")
        != "over 504 MW planned; scalable to 1 GW"
        or laverton["evidence"][0]["metadata"].get("capacity_label_as_reported")
        != "over 400 MW additional capacity when complete"
        or brooklyn["evidence"][0]["metadata"].get("capacity_label_as_reported")
        != "over 350 MW campus capacity"
        or maddington["evidence"][0]["metadata"].get(
            "project_value_and_capacity_as_reported"
        )
        != "$415 million, 200MW development"
        or maddington["evidence"][2]["metadata"].get(
            "capacity_label_as_reported"
        )
        != "200 MW+ total capacity upon completion"
        or gu3["capacities"][0].get("entity") != "project"
        or gu3["capacities"][0].get("metric") != "gross_facility_mw"
        or gu3["capacities"][0].get("stage") != "design"
        or float(gu3["capacities"][0].get("base")) != 4.0
        or gu3["operating_models"][0].get("entity") != "campus"
        or gu3["operating_models"][0].get("value") != "colocation"
        or gu3["lifecycle"][0].get("evidence_key")
        != "gta-gu3-alupang-running-on-genset-captured-2026-07-21"
        or gu3["evidence"][1]["metadata"].get(
            "publication_version_guardrail"
        )
        != (
            "This dynamic current-body text is retrieval-date corroboration and "
            "is not represented as text from the page's 2022 article version."
        )
        or any(
        ...
        raise OpenSeedV83Error("v83 Oceania claim guardrail differs")
    return documents
...
def _base_paths(base: Mapping[str, Any]) -> list[Path]:
    ...
    for row in base["curated_inputs"]:
        ...
        if (
            ...
            raise OpenSeedV83Error(f"accepted v82 input pin differs: {row['path']}")
        paths.append(path)
    return paths
...
def selected_inputs(
    ...
    *,
    ...
    if base.get("release_id") != "2026-07-21-open-seed-v82":
        raise OpenSeedV83Error("v83 base must be exactly accepted v82")
    rows = base.get("curated_inputs")
    if not isinstance(rows, list) or len(rows) != 433:
        raise OpenSeedV83Error("accepted v82 curated inventory differs")
    if any(not isinstance(row, dict) or set(row) != {"path", "sha256"} for row in rows):
        raise OpenSeedV83Error("accepted v82 curated rows differ")
    base_names = [row["path"] for row in rows]
    if len(set(base_names)) != 433:
        raise OpenSeedV83Error("accepted v82 input paths collide")
    ...
    if set(base_names) & (set(ADDITION_PINS) | EXCLUDED_SOURCE_PATHS):
        raise OpenSeedV83Error("v82 already contains an Oceania source")
    ...
    if base_entity_keys & ADDED_ENTITY_KEYS or base_evidence_keys & ADDED_EVIDENCE_KEYS:
        raise OpenSeedV83Error("v83 addition collides with accepted v82 semantics")
    ...
    if (
        selected[:433] != rows
        or [row["path"] for row in selected[433:]] != list(ADDITION_ORDER)
        or tuple(documents) != ADDITION_ORDER
        or len(selected) != 441
        or set(row["path"] for row in selected) & EXCLUDED_SOURCE_PATHS
    ):
        raise OpenSeedV83Error("v83 did not preserve v82 and append exactly eight inputs")
...
def _validate_database_contract(
    ...
    *,
    ...
    if actual_counts != expected_counts:
        raise OpenSeedV83Error(f"v83 database counts differ: {actual_counts}")
    with tempfile.TemporaryDirectory(
        prefix="open-seed-v83-base-", dir="/private/tmp"
    ) as td:
        prior = v69._populate_database(
            base, _base_paths(base), Path(td) / "v82.sqlite", recorded_at=recorded_at
        )
        try:
            tables = (
                "entities",
            ...
            for table in tables:
                if not _table_state(prior, table) <= _table_state(connection, table):
                    raise OpenSeedV83Error(f"v83 changed a prior database row: {table}")
            ...
            if after_keys - before_keys != ADDED_ENTITY_KEYS or before_keys - after_keys:
                raise OpenSeedV83Error("v83 database identity delta differs")
            ...
            if (
                ...
                raise OpenSeedV83Error("v83 database evidence delta differs")
        finally:
            prior.close()
    ...
    if {key for key, _ in kinds} != ADDED_ENTITY_KEYS or {
        ...
        raise OpenSeedV83Error("v83 entity-kind contract differs")
    ...
    if (
        ...
        raise OpenSeedV83Error("v83 imported claim contract differs")
    for stable_key, latitude, longitude, geometry in connection.execute(
        ...
        if expected_point is None:
            if latitude is not None or longitude is not None or geometry not in {
                ...
                raise OpenSeedV83Error(
                    f"v83 imported unsupported coordinates: {stable_key}"
                )
        elif (
            ...
            raise OpenSeedV83Error(f"v83 imported coordinate differs: {stable_key}")
    ...
    *,
    ...
    if precreated:
        if output.is_symlink() or not output.is_dir() or any(output.iterdir()):
            raise OpenSeedV83Error("precreated v83 release stage must be empty")
    else:
...
CSV_DELTA_COUNT_CONTRACT = {
    "entities.csv": (890, 15, 0),
    "evidence.csv": (574, 12, 0),
    "capacity_estimates.csv": (541, 1, 0),
    "construction_pipeline.csv": (455, 7, 0),
    "construction_source_signals.csv": (357, 7, 0),
    "resolution_candidates.csv": (7, 0, 0),
    "lifecycle_freshness.csv": (498, 8, 0),
}
...
def _validate_release_delta(stage: Path, *, recorded_at: str) -> None:
    ...
    for filename, expected in CSV_DELTA_COUNT_CONTRACT.items():
        ...
        if actual != expected:
            raise OpenSeedV83Error(
                f"v83 public CSV delta differs: {filename}: {actual}"
            )
    ...
    if set(after_entities) - set(before_entities) != ADDED_ENTITY_KEYS:
        raise OpenSeedV83Error("v83 public identity delta differs")
    if set(before_entities) - set(after_entities) or any(
        after_entities[key] != row for key, row in before_entities.items()
    ):
        raise OpenSeedV83Error("v83 changed an accepted v82 public entity")
    for stable_key in ADDED_ENTITY_KEYS:
        ...
        if expected_point is None:
            if row["latitude"] or row["longitude"] or row["geometry_json"] != "null":
                raise OpenSeedV83Error("v83 public addition gained unsupported coordinates")
        elif (
            ...
            raise OpenSeedV83Error("v83 public official coordinate differs")
        ...
        if actual_workloads != expected_workloads or len(workloads) != len(
            ...
            raise OpenSeedV83Error("v83 public workload boundary differs")
        expected_model = (
            "colocation"
            if stable_key == "curated:gta-gu3-alupang-data-center"
            else ""
        )
        if row.get("operating_model") != expected_model:
            raise OpenSeedV83Error("v83 public operating-model boundary differs")
        ...
        if expected_capacity is None:
            if capacities != []:
                raise OpenSeedV83Error("v83 public addition gained unsupported capacity")
        elif (
            len(capacities) != 1
            or capacities[0].get("metric") != "gross_facility_mw"
            ...
            raise OpenSeedV83Error("v83 public gross-facility capacity differs")
    ...
    if set(before_evidence) - set(after_evidence) or any(
        ...
        raise OpenSeedV83Error("v83 changed accepted v82 public evidence")
    added_evidence = [
        row for key, row in after_evidence.items() if key not in before_evidence
    ]
    if (
        len(added_evidence) != 12
        ...
        raise OpenSeedV83Error("v83 public evidence projection differs")
    ...
    if (
        ...
        raise OpenSeedV83Error("v83 construction-pipeline delta differs")
    before_signals = {
    ...
    for stable_key, row in signal_by_key.items():
        ...
        if expected_point is None:
            if row["representative_latitude"] or row["representative_longitude"]:
                raise OpenSeedV83Error("v83 signal gained unsupported coordinates")
        elif (
            float(row["representative_latitude"]) != expected_point[0]
            or float(row["representative_longitude"]) != expected_point[1]
        ):
            raise OpenSeedV83Error("v83 signal official coordinate differs")
    ...
    if _normalize_timestamps(before_resolution, recorded_at) != _normalize_timestamps(
        ...
        raise OpenSeedV83Error("v83 changed the resolution advisory")
    for filename in ("resolution_candidates.csv", "resolution_candidates.json"):
        if (stage / filename).read_bytes() != (BASE_RELEASE / filename).read_bytes():
            raise OpenSeedV83Error(f"v83 changed invariant release file: {filename}")
    ...
    if (
        ...
        raise OpenSeedV83Error("v83 source-input projection differs")
    ...
    if set(after_features) - set(before_features) != ADDED_ENTITY_KEYS or any(
        ...
        raise OpenSeedV83Error("v83 GeoJSON base preservation differs")
    for stable_key in ADDED_ENTITY_KEYS:
        expected_point = COORDINATE_CONTRACT.get(stable_key)
        expected_geometry = (
            None
        ...
        if after_features[stable_key]["geometry"] != expected_geometry:
            raise OpenSeedV83Error("v83 GeoJSON coordinate boundary differs")
...
def _validate_release_facts(stage: Path, *, recorded_at: str) -> None:
    ...
    expected_summary = {
        ...
        "capacity_estimates_current": 542,
        "construction_pipeline_records": 462,
        "construction_source_signals": 364,
        "entities_total": 905,
        "entities_with_coordinates": 200,
        "evidence_total": 736,
        "lifecycle_observations_current": 506,
        "projects_total": 431,
        "recorded_at": recorded_at,
    }
    if {key: summary.get(key) for key in expected_summary} != expected_summary:
        raise OpenSeedV83Error(
            f"v83 summary facts differ: "
            f"{ {key: summary.get(key) for key in expected_summary} }"
        )
    if (
        summary.get("entities_by_status", {}).get("under_construction") != 350
        or summary.get("entities_by_status", {}).get("commissioning") != 3
        or summary.get("entities_by_status", {}).get("shell") != 32
        or summary.get("entities_by_status", {}).get("operational") != 44
    ):
        raise OpenSeedV83Error("v83 last-observed status summary differs")
    if (
        ...
        or summary.get("capacity_estimates_by_stage", {}).get("design") != 22
        or summary.get("capacity_estimates_by_stage", {}).get("operational") != 172
    ):
        raise OpenSeedV83Error("v83 typed-capacity summary differs")
    ...
    expected_summary_document["country_assignment_counts"]["not_evaluated"] = 905
    expected_summary_document["country_source_claims_by_method"][
        "source_explicit_country_tag"
    ] = 905
    expected_summary_document["country_source_tag_fallbacks"] = 905
    expected_summary_document["entities_by_country"].update(
        {"Australia": 51, "Guam": 2}
    )
    expected_summary_document["entities_by_kind"] = {"campus": 474, "project": 431}
    expected_summary_document["entities_by_status"].update(
        {"operational": 44, "shell": 32, "under_construction": 350}
    )
    expected_summary_document["evidence_by_kind"].update(
        {"company_disclosure": 545, "government_record": 113}
    )
    expected_summary_document["capacity_estimates_by_metric"][
        "gross_facility_mw"
    ] = 134
    expected_summary_document["capacity_estimates_by_stage"]["design"] = 22
    if summary != expected_summary_document:
        raise OpenSeedV83Error("v83 full summary delta differs")
    ...
    if {key: manifest.get(key) for key in expected_manifest} != expected_manifest:
        raise OpenSeedV83Error("v83 release manifest facts differ")
    ...
    if (
        ...
        or len(manifest["source_families"]) != 348
    ):
        raise OpenSeedV83Error("v83 public source-family delta differs")
    ...
    if (
        ...
        raise OpenSeedV83Error("v83 full manifest delta differs")
    ...
    if (
        ...
        or classes
        != {
            "recent_0_90_days": 266,
            "aging_91_365_days": 208,
        ...
        raise OpenSeedV83Error("v83 freshness/current-status boundary differs")
    expected_freshness = {
        "curated:airtrunk-syd3-western-sydney-campus:current-build": (
            "under_construction",
            "2026-03-26",
    ...
    for stable_key, expected in expected_freshness.items():
        ...
        if actual != expected:
            raise OpenSeedV83Error(f"v83 source freshness differs: {stable_key}")
    ...
    if projected_capacities != {
        ...
        raise OpenSeedV83Error("v83 public capacity boundary differs")

    readme = (stage / "README.md").read_text()
    for marker in (
        OFFICIAL_MANIFEST_PIN[1],
        OFFICIAL_PHYSICAL_TREE_SHA256,
        "All 17 other carrier candidates remain review-only",
        "AirTrunk's conflicting 320+ MW IT-load and 400+ MW total-capacity",
        "CDC's campus, regional, and untyped figures",
        "GU3 contributes only 4 MW gross facility capacity",
        "design stage and generic colocation; no workload is inferred",
        "dynamic About\ntext remains retrieval-date identity corroboration",
        "Guam JIC record\nanchors the dated operational observation",
        "current_status_classification`\nremains `unknown",
        "current_construction_claim` remains `false",
    ):
        if marker not in readme:
            raise OpenSeedV83Error(f"v83 README guardrail differs: {marker}")
    if len(list(stage.iterdir())) != 14:
        raise OpenSeedV83Error("v83 release file inventory count differs")
...
def _validate_definition(
    ...
    *,
    ...
    if set(document) != set(base) or document.get("release_id") != RELEASE_ID:
        raise OpenSeedV83Error("v83 definition identity or schema differs")
    build = document.get("build")
    if not isinstance(build, dict) or set(build) != {"as_of", "recorded_at"}:
        raise OpenSeedV83Error("v83 definition build carrier differs")
    if build["as_of"] != AS_OF:
        raise OpenSeedV83Error("v83 as_of differs")
    recorded = v70.parse_utc(build["recorded_at"], label="v83 recorded_at")
    if (
    ...
    for key in (
        ...
        if document.get(key) != base.get(key):
            raise OpenSeedV83Error(f"v83 inherited definition field differs: {key}")
...
def _validate_publication_times(
    ...
    *,
    ...
    for path in (definition, release, *release.iterdir()):
        ...
        if max(metadata.st_birthtime, metadata.st_mtime) > target.timestamp() + 1e-6:
            raise OpenSeedV83Error(f"v83 staged inode post-dates recorded_at: {path.name}")
    if require_live:
        if datetime.now(UTC) < target:
            raise OpenSeedV83Error("v83 recorded_at is not live")
        for path in (definition, release):
            if path.stat(follow_symlinks=False).st_ctime + 1e-6 < target.timestamp():
                raise OpenSeedV83Error(
                    f"v83 final root ctime predates recorded_at: {path.name}"
                )


def validate_open_seed_v83(
    ...
    *,
    ...
    if replay_count != 2:
        raise OpenSeedV83Error("v83 requires exactly two offline replays")
    wall = validation_wall_clock or datetime.now(UTC)
    guard = _guard_state()
    if guard["base_definition"] != BASE_DEFINITION_PIN:
        raise OpenSeedV83Error("accepted v82 definition pin differs")
    if (
        guard["base_manifest"] != BASE_MANIFEST_SHA256
        or guard["base_tree"] != BASE_TREE_SHA256
        or guard["base_entities"] != BASE_ENTITIES_PIN
    ):
        raise OpenSeedV83Error("accepted v82 release pin differs")
    if guard["excluded"] != EXCLUDED_SOURCE_PINS:
        raise OpenSeedV83Error("v83 excluded source pin differs")
    ...
    if definition.get("curated_inputs") != selected:
        raise OpenSeedV83Error("v83 selected input inventory differs")
    if release_path.is_symlink() or not release_path.is_dir():
        raise OpenSeedV83Error("v83 release must be an ordinary directory")
    if require_frozen and stat.S_IMODE(release_path.stat().st_mode) != 0o555:
        raise OpenSeedV83Error("v83 release root is not frozen")
    ...
    if any(path.is_symlink() or not path.is_file() for path in release_files.values()):
        raise OpenSeedV83Error("v83 release contains a non-file")
    if require_frozen and any(
        stat.S_IMODE(path.stat().st_mode) != 0o444 for path in release_files.values()
    ):
        raise OpenSeedV83Error("v83 release file is not frozen")
    ...
    if _sha256(manifest_raw) != definition["expected_release"].get(
        ...
        raise OpenSeedV83Error("v83 manifest hash differs")
    expected_release = {
        key: value
    ...
    if {key: summary[key] for key in definition["expected_summary"]} != definition[
        ...
        raise OpenSeedV83Error("v83 expected summary differs")

    for replay in range(replay_count):
        with tempfile.TemporaryDirectory(
            prefix=f"open-seed-v83-replay-{replay + 1}-", dir="/private/tmp"
        ) as td:
            root = Path(td)
            ...
            if {path.name for path in replay_release.iterdir()} != set(release_files):
                raise OpenSeedV83Error("v83 replay file inventory differs")
            for filename, frozen in release_files.items():
                if (replay_release / filename).read_bytes() != frozen.read_bytes():
                    raise OpenSeedV83Error(f"v83 offline replay differs: {filename}")
    if _guard_state() != guard:
        raise OpenSeedV83Error("v83 validation mutated accepted inputs")
    return manifest
...
def _path_identity(path: Path, *, directory: bool) -> tuple[int, int]:
    ...
    if not expected:
        raise OpenSeedV83Error(f"v83 stage type differs: {path}")
...
def _assert_release_identities(
    ...
    if _path_identity(root, directory=True) != root_identity:
        raise OpenSeedV83Error("v83 release stage root identity changed")
    if _release_identities(root) != dict(members):
        raise OpenSeedV83Error("v83 release stage member identity changed")
...
def _discard_file_stage(path: Path, identity: tuple[int, int]) -> None:
    ...
    if _path_identity(path, directory=False) != identity:
        raise OpenSeedV83Error("refusing substituted v83 definition cleanup")
    path.chmod(0o600)
    path.unlink()
...
def _publication_lock() -> Iterator[None]:
    try:
        ...
        raise OpenSeedV83Error("active v83 publication lock exists") from error
    ...
    try:
        ...
        try:
            ...
            if not stat.S_ISREG(current.st_mode) or (
                ...
                raise OpenSeedV83Error("refusing substituted v83 lock cleanup")
...
def _require_absent(label: str) -> None:
    if DEFINITION.exists() or DEFINITION.is_symlink():
        raise OpenSeedV83Error(f"{label} v83 definition collision")
    if RELEASE.exists() or RELEASE.is_symlink():
        raise OpenSeedV83Error(f"{label} v83 release collision")
...
def _rollback_release(release_identity: tuple[int, int], release_stage: Path) -> None:
    if _path_identity(RELEASE, directory=True) != release_identity:
        raise OpenSeedV83Error("refusing rollback of substituted v83 release")
    if release_stage.exists() or release_stage.is_symlink():
        raise OpenSeedV83Error("v83 release rollback stage is occupied")
    v69.promote_noreplace(RELEASE, release_stage)


def build_open_seed_v83(recorded_at: str | None = None) -> dict[str, Any]:
    """Build and atomically publish the eight-source v82 successor."""

    if (DEFINITION.exists() or DEFINITION.is_symlink()) and (
        RELEASE.exists() or RELEASE.is_symlink()
    ):
        manifest = validate_open_seed_v83()
        return {
    ...
    if (
        ...
        raise OpenSeedV83Error("partial v83 final-path collision")
    ...
    if guard["excluded"] != EXCLUDED_SOURCE_PINS:
        raise OpenSeedV83Error("v83 excluded source pin differs")
    target = (
        v70.parse_utc(recorded_at, label="v83 recorded_at")
        if recorded_at is not None
        else datetime.now(UTC).replace(microsecond=0) + timedelta(seconds=45)
    )
    if datetime.now(UTC) >= target:
        raise OpenSeedV83Error("v83 recorded_at must be future before staging")
    ...
    with _publication_lock():
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(
                prefix="open-seed-v83-db-", dir="/private/tmp"
            ) as td:
                connection = _build_database(
            ...
            if _path_identity(definition_stage, directory=False) != definition_identity:
                raise OpenSeedV83Error("v83 definition stage identity changed")
            ...
            if (
                ...
                raise OpenSeedV83Error("v83 private stage changed while waiting")
            _validate_publication_times(
                definition_stage,
                release_stage,
                recorded_at=recorded_at,
            ...
            try:
                ...
                try:
                    ...
                    error.add_note(f"v83 release rollback failed: {rollback_error}")
                raise
            manifest = validate_open_seed_v83(DEFINITION, RELEASE)
        finally:
    ...
    if _guard_state() != guard:
        raise OpenSeedV83Error("v83 build mutated accepted inputs")
    return {
        "definition": str(DEFINITION),
        "definition_sha256": v69.sha256(DEFINITION),
        "manifest_sha256": v69.sha256(RELEASE / "manifest.json"),
...
def main() -> int:
    print(json.dumps(build_open_seed_v83(), indent=2, sort_keys=True))
    return 0
