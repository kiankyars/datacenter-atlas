#!/usr/bin/env python3
"""Build or offline-validate the immutable v22 construction master."""
...
from datacenter_atlas.construction_master_v6 import (  # noqa: E402
...
def main() -> int:
    ...
    try:
        if arguments.validate_only:
            manifest = validate_construction_master_v6(
                arguments.output, definition_path=arguments.definition
            )
        else:
            manifest = write_construction_master_v6(
                arguments.definition,
                arguments.output,
                freeze=arguments.freeze,
            )
    except ConstructionMasterV6Error as error:
        print(f"construction-master-v6 error: {error}", file=sys.stderr)
        return 2
    print(
        json.dumps(
            {
