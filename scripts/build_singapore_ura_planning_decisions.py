#!/usr/bin/env python3
"""Build the Singapore URA fail-closed assessment without network access."""
...
def _write_definition(path: Path) -> None:
    ...
    if path.parent.is_symlink() or not path.parent.is_dir():
        raise ValueError("definition parent must be a regular directory")
        ...
        json.dumps(
            {
                ...
                "credentialed_requests": assessment["retrieval_batch"][
                    "credentialed_requests"
                ],
                "release_id": RELEASE_ID,
                "result_bearing_requests": assessment["retrieval_batch"][
