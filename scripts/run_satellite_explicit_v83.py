#!/usr/bin/env python3
"""Run the immutable four-project open-seed v83 catalog selection."""
...
from datacenter_atlas.satellite_queue_v83 import QUEUE


class ExplicitV83RunnerLockError(RuntimeError):
...
def _single_writer_lock(output: Path) -> Iterator[None]:
    ...
    if lock.is_symlink():
        raise ExplicitV83RunnerLockError("explicit v83 runner lock may not be a symlink")
    flags = os.O_RDWR | os.O_CREAT | getattr(os, "O_NOFOLLOW", 0)
    descriptor = os.open(lock, flags, 0o600)
    locked = False
    try:
        if not stat.S_ISREG(os.fstat(descriptor).st_mode):
            raise ExplicitV83RunnerLockError("explicit v83 runner lock is not a regular file")
        try:
            ...
            if error.errno not in {errno.EACCES, errno.EAGAIN}:
                raise
            raise ExplicitV83RunnerLockError(
                f"another explicit satellite runner holds {lock}"
            ) from error
        opened = os.fstat(descriptor)
        current = os.stat(lock, follow_symlinks=False)
        if (opened.st_dev, opened.st_ino) != (current.st_dev, current.st_ino):
            raise ExplicitV83RunnerLockError("explicit v83 runner lock changed during open")
        yield
    finally:
        if locked:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except ExplicitV83RunnerLockError as error:
