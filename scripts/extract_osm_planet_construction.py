#!/usr/bin/env python3
"""Extract a bounded review-only structural construction layer from OSM Planet."""
...
SCHEMA_VERSION = 1
OUTPUT_FILENAME = (
    f"planet-{planet_fetch.PLANET_DATE_TOKEN}-structural-construction-review.osm.pbf"
)
MANIFEST_FILENAME = "construction-extract-manifest.json"
GENERATOR = "DataCenterAtlas/0.1 planet-structural-construction-review"
...
def build_command_plan(
    ...
    *,
    ...
    if not output.name.endswith(".osm.pbf"):
        raise exact.ExtractionError("construction extraction output must end with .osm.pbf")
    if output.parent != manifest.parent:
        raise exact.ExtractionError("construction output and manifest must share one directory")
...
def _validate_manifest(
    ...
    *,
    ...
    for field, value in expected.items():
        if document.get(field) != value:
            raise exact.ExtractionError(
                f"construction extraction manifest {field} does not match"
            )
    output = document.get("output")
    if not isinstance(output, Mapping) or output.get("path") != plan.output_path.name:
        raise exact.ExtractionError("construction extraction output record is invalid")
    actual = exact._output_hashes(plan.output_path)
    for field in ("bytes", "md5", "sha256"):
        if output.get(field) != actual[field]:
            raise exact.ExtractionError(
                f"construction extraction output.{field} does not match"
            )
    expected_commands = {
    ...
    if (
        ...
        raise exact.ExtractionError("construction extraction tool contract does not match")
    if document.get("referential_integrity_check") != {
        "command": list(plan.check_refs_command),
        "relations_checked": True,
        "passed": True,
    }:
        raise exact.ExtractionError("construction reference check does not match")
    fileinfo = document.get("fileinfo")
    if not isinstance(fileinfo, Mapping):
        raise exact.ExtractionError("construction extraction fileinfo is missing")
    if fileinfo.get("file", {}).get("name") != plan.output_path.name:
        raise exact.ExtractionError("construction fileinfo filename does not match")
    if fileinfo.get("file", {}).get("size") != actual["bytes"]:
        raise exact.ExtractionError("construction fileinfo size does not match")
    counts = fileinfo.get("object_counts_including_references", {})
    for object_type in ("nodes", "ways", "relations"):
        exact._nonnegative_integer(counts.get(object_type), field=f"construction.{object_type}")
...
def _existing(
    ...
    if output_exists != manifest_exists:
        raise exact.ExtractionError(
            "existing construction extraction is incomplete; output and manifest must coexist"
        )
    exact._require_regular_file(plan.output_path, label="existing construction PBF")
    document, _ = exact._read_json_file(
        plan.manifest_path, label="construction extraction manifest"
    )
    _validate_manifest(document, plan=plan, source=source)
    return document


def extract_construction_planet(
    source_path: str | Path,
    output_path: str | Path,
    manifest_path: str | Path,
    *,
    ...
    try:
        ...
        exact._run_command(
            runner, plan.tags_filter_command, label="osmium construction tags-filter"
        )
        exact._require_regular_file(
            plan.temporary_output_path, label="temporary construction PBF"
        )
        hashes = exact._output_hashes(plan.temporary_output_path)
        if hashes["bytes"] <= 0:
            raise exact.ExtractionError("construction tags-filter produced an empty PBF")
        exact._run_command(
            runner, plan.check_refs_command, label="osmium construction check-refs"
        )
        fileinfo = exact.normalize_fileinfo(
            exact._run_command(
                runner, plan.fileinfo_command, label="osmium construction fileinfo"
            ),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        result = extract_construction_planet(
            source,
            output,
            manifest,
