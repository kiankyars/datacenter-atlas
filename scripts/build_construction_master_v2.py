#!/usr/bin/env python3
"""Build or offline-validate the immutable role-preserving master v2."""
...
from datacenter_atlas.construction_master_v2 import (  # noqa: E402
...
def main() -> int:
    ...
    try:
        if arguments.validate_only:
            manifest = validate_construction_master_v2(
                arguments.output, definition_path=arguments.definition
            )
        else:
            manifest = write_construction_master_v2(
                arguments.definition,
                arguments.output,
                freeze=arguments.freeze,
            )
    except ConstructionMasterV2Error as error:
        print(f"construction-master-v2 error: {error}", file=sys.stderr)
        return 2
    print(
        json.dumps(
            {
