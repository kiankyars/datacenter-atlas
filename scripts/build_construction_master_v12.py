#!/usr/bin/env python3
"""Prepare, publish, or offline-validate the immutable v28 construction master."""
...
from datacenter_atlas.construction_master_v12 import (  # noqa: E402
    BUNDLE_PATH,
    DEFINITION_PATH,
...
def main() -> int:
    ...
    try:
        if arguments.prepare_only:
            transaction, definition, bundle = prepare_construction_master_v28()
            manifest = validate_construction_master_v12(
            ...
            if arguments.discard_prepared:
                discard_construction_master_v28_stage(transaction)
        elif arguments.publish:
            manifest = publish_construction_master_v28()
            print(
                json.dumps(
                    {
    ...
    except (ConstructionMasterV12Error, OSError, SystemExit) as error:
        print(f"construction-master-v12 error: {error}", file=sys.stderr)
        return 2
    return 0
