#!/usr/bin/env python3
"""Preflight or build current-coverage ledger v24 once all pins are sealed."""
...
from datacenter_atlas.current_coverage_v24 import (  # noqa: E402
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        "--definition",
        type=Path,
        default=PACKAGE_ROOT / "sources/current-coverage-2026-07-21-v24.json",
    )
    result.add_argument(
        "--output",
        type=Path,
        default=PACKAGE_ROOT / "current_coverage_ledgers/2026-07-21-v24",
    )
...
def main() -> int:
    ...
    if selected > 1:
        print("current-coverage-v24 error: select only one mode", file=sys.stderr)
        return 2
    try:
        if arguments.preflight:
            payload = preflight_v24_publication(PACKAGE_ROOT)
        elif arguments.publish:
            if not arguments.generated_at:
                raise CurrentCoverageV24Error(
                    "--generated-at is required with --publish"
                )
            payload = publish_current_coverage_v24(
            ...
            payload = validate_current_coverage_ledger_v24(
                arguments.output, definition_path=arguments.definition
            )
        else:
            payload = write_current_coverage_ledger_v24(
                arguments.definition, arguments.output, freeze=arguments.freeze
            )
    except CurrentCoverageV24Error as error:
        print(f"current-coverage-v24 error: {error}", file=sys.stderr)
        return 2
